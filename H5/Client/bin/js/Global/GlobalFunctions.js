...
(function (Global) {
    ...
    Global.CalcOriginalOccupationID = CalcOriginalOccupationID;
    /**
     * 根据职业获取对应角色的骨架资源的名称
     * @param occupation 指定要获取骨架资源名称的职业
     */
    function getSkeletonNameByOccupation(occupation) {
    ...
    Global.getSkeletonNameByOccupation = getSkeletonNameByOccupation;
    /**
     * 获取裸模的部件名称列表.它必须与BodyPartIDs枚举的有效部位相符
     */
    function getNakePartsList(occupation) {
        switch (occupation) {
    ...
    Global.getNakePartsList = getNakePartsList;
    /**
     ...
     */
    function changePlayerEquip(playerView, equipDataList) {
        // to do ... 如果装备中有时装,则获取时装的道具列表.
        playerView.changeEquip(equipDataList);
    }
    Global.changePlayerEquip = changePlayerEquip;
    /**
     * 从身体部位到物品类型的转换
     * @param bodyPartId
     */
    function getItemCategoriyByBodyPartId(bodyPartId) {
        return bodyPartId;
    }
    Global.getItemCategoriyByBodyPartId = getItemCategoriyByBodyPartId;
    /**
     * 由物品类型转换成对应的身体装配位
     * @param itemCategoriy 指定要转换的物品类型
     */
    function getBodySlotByItemCategoriy(itemCategoriy) {
        if (itemCategoriy <= BodyPartIDs.Invalid ||
            itemCategoriy >= BodyPartIDs.Max)
            return BodyPartIDs.Invalid;
        return itemCategoriy;
    }
    Global.getBodySlotByItemCategoriy = getBodySlotByItemCategoriy;
    /**
     * 由物品的类型来判定是否属于武器
     * @param itemCategoriy
     */
    function isWeapon(itemCategoriy) {
        if (itemCategoriy >= ItemCategories.WeaponStart &&
            itemCategoriy <= ItemCategories.WeaponEnd)
            return true;
        return false;
    }
    Global.isWeapon = isWeapon;
    var Quaternion = Laya.Quaternion;
    /**
     *  将dir转换成Quaternion
     * @param dir 指定要转换成弧度的度数(服务器坐标系)
     */
    function GetQuaternionByDir(dir) {
        let dirInDegree = 0;
        if (0 === dir) {
            dirInDegree = 0;
        }
        ...
        Quaternion.createFromYawPitchRoll(Laya.Utils.toRadian(dirInDegree), 0, 0, target);
        return target;
    }
    Global.GetQuaternionByDir = GetQuaternionByDir;
    /**
     * 把GS坐标系的角度转换成Laya坐标系的角度
     * @param angle GS坐标系的角度(度)
     */
    function GSAngle2Laya(angle) {
        return 360 - angle + 180; // 服务器的x轴与Layabox的坐标轴相反的
    }
    Global.GSAngle2Laya = GSAngle2Laya;
    /**
     * 把GameServer的x轴坐标转换成Laya的. 注:GameServer(Unity3D)是左手坐标系,LayaBox是右手坐标系
     * @param posX 指定要转换的U3D坐标x
     */
    function GSCoordX2Laya(posX) {
        return -posX / 100;
    }
    Global.GSCoordX2Laya = GSCoordX2Laya;
    /**
     * 把GameServer的y轴坐标转换成Laya的. 注:GameServer(Unity3D)是左手坐标系,LayaBox是右手坐标系
     * @param posY 指定要转换的U3D坐标y
     */
    function GSCoordY2Laya(posY) {
        return posY / 100;
    }
    Global.GSCoordY2Laya = GSCoordY2Laya;
    /**
     * 把给定的限定在给定的范围内
     * @param value 要限定的值
     * @param min 最小值
    ...
    Global.getDeltaTime = getDeltaTime;
    /// <summary>
    ...
    function ClampMagnitude(vector, maxLength) {
        if (Global.Vector3.scalarLengthSquared(vector) > maxLength * maxLength) {
    ...
    Global.ClampMagnitude = ClampMagnitude;
    ...
    function SmoothDamp(current, target, currentVelocity, smoothTime, maxSpeed = 999, deltaTime = getDeltaTime()) {
        // 从Unity3D中抄来的算法
        ...
        if (Global.Vector3.dot(vector2.subtractClone(current), vector4.subtractClone(vector2)) > 0) {
            // vector4 = vector2;
     ...
     */
    function GetAngleRangeByDirection(/*direction: number*/ angleRotation, angleLimit) {
        let nLoAngle = 0;
        let nHiAngle = 360;
        ...
        if (nHiAngle >= 360)
            nHiAngle = nHiAngle - 360;
        nLoAngle = centerAngle - (angleLimit / 2);
        if (nLoAngle < 0)
            nLoAngle = 360 + nLoAngle;
        return { loAngle: nLoAngle, hiAngle: nHiAngle };
    }
    Global.GetAngleRangeByDirection = GetAngleRangeByDirection;
    /**
     ...
     */
    function InCircleByAngle(target, center, radius, loAngle, hiAngle) {
    ...
    Global.InCircleByAngle = InCircleByAngle;
    /**
     * 射线p0->p1相对于Y轴负向顺时针旋转角度
     * @param p0 p0点
     * @param p1 p1点
     */
    function GetCircleAngle(p0, p1) {
    ...
    Global.GetCircleAngle = GetCircleAngle;
    /**
     * 求角度是否在某个角度范围内
     * @param angle 指定的角度
     * @param loAngle 角度范围下限值
     * @param hiAngle 角度范围上限值
     */
    function InAngleRange(angle, loAngle, hiAngle) {
        if (hiAngle > loAngle) {
            return (angle >= loAngle && angle <= hiAngle);
        }
        return (angle >= loAngle && angle < 360) || (angle >= 0 && angle <= hiAngle);
    }
    Global.InAngleRange = InAngleRange;
    /**
     * 求两点间的距离
     * @param start 开始的点
     * @param end 结束的点
     */
    function GetTwoPointDistance(start, end) {
        return start.distance(end.x, end.y);
    }
    Global.GetTwoPointDistance = GetTwoPointDistance;
    /**
     * 格式化带战区的角色名称
     * @param zoneID 指定战区Id
     * @param roleName 指定角色名称
     */
    function formatRoleNameZoneid(zoneID, roleName) {
        if (zoneID) {
            return Global.String.Format(Loca.getLang("[{0}区]{1}"), zoneID, roleName);
        }
        return roleName;
    }
    Global.formatRoleNameZoneid = formatRoleNameZoneid;
    function ParsePropNameInfo(propName, goodsName, forge_level, quality) {
        goodsName = propName;
        forge_level = 0;
        quality = 0;
        if (Global.String.IsNullOrWhiteSpace(propName)) {
            return;
        }
    ...
    Global.ParsePropNameInfo = ParsePropNameInfo;
    /**
     * 以Html的形式来格式化给定的参数: 参数依次是颜色，字符串，颜色，字符串
     * @param _args 传入要格式化的颜色与字符串
     */
    function GetColorStringForNGUIText(...args) {
        let retString = "";
    ...
    Global.GetColorStringForNGUIText = GetColorStringForNGUIText;
    /**
     * 获取NPC或怪物的地图Id
     * @param mapCode 指定原始地图ID,如果不存在,则使用玩家所在的地图Id
     */
    function getNPCOrMonsterMapCodeByID(mapCode) {
        if (mapCode < 0) {
            return gameIns.gameState.roleData.MapCode;
        }
        return mapCode;
    }
    Global.getNPCOrMonsterMapCodeByID = getNPCOrMonsterMapCodeByID;
    /**
     ...
     * 注: 此函数是异步的,要注意!!!
     */
    function GetMonsterPointByID(mapCode, monsterID) {
        return __awaiter(this, void 0, void 0, function* () {
            const table = yield tableMgr.getLevelMonstersTable(mapCode);
            if (!table) {
                return null;
            }
    ...
    Global.GetMonsterPointByID = GetMonsterPointByID;
    /**
     * 根据关卡Id和npcID获取所在的位置
     * @param mapCode 指定关卡
     ...
     * 注: 此函数是异步的,一定要注意!!!
     */
    function getNPCPointByID(mapCode, npcID) {
        return __awaiter(this, void 0, void 0, function* () {
            const table = yield tableMgr.getLevelNpcTable(mapCode);
            if (table) {
    ...
    Global.getNPCPointByID = getNPCPointByID;
    /**
     * 获取系统配置参数中的整型参数
     * @param name
     * @returns {}
     */
    function GetSystemParamIntByName(name) {
    ...
    Global.GetSystemParamIntByName = GetSystemParamIntByName;
    /**
     * 把给定的限定在给定的范围内
     * @param target 目标点
     * @param center 圆心坐标
     * @param radius 圆半径
     */
    function inCircle(target, center, radius) {
        const lenght1 = Math.pow(target.x - center.x, 2) + Math.pow(target.y - center.y, 2);
        const lenght2 = Math.pow(radius, 2);
        return lenght1 <= lenght2;
    }
    Global.inCircle = inCircle;
    /**
     * 根据两点及延伸长度获取延伸后的点
     * @param start         起点
     * @param end           终点
     * @param length        距离
     */
    function getExtensionPoint(start, end, length) {
        const scale = Math.sqrt(Math.pow(end.y - start.y, 2) + Math.pow(end.x - start.x, 2)) / length;
        return new Laya.Point((start.x + (end.x - start.x) / scale) | 0, (start.y + (end.y - start.y) / scale) | 0);
    }
    Global.getExtensionPoint = getExtensionPoint;
    /**
     * 通过正切值获取精灵的朝向代号
     ...
     */
    function getDirectionByTan(targetX, targetY, currentX, currentY) {
        let direction = 0;
        if (targetX < currentX) {
            if (targetY < currentY) {
                direction = 5;
    ...
    Global.getDirectionByTan = getDirectionByTan;
    /**
     * 获取道具类型名称
     * @param value 道具分类
     */
    function GetGoodsType(value) {
        if (isWeapon(value)) {
    ...
    Global.GetGoodsType = GetGoodsType;
    /**
     * 获取道具限制职业
     * @param occup 职业ID
     */
    function GetOccupationStrByGoods(occup) {
        const aOccupation = [];
        if (0 !== (1 & occup)) {
    ...
    Global.GetOccupationStrByGoods = GetOccupationStrByGoods;
    /**
     * 根据货币类型返回主角拥有的数量
     * @param moneyType
     */
    function GetRoleOwnNumByMoneyType(moneyType) {
        let hasNum = 0;
        switch (moneyType) {
            case MoneyTypes.TongQian:
                hasNum = Global.Data.roleData.Money1;
                break;
            case MoneyTypes.YinLiang:
                hasNum = Global.Data.roleData.YinLiang;
                break;
            case MoneyTypes.BindYuanBao:
                hasNum = Global.Data.roleData.Gold;
                break;
            case MoneyTypes.YuanBao:
                hasNum = Global.Data.roleData.UserMoney;
                break;
            case MoneyTypes.JingYuanZhi:
                hasNum = Global.GetRoleCommonUseParamsValue(RoleCommonUseIntParamsIndexs.TianDiJingYuan);
                break;
            case MoneyTypes.JunGongZhi:
                hasNum = Global.GetRoleCommonUseParamsValue(RoleCommonUseIntParamsIndexs.JunGong);
                break;
            case MoneyTypes.LieShaZhi:
                hasNum = Global.GetRoleCommonUseParamsValue(RoleCommonUseIntParamsIndexs.LieShaZhi);
                break;
            case MoneyTypes.JiFenZhi:
                hasNum = Global.GetRoleCommonUseParamsValue(RoleCommonUseIntParamsIndexs.ZhuangBeiJiFen);
                break;
            case MoneyTypes.ZhanHun:
                hasNum = Global.GetRoleCommonUseParamsValue(RoleCommonUseIntParamsIndexs.ZhanHun);
                break;
            case MoneyTypes.KingOfBattlePoint:
                hasNum = Global.GetRoleCommonUseParamsValue(RoleCommonUseIntParamsIndexs.KingOfBattlePoint);
                break;
            default:
                break;
        }
    ...
    Global.GetRoleOwnNumByMoneyType = GetRoleOwnNumByMoneyType;
    /**
     * 取整数的某一位，返回某一位的值（0或者1）
     * @param resource 要取某一位的整数
     * @param mask 要取的位置索引，自右至左为0-7
     */
    function GetIntSomeBit(resource, mask) {
        return resource >> mask & 1;
    }
    Global.GetIntSomeBit = GetIntSomeBit;
    /**
     * 得到物品的类型
     * @param goodsID
     */
    function GetCategoriyByGoodsID(goodsID) {
        return tableMgr.goodsTable.getCategoriy(goodsID);
    }
    Global.GetCategoriyByGoodsID = GetCategoriyByGoodsID;
    /**
     * 获取道具与人物身上装备对比后各项属性的差值
     * @param gd 装备数据
     * @param CompareType 武器的手持状态
     */
    function GetCompareAttributeInfo(gd, CompareType = HandTypes.None) {
        let AttributeInfoDic = new Map();
                ...
                if ((3 === Global.Data.roleData.Occupation) && (actionType === WeaponStates.D)) { // 魔剑士做特殊处理
                    if (gdRight == null || gdLeft == null) {
                        if (gdRight != null) {
                            ...
                            const iRightHandType = rGoodVO.HandType;
                            const iRightActionType = rGoodVO.ActionType;
                            if (iRightActionType === WeaponStates.S || iRightActionType === WeaponStates.MJ) { // 如果右手是双手武器就比较
                                AttributeInfoDic = CalculatAttribute(gdAttributeDic, iRightAttributegDic, "JIAN");
                            }
                            else {
                                AttributeInfoDic = gdAttributeDic;
    ...
    Global.GetCompareAttributeInfo = GetCompareAttributeInfo;
    /**
     * 获取装备上面基本属性+强化\追加\重生\卓越等信息相加值
     * @param gd 装备数据
     */
    function GetGoodsBasicAttribute(gd) {
        if (null == gd) {
            return null;
        }
        const basicAttributeDic = new Map();
     ...
     */
    function GetAttributeAddBaseValue(gd, da, extPropIndex) {
        // 直接返回原属性
        ...
        if (ItemCategories.ChongWu !== categoriy && ItemCategories.ShouHuChong !== categoriy && ItemCategories.MengChongWu !== categoriy) {
            zhuoyueRates = (gd.ExcellenceInfo > 0) ? Global.GetZhuoYueAddAttackRates(gd.ExcellenceInfo) : 0;
        }
        else {
            zhuoyueRates = 0;
        }
        let qianghuaRates = Global.GetEquipForgeAddBaseValue(gd, extPropIndex);
        let zhuijiaRates = Global.GetEquipZhuijiaAddBaseValue(gd, extPropIndex);
        const zhuanshengRates = Global.GetEquipZhuanshengAddBaseValue(gd, extPropIndex);
        // 结婚戒指的处理
     ...
     * @param gd 装备数据
     */
    function GetGoodsDataZhanLi(gd) {
        let iGoodsZhanLi = 0;
        const goodVO = tableMgr.goodsTable.Find(gd.GoodsID);
        if (!goodVO) {
            return iGoodsZhanLi;
    ...
    Global.GetGoodsDataZhanLi = GetGoodsDataZhanLi;
    /**
     * 获取GoodsData指定索引的WashProps属性
     * @param gd 装备数据
     * @param extPropIndexes 属性索引
     */
    function GetGoodsWashProps(gd, extPropIndexes) {
        let propV = 0;
        if (gd.WashProps != null) {
     ...
     * @param goodsID 道具ID
     */
    function GetGoodsActionNameByID(goodsID) {
        const goodVO = tableMgr.goodsTable.Find(goodsID);
        if (!goodVO)
            return WeaponStates.K;
        return goodVO.ActionType;
    }
    Global.GetGoodsActionNameByID = GetGoodsActionNameByID;
    /**
     * 计算对比装备的属性差值
     * @param addDtc
     * @param beaddDtc
     * @param type
     */
    function CalculatAttribute(addDtc, beaddDtc, type = "JIAN") {
        if (!addDtc || !beaddDtc) {
    ...
    function tryAddRecommendPoint() {
        ...
        switch (nOccu) {
            ...
            case EnumOccupation.DouXian: // 斗仙
                ...
                break;
            default:
                if (Global.currentMJSType === MJSSkillType.Strength_Sword)
                    addStrengthWeight = 5; // 力剑士
                else
                    addIntelligenceWeight = 5; // 魔剑士
                break;
        }
        do {
            let nTotalWeight = nRemainPoint; // 参与计算权重的点数（只有未达到推荐点数的属性才参与计算）
            if (addStrengthWeight > 0)
                nTotalWeight += nStrengthPoint;
            if (addIntelligenceWeight > 0)
                nTotalWeight += nIntelligencePoint;
            if (addDexterityWeight > 0)
                nTotalWeight += nDexterityPoint;
            if (addConstitutionWeight > 0)
                nTotalWeight += nConstitutionPoint;
            ...
            if (addStrengthWeight > 0)
                addStrengthPoint = Math.floor(nTotalWeight * addStrengthWeight / (addConstitutionWeight + addStrengthWeight + addIntelligenceWeight + addDexterityWeight) - nStrengthPoint);
            if (addIntelligenceWeight > 0)
                addIntelligencePoint = Math.floor(nTotalWeight * addIntelligenceWeight / (addConstitutionWeight + addStrengthWeight + addIntelligenceWeight + addDexterityWeight) - nIntelligencePoint);
            if (addDexterityWeight > 0)
                addDexterityPoint = Math.floor(nTotalWeight * addDexterityWeight / (addConstitutionWeight + addStrengthWeight + addIntelligenceWeight + addDexterityWeight) - nDexterityPoint);
            if (addConstitutionWeight > 0)
                addConstitutionPoint = Math.floor(nTotalWeight * addConstitutionWeight / (addConstitutionWeight + addStrengthWeight + addIntelligenceWeight + addDexterityWeight) - nConstitutionPoint);
            // 剩余尾数,按优先级（代码顺序）分配
            const nFinalRemain = nRemainPoint - (addStrengthPoint + addIntelligencePoint + addDexterityPoint + addConstitutionPoint);
            if (addConstitutionWeight > 0)
                addConstitutionPoint += nFinalRemain;
            else if (addStrengthWeight > 0)
                addStrengthPoint += nFinalRemain;
            else if (addIntelligenceWeight > 0)
                addIntelligencePoint += nFinalRemain;
            else if (addDexterityWeight > 0)
                addDexterityPoint += nFinalRemain;
            break;
        } while (true);
        // 发送推荐加点消息
        Net.sendRecommendPoint(addStrengthPoint, addIntelligencePoint, addDexterityPoint, addConstitutionPoint);
    }
    Global.tryAddRecommendPoint = tryAddRecommendPoint;
    /**
     * 获取角色属性
     * @param level 几级属性（一级属性/扩展属性）
     * @param index 属性索引
     */
    function GetCurrentRoleProp(level, index) {
        let result = 0;
    ...
    Global.GetCurrentRoleProp = GetCurrentRoleProp;
    /**
     * 根据宠物数据获取宠物属性值
     * @param goodData 宠物数据
     */
    function GetPetAttribute(goodData) {
        const lJibanState = [];
        const attrList = [];
        const goodVO = tableMgr.goodsTable.Find(goodData.GoodsID);
        if (goodVO != null) {
            const dJibanAttr = new Map();
    ...
    Global.GetPetAttribute = GetPetAttribute;
    /**
     * 获取对应ID的萌宠
     * @param nQichongDBID
     */
    function GetJibanMengchong(nQichongDBID) {
        const lst = Global.Data.roleData.MengChongsInBag;
     ...
     * @param gd
     */
    function GetJingLingRecoverAward(gd) {
        const all = [0, 0, 0];
        if (gd) {
            const goodVO = tableMgr.goodsTable.Find(gd.GoodsID);
    ...
    Global.GetJingLingRecoverAward = GetJingLingRecoverAward;
    let _LastMoveData = null;
    let _LastSendMoveTick = 0;
    const _SendMoveInterval = 200;
    /**
     * 清空移动数据
     */
    function ClearLastMoveData() {
        _LastMoveData = null;
        _LastSendMoveTick = 0;
    }
    Global.ClearLastMoveData = ClearLastMoveData;
     ...
     * @returns {}
     */
    function getMaxActiveSystemIndex() {
        let indexMax = 0;
        for (let i = 1; i <= 31; i++) {
            if (isSystemOpen(i)) {
                indexMax++;
    ...
    Global.getMaxActiveSystemIndex = getMaxActiveSystemIndex;
    /**
     * 判断某项系统是否激活，通过配置文件SystemOpen.Xml中配置，这儿采用服务器的数据进行判断
     * @param sytemIndex
     * @returns {}
     */
    function isSystemOpen(sytemIndex) {
        const valueSystem = Global.GetRoleCommonUseParamsValue(RoleCommonUseIntParamsIndexs.SystemOpenValue);
        return (valueSystem & (1 << sytemIndex)) > 0;
    }
    Global.isSystemOpen = isSystemOpen;
    /**
     * 根据输入数值获取位的设置值
     * @param whichOne
     * @returns {}
     */
    function getBitValue(whichOne) {
        return Math.pow(2, whichOne - 1);
    }
    Global.getBitValue = getBitValue;
    /**
     * 判断是否是装备
     * @param goodsId     物品Id
     */
    function goodsIsEquip(goodsId) {
        const categoriy = Global.GetCategoriyByGoodsID(goodsId);
        if (categoriy >= ItemCategories.TouKui && categoriy < ItemCategories.EquipMax && categoriy !== ItemCategories.JieZhi && categoriy !== ItemCategories.XiangLian) {
            return true;
        }
        return false;
    }
    Global.goodsIsEquip = goodsIsEquip;
})(Global || (Global = {}));
