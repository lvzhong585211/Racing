...
(function (MyUI) {
    ...
    class GongnengYugaoMgr {
        ...
        /**
         * 功能是否开启，返回值[IsOpened, trigger, param1, param2]
         * @param id 功能ID
         */
        static IsGongNengOpened(id) {
            const vo = tableMgr.systemOpenTable.Find(id);
            if (vo) {
                return [vo.IsOpened, vo.TriggerCondition, vo.TimeParameters, vo.TimeParameters2];
            }
            return [true, 0, 0, 0];
        }
        /**
         ...
         * @param id 功能ID
         */
        static HintGongNengOpened(id) {
            const aVal = GongnengYugaoMgr.IsGongNengOpened(id);
            const bOpen = aVal[0];
            const trigger = aVal[1];
            const param1 = aVal[2];
            const param2 = aVal[3];
            if (!bOpen)
                UIHelper.HintGongNengOpenCondition(id, trigger, param1, param2);
            return bOpen;
        }
        /**
         ...
         * @returns {}
         */
        static getNewIconOnCompleteTask(completedMainTaskId) {
            let newId = -1;
         ...
         */
        static validateOccupation(needOccupation) {
            if (needOccupation < 0 ||
         ...
         * @returns {}
         */
        static getNewIconOnLevelUp(oldLevel, newLevel) {
            let newId = -1;
         ...
         * @returns {}
         */
        static getNewIconOnShengWangLevelUp(shengWangLevel) {
            const ret = -1;
            const len = tableMgr.systemOpenTable.AllRows().length;
            for (let i = 0; i < len; i++) {
                const item = tableMgr.systemOpenTable.AllRows()[i];
         ...
         * @returns {}
         */
        static updateIcons(showState = false) {
            let newId = -1;
            ...
            for (let i = 0; i < len; i++) {
                ...
                if (!item.IsOpened) {
                    ...
                    // 判断功能开启的声望等级条件是否符合
                        ...
                        const shengwangLevel = Global.GetRoleCommonUseParamsValue(RoleCommonUseIntParamsIndexs.ShengWangLevel);
         ...
         * @param visible       显隐状态
         */
        static setCompVisibility(comp, visible) {
            if (null !== comp) {
         ...
         * @returns {}
         */
        static readyFlyingImgAnimation(systemOpenVoId) {
     ...
     */
    GongnengYugaoMgr.iconsDict = new Map();
    /**
     * 根据开启功能 Key:order ID  Value:对应按钮Button的显示状态 存储的Map列表
     */
    GongnengYugaoMgr.iconsOpenDictReal = new Map();
    // 记录当前时间，延迟时间调用
    GongnengYugaoMgr.startTimes = 0;
    MyUI.GongnengYugaoMgr = GongnengYugaoMgr;
})(MyUI || (MyUI = {}));
//# sourceMappingURL=GongnengYugaoMgr.js.map