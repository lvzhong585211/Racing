...
(function (MyUI) {
    ...
    (function (MapUI) {
         ...
         * 更新地图路径点显示
         */
        function updateMapLinePoints() {
            const win = windowMgr.getWindow(WindowID.Map);
            if (win && win.isPopup) {
                win.initializeLinePoints();
            }
        }
        MapUI.updateMapLinePoints = updateMapLinePoints;
        /** 传送项数据 */
         ...
         * 地图窗口
         */
        class MapWindow extends ui.MapUI.MapWindowUI {
            constructor() {
                super();
                this.mLastLeaderPos = new Laya.Point(); // 上次设置的主角位置
                this.mCurrLeaderPos = new Laya.Point(); // 主角当前所处的位置
                this.mVec3Rotation = new Laya.Vector3(); // 角色旋转四元数输出向量值
                this.mWorldPiecesList = [];
                this.mMapActorPointsList = [];
                this.mMapLinePointsList = [];
                this.mMapPointsCachePool = [];
                this.mTransferItemsDic = new Map();
                ...
                this._transferList.selectEnable = true;
                ...
                this._transferList.selectHandler = Laya.Handler.create(this, this.onTransferListSelected, null, false);
                ...
                this._imgLocal.on(Laya.Event.CLICK, this, (e) => {
            ...
            initializeWorldGroup() {
                ...
                // 初始化世界地图地图块
                this._groupWorld._childs.forEach(element => {
                    if (element instanceof MapUI.WorldMapPiece) {
            ...
            initializeTeleports(nLevelId) {
                ...
                for (let nIdx = 0; nIdx < aTeleports.length; nIdx++) {
                    ...
                    point.title = sTitle;
                    point.titleColor = `#${MyUI.ColorCode.blue1}`;
             ...
             * @param nLevelId 关卡ID
             */
            initializeMonsters(nLevelId) {
                return __awaiter(this, void 0, void 0, function* () {
                    const table = yield tableMgr.getLevelMonstersTable(nLevelId);
                    const aRows = table.AllRows();
                    const mapExisted = new Map(); // 已经设置过的怪物
                    ...
                    aRows.forEach(element => {
                        ...
                        point.title = sTitle;
                        point.titleColor = `#${MyUI.ColorCode.value}`;
             ...
             * @param nLevelId 关卡ID
             */
            initializeNpcs(nLevelId) {
                return __awaiter(this, void 0, void 0, function* () {
                    const table = yield tableMgr.getLevelNpcTable(nLevelId);
                    const aRows = table.AllRows();
            ...
            initializeTransfers(eActor) {
                ...
                const aSources = new Array();
                if (aTransDatas) {
                    aTransDatas.forEach(element => { aSources.push({ label: element.title, name: element.tag }); });
                }
            ...
            initializeLinePoints() {
                ...
                const aFinalNodes = []; // 存储最终的路径点
                aExpandNodes.forEach((value, index) => {
                ...
                aFinalNodes.forEach((element, index) => {
                    const point = this.createMapPoint();
                    point.name = `${index}`;
                    point.title = "";
            ...
            updateWorldPiecesUI() {
                let piecePlace = null;
                this.mWorldPiecesList.forEach(element => {
             ...
             */
            updateLeaderPlacePos(piece = null) {
                if (piece) {
        ...
        MapUI.MapWindow = MapWindow;
    })(MapUI = MyUI.MapUI || (MyUI.MapUI = {}));
})(MyUI || (MyUI = {}));
//# sourceMappingURL=MapWindow.js.map