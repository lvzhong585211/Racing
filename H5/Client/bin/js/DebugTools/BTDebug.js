...
(function (FBT) {
    ...
    class Debugger {
        ...
        static updateRender() {
            ...
            // 刷新数据
            {
                ...
                d3.selectAll("text").text(function (d) {
                    ...
                    if (d.data instanceof FBT.SelectorNode) {
                        rawName += " (Selector)";
                    }
                    else if (d.data instanceof FBT.SequenceNode) {
                        rawName += " (Sequence)";
                    }
                    else if (d.data instanceof FBT.ParallelNode) {
                        rawName += " (Parallel)";
                    }
                    const nodeRunList = Debugger.mNodeRunList;
                    const index = nodeRunList.indexOf(d.data);
        ...
        static Debug(rootNode) {
            ...
            const margin = {
                ...
                bottom: 100,
                left: 90,
            }, width = 1500 - margin.left - margin.right, height = 800 - margin.top - margin.bottom;
            // declares a tree layout and assigns the size
            const treemap = d3.tree()
                .size([width, height]);
            //  assigns the data to a hierarchy using parent-child relationships
            Debugger.mNodes = d3.hierarchy(rootNode, (data) => {
    ...
    Debugger.mCurrentDebugNode = null; // 保存当前正在调试的行为树根节点
    Debugger.mG = null; // svg
    Debugger.mNodes = null; // 用于渲染的nodes
    FBT.Debugger = Debugger;
})(FBT || (FBT = {}));
//# sourceMappingURL=BTDebug.js.map