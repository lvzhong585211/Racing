...
(function (Logic) {
    ...
    class StoryBoard {
        constructor() {
            this._PathIndex = 0; // 当前的路径点索引
            this._CurrentX = 0; // 当前坐标(2D坐标系)
            this._CurrentY = 0; // 当前坐标(2D坐标系)
            this._Path = null; // 保存使用的路径点列表
            this._CompletedState = false; // 是否已经结束了
            this._Started = false;
            this._Stopped = false;
            this._CellSizeX = 100;
            this._CellSizeY = 100;
            this._DestX = -1;
            this._DestY = -1;
            this._MovingSpeedPerSec = 500.0; // 移动速度(基础值)
            this.mSpeedScale = 1.0; // 移动速度的绽放系数,额外的参数主要是方便修改
            this.mRealMoveSpeed = 1; // 实时计算的真正的移动速度(厘米/秒)
        }
        /**
         * 设置移动速度
         ...
         */
        start(startPos, path, cellSizeX, cellSizeY, destX, destY) {
         ...
         */
        getCoordinate() {
            return new Logic.CellCoord(this._CurrentX, this._CurrentY);
        }
        /**
         * @desc    每帧被调用
