...
(function (Logic) {
    ...
    class Level {
        constructor(levelId) {
            this.mLevelId = -1; // 关卡ID
            this.mSceneId = -1; // 关卡使用的地图ID.注: 不同的关卡可能使用同一张地图,以共享图形显示
            this.mScene = null; // 在使用的场景
            this.mTerrain = null; // 在使用的地形.注:一般应该只有一个地形
            this.mCurrentMapData = null; // 当前关卡的逻辑模板数据,如障碍物信息
            // 角色的RoleID与角色实例映射
            // 注: 我们把角色全部放在关卡实例中,是假设任何角色都只能存在于某一个关卡中,方便管理与释放资源        
            this.mActorsMap = new Map();
            this.mToDelActors = new Array(); // 保存要删除的角色.注: 当调用delActor()函数时,我们并不会立即删除角色,因为这有可能会引起本帧正在引用的逻辑处理过于复杂,会等到本帧结束再删除
            this.mEntitiesCollection = new ECS.EntitiesCollection(); // 支持组件的实体的集合
            this.mLastTeleportKey = -1; // 上次设置的传送点ID（用来判断防止触发多次传送）
            // 可被攻击的对象集合
            this.mCanBeAttackedCollection = ECS.EntitiesSearchCache.from(this.mEntitiesCollection).componentsContainsAll([
                Logic.ActorState.Life // 生命组件
            ]);
            // 角色的RoleId与AI故事板间的映射
            this.mStoryBoardMap = new Map();
            this.mCurLevelTaskState = new Map(); // 保存当前地图npc的任务状态
            this.m_lstAutoOffEvent = []; // 事件监听列表
            this._debug_action = [0, 0];
            this._debug_keyClick = [0, 0];
            this.mLevelId = levelId;
            // 用户输入初始化
            this.mInput = new Logic.LevelInput();
        }
        /**
         * 获取关卡ID
         */
        ...
        load(progress) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                // 关卡逻辑数据
                this.mCurrentMapData = new Logic.GMapData();
                // 加载障碍物信息
                ...
                const sceneDataLoaded = yield mapDataPromise;
                const levelLoaded = yield levelPromise;
                ...
                this.mFollowTargetMgr.visualNameCollection = ECS.EntitiesSearchCache.from(this.mEntitiesCollection).componentsContainsAll([Logic.ActorState.VisualName]);
                // 查找地形对象
        ...
        initializeTeleports() {
            ...
            for (let nIdx = 0; nIdx < aTeleports.length; nIdx++) {
                ...
                const actor = new Logic.Teleport();
                actor.load(datTeleport);
                // 添加ECS组件
                const compVName = new Logic.ActorState.VisualName(Loca.getLang(datTeleport.Tip), 3, 0, "#73dcff");
         ...
         */
        createPlayer(type, playerState, life, magic, level) {
            // 创建玩家实例
            ...
            switch (playerState.Occupation) {
                case EnumOccupation.LongDan:
                    type === EActorType.NetPlayer && (player.components.add(new Logic.ActorState.Collider(new Laya.Vector3(0, 1.05, 0), new Laya.Vector3(1, 2, 1))));
                    player.components.add(new Logic.ActorState.VisualName(playerState.VSName, 2.5));
                    break;
         ...
         */
        findCharacter(roleId) {
            const retCharacter = this.mActorsMap.get(roleId);
            if (retCharacter) {
                Global.Log.Assert(retCharacter instanceof Logic.Character);
                return retCharacter;
            }
            return null;
        }
        ...
        findPlayer(actorID) {
            ...
            if (player != null) {
                Global.Log.Assert(player instanceof Logic.Player);
                return player;
            }
            return null;
        }
        ...
        addNpc(npcRole) {
            ...
            const compVName = new Logic.ActorState.VisualName(npcState.VSName, 2.5, 0, "#49bd1b");
            npcState.RoleSex = Global.Utils.getElementAttributeInt(npcElement, "Sex");
            ...
            */
            const actorNpc = new Logic.NPC(npcState);
        ...
        // 更新一下npc的任务状态
        UpdateNpcState(roleId, actorNpc) {
        ...
        addMonster(monsterData) {
            ...
            // 创建怪物实例
            monster = new Logic.Monster(monsterState);
            // 添加ECS组件
            monster.components.add(new Logic.ActorState.Life(monsterData.LifeV, monsterData.MaxLifeV));
            monster.components.add(new Logic.ActorState.Level(monsterData.Level));
            // 添加碰撞组件
            let collider = null;
            const voMonster = TableUtils.getMonsterXmlNodeByID(monsterData.ExtensionID);
            if (voMonster.IsCollide) {
                collider = new Logic.ActorState.Collider(voMonster.CollideCenter.clone(), voMonster.CollideSize.clone());
            }
            else {
                collider = new Logic.ActorState.Collider(new Laya.Vector3(0, 1.15, 0), new Laya.Vector3(1.5, 2, 1.5));
            }
            ...
            // 添加控制器
            const controller = new Logic.MonsterController();
            ...
            monster.on(Logic.EActorEvent.coordChanged, this, this.onMonsterCoordChanged);
         ...
         */
        onMonsterCoordChanged(monster, cx, cy) {
            // 添加到地图格子中
         ...
         * @param datRole 角色数据
         */
        addNetPlayer(datRole) {
            let tPlayer = this.findCharacter(datRole.RoleID);
            if (tPlayer) {
                Global.Log.Assert(false); // 已经存在了???
                return;
            }
            ...
            tPlayer = this.createPlayer(EActorType.NetPlayer, playerState, new Logic.ActorState.Life(datRole.LifeV, datRole.MaxLifeV), new Logic.ActorState.Magic(datRole.MagicV, datRole.MaxMagicV), new Logic.ActorState.Level(datRole.Level, datRole.ChangeLifeCount));
            tPlayer.load();
            // 添加控制器
            const controller = new Logic.APlayerController();
            controller.setOwner(tPlayer);
            // 设置行为处理器
            controller.setActionPlayType(Logic.PlayerActionPlay);
            tPlayer.on(Logic.EActorEvent.coordChanged, this, this.onNetPlayerCoordChanged);
         ...
         * @param cy 新的位置坐标(2D坐标系)
         */
        onNetPlayerCoordChanged(actor, cx, cy) {
         ...
         * @param cy 新的位置坐标(2D坐标系)
         */
        onLeaderCoordChanged(actor, cx, cy) {
            const bMoveRet = this.mCurrentMapData.mapGrid.moveObject(cx, cy, actor);
            if (!bMoveRet) {
                return;
            }
                    ...
                    if (this.isLocalMapTrans(nTeleportKey)) {
                        ...
                        if (!Global.Data.WaitingForMapChange) { // 如果正在传送中，则不能再次触发
                            if (Global.CanBeTransport(gameIns.gameState.roleData.MapCode, nTeleportKey)) {
                                actor.getController().SetState(Logic.EControllerStateId.Idling);
         ...
         */
        judgeSafeRegion(actor, nPosX, nPosY) {
         ...
         */
        isLocalMapTrans(nTeleportKey) {
            const nActorID = SpriteBaseIds.calcTeleportActorID(nTeleportKey);
            const teleport = this.mActorsMap.get(nActorID);
         ...
         * @param nTeleportKey 传送点ID
         */
        canMapConversionByTeleportCode(nTeleportKey) {
            const nActorID = SpriteBaseIds.calcTeleportActorID(nTeleportKey);
            const teleport = this.mActorsMap.get(nActorID);
            if (!teleport) {
                return;
         ...
         * @param nTeleportKey 传送点ID
         */
        toMapConversionByTeleportCode(nTeleportKey) {
            const nActorId = SpriteBaseIds.calcTeleportActorID(nTeleportKey);
            const teleport = this.mActorsMap.get(nActorId);
            if (!teleport) {
                return;
         ...
         */
        toMapConversionByMapCode(nMapCode, nMapX, nMapY, nDir, nRelife) {
            if (nRelife > 0) {
         ...
         * @param state 指定任务的状态
         */
        serverUpdateNPCTaskState(npcRoleId, state) {
        ...
        SaveNpcTaskState(npcRoleId, state) {
            if (state !== 0) {
        ...
        /**
         * 由传入的角色数据来格式化角色的名称显示
         * @param roleData 指定要格式化显示名称的角色的信息
         */
        formatShowName(roleData) {
            const mapSceneUIClass = TableUtils.getMapSceneUIClass(this.mLevelId);
            if (Global.isKuaFuHuoDongMapSceneUIClass(mapSceneUIClass) ||
                mapSceneUIClass === SceneUIClasses.KuaFuMap ||
                mapSceneUIClass === SceneUIClasses.LangHunLingYu ||
                mapSceneUIClass === SceneUIClasses.HuanShuYuan) {
                return Global.formatRoleNameZoneid(roleData.ZoneID, roleData.RoleName);
            }
            else if (Global.isBattleMap(this.mLevelId)) {
                if (roleData.BattleWhichSide === BattleWhichSides.Sui) {
                    return ConfigLoca.UI_ROLENAME_FENGSHEN;
                }
                else if (roleData.BattleWhichSide === BattleWhichSides.Tang) {
                    return ConfigLoca.UI_ROLENAME_SHASHEN;
                }
            }
         ...
         */
        foreachActorsMap(caller, callBackFn) {
         ...
         */
        stopOnNextGrid(roleId, stopIndex = -1) {
         ...
         */
        _SeekSpriteToLock(isNPC, radius, angleLimit = 0.0) {
            ...
            for (const [nRoleId, actor] of this.mActorsMap) {
                if (actor.getType() === EActorType.LocalPlayer || !(actor instanceof Logic.Character)) {
                    continue;
                }
                ...
                if (isNPC) {
                    if (eType === EActorType.NPC && !actor.isStatueStand()) {
         ...
         */
        seekMonsterToLock(attacker, isAutoFighting = true, targetMonsterID = -1, seedRadius = -1) {
            if (attacker == null) {
                return;
            }
            ...
            for (const actor of this.mCanBeAttackedCollection.entities) {
                if (actor instanceof Logic.Character) { // 必须是Character派生
                    if (actor === attacker) {
                        continue;
                    }
         ...
         * 注: 是给本地玩家使用的寻路函数
         */
        linearMoveByRunTo(character, targetPos, offset, action, extAction) {
         ...
         * @param extAction 发送给服务器的寻路完成后的行为
         */
        linearMove(sprite, to, action, extAction) {
        ...
        ServerLinearMove(moveData) {
            ...
            if (!(actor instanceof Logic.Character)) {
                return;
            }
            if (actor.isDead()) {
                return; // 死亡了,直接返回.
            ...
            if (controller.GetState() !== Logic.EControllerStateId.AutoPath) {
                // 切换到自动寻路状态
                controller.SetState(Logic.EControllerStateId.AutoPath, { action: moveData.action, moveFunction: moveFunction });
            }
            else {
                // 已经是自动寻路状态了,直接移动吧
                moveFunction();
         ...
         */
        astarMove(sprite, targetPos, action, pathString = null, currentPathIndex = -1) {
            ...
            const moveComponent = { Coordinate: coord, SpriteType: sprite.getType() };
            ...
            if (!storyBoard) {
                storyBoard = new Logic.StoryBoard();
         ...
         */
        static transStringToPathArr(pathStr) {
            ...
            for (let n = 0; n < points.length; n++) {
                ...
                const node = new Logic.ANode(0, 0);
         ...
         */
        debugAllNonCharacterIds() {
            const retArray = new Array();
            this.mActorsMap.forEach(actor => {
                if (!(actor instanceof Logic.Character)) {
                    if (Logic.isDebugInnerID(actor)) {
                        retArray.push(actor);
                    }
                }
            });
         ...
         * @param nActorID 角色ID
         */
        _getActorName(nActorID) {
            const tActor = this.mActorsMap.get(nActorID);
            if (tActor) {
                Global.Log.Assert(tActor instanceof Logic.Character);
                const datActor = tActor.getState();
         ...
         */
        updateActorLifeAndMagic(nActorID, nLifeV, nMaxLifeV, nMagicV, nMaxMagicV) {
            ...
            if (nLifeV !== undefined) {
                Logic.setLifeV(tActorImp, nLifeV, nMaxLifeV);
            }
            if (nMagicV !== undefined) {
                Logic.setMagicV(tActorImp, nMagicV, nMaxMagicV);
            }
            // 通知血量更新
            const tLifeImp = {
                roleID: nActorID,
                curLifeV: Logic.getLifeV(tActorImp),
                curMagicV: nMagicV !== undefined ? Logic.getMagicV(tActorImp) : 0,
                maxLifeV: Logic.getMaxLifeV(tActorImp),
                maxMagicV: nMagicV !== undefined ? Logic.getMaxMagicV(tActorImp) : 0
            };
            gameEventBus.lifeChange.event(tLifeImp);
            // 角色死亡
            if (tLifeImp.curLifeV <= 0) {
                tActorImp.getController().SetState(Logic.EControllerStateId.Dead);
            }
        }
        /**
        ...
        serverInjured(datInjured) {
            ...
            if (datInjured.injuredRoleID === nLeaderID) { // 主角自己受伤
                ...
                if (tInjurer) {
                    const tActionPlayer = tInjurer.getController().getActionPlayer();
         ...
         * @param nDir 方向
         */
        serverRealive(nActorID, nPosX, nPosY, nDir) {
            const tActorImp = this.findCharacter(nActorID);
            if (!tActorImp)
                return;
            // 复活之后设置血值为最大
            ...
            // 设置复活之后的位置和方向
            tActorImp.getController().SetState(Logic.EControllerStateId.Idling);
         ...
         */
        PlayerChangeEquip(dtGoods, actorID) {
         ...
         */
        ServerChangeCode(changeEquipData) {
            let goodsData = null;
                ...
                if (changeEquipData.EquipGoodsData != null) {
                    ...
                    if (goodsData == null) {
                        if (otherRoleData.GoodsDataList == null) {
                            otherRoleData.GoodsDataList = new Array();
                        }
         ...
         */
        serverChangePos(roleID, x, y, direction, animation) {
