...
(function (Logic) {
    ...
    class ParticleCacheInfo {
        constructor() {
            this.mLoadingPromise = null; // 加载用的Promise
            this.mLoaded = 0; // 标识加载的状态: 0 - 加载中, 1 - 加载成功, 2 - 加载失败
            this.mIdlingTime = 0; // 空闲时间
            this.mCachedParticles = []; // 空闲的粒子列表
        }
    }
    /**
     * 实现对粒子播放的缓冲与管理
    ...
    class ParticlesManager {
        constructor() {
            this.mParticleMap = new Map(); // 保存粒子名称到可用的粒子实例的映射
         ...
         */
        preCacheParticles(particleName) {
         ...
         */
        queryParticle(particleName, allowAsyncLoading = false, autoReturnPool = true) {
            ...
            if (cachedInfo.mLoaded === 0) {
                ...
                cachedInfo.mLoadingPromise.then((particleRoot) => {
                    ...
                    // Clone特效对象
                    const child = particleRoot.clone();
                    ...
                    if (SystemConfig.debugShowActor3DAxis) {
                        const box = retParticle.addChild(new Laya.MeshSprite3D(new Laya.BoxMesh(0.1, 0.1, 0.1)));
                        box.addComponent(Logic.DebugShowAxisComponent);
                    }
                    if (autoReturnPool) {
                        retParticle.addComponent(Logic.ParticleAutoReturnPool); // 添加播放完成自动还回特效池的脚本
                    }
                });
            }
         ...
         */
        static playParticleSystem(particleRoot) {
         ...
         */
        static stopParticleSystem(particleRoot) {
            if (particleRoot instanceof Laya.ShuriKenParticle3D) {
         ...
         */
        static isParticleSystemAlive(particleRoot) {
            if (particleRoot instanceof Laya.ShuriKenParticle3D) {
        ...
        returnParticle(particle) {
            ...
            if (cachedInfo.mCachedParticles.length > 10) {
                ...
                particle.removeSelf();
                particle.removeComponentByType(Logic.ParticleAutoReturnPool);
            }
    ...
    ParticlesManager.instance = null; // 全局实例
    Logic.ParticlesManager = ParticlesManager;
})(Logic || (Logic = {}));
