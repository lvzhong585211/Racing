...
(function (GameMode) {
    ...
    class PlayingMode_NetMsg {
        constructor() {
            this.mPlayingMode = null; // PlayingMode引用
            this.mMsgHandler = {}; // 定义消息处理的映射.注:理论上这样做比switch速度快,且方便维护
        }
        destroy() {
         ...
         */
        processNetMsgHandler(uMsgtype, msgReader) {
            const msgHandler = this.mMsgHandler[uMsgtype];
            if (msgHandler) {
         ...
         */
        registerNetMsgHandler(value) {
            this.mPlayingMode = value;
            const msgHandler = this.mMsgHandler;
            // --------------------
            // 处理NPC任务状态
            // --------------------
            msgHandler[EMessageType.CMD_SPR_NPCSTATELIST] = (msgReader) => {
                const npcTaskStateList = msgReader.array(NetMsg.NPCTaskState.decode);
            ...
            // 更新NPC身上的任务状态
            // --------------------
            msgHandler[EMessageType.CMD_SPR_UPDATENPCSTATE] = (msgReader) => {
            ...
            // 添加一个NPC
            // --------------------
            msgHandler[EMessageType.CMD_SPR_NEWNPC] = (msgReader) => {
            ...
            // 删除一个NPC
            // --------------------
            msgHandler[EMessageType.CMD_SPR_DELNPC] = (msgReader) => {
                const fields = gameIns.getStringMsgFields(msgReader);
            ...
            // 回血回蓝消息
            // --------------------
            msgHandler[EMessageType.CMD_SPR_RELIFE] = (msgReader) => {
            ...
            // 添加一个Monster
            // --------------------
            msgHandler[EMessageType.CMD_SYSTEM_MONSTER] = (msgReader) => {
            ...
            // 删除指定的Monster或角色
            // --------------------
            msgHandler[EMessageType.CMD_SPR_LEAVE] = (msgReader) => {
            ...
            // 处理与NPC对话的消息
            // --------------------
            msgHandler[EMessageType.CMD_SPR_CLICKON] = (msgReader) => {
            ...
            // 开始游戏
            // --------------------
            msgHandler[EMessageType.CMD_PLAY_GAME] = (msgReader) => {
            ...
            // 金钱改变
            // --------------------
            msgHandler[EMessageType.CMD_SPR_MONEYCHANGE] = (msgReader) => {
                const fields = gameIns.getStringMsgFields(msgReader);
                const roleData = gameIns.gameState.roleData;
            ...
            // 经验改变消息
            // --------------------
            msgHandler[EMessageType.CMD_SPR_EXPCHANGE] = (msgReader) => {
                const aFields = gameIns.getStringMsgFields(msgReader);
                ...
                if (aFields.length >= 5)
                    nChangeLife = parseInt(aFields[4]);
            ...
            /** 更新角色数据 */
            msgHandler[EMessageType.CMD_SPR_UPDATE_ROLEDATA] = (msgReader) => {
            ...
            // 背包整理
            // --------------------
            msgHandler[EMessageType.CMD_SPR_RESETBAG] = (msgReader) => {
            ...
            // 换装消息
            // --------------------
            msgHandler[EMessageType.CMD_SPR_CHGCODE] = (msgReader) => {
            ...
            // 开始寻路消息
            // --------------------
            msgHandler[EMessageType.CMD_SPR_MOVE] = (msgReader) => {
            ...
            // 技能列表消息
            // --------------------
            msgHandler[EMessageType.CMD_SPR_GETSKILLINFO] = (msgReader) => {
            ...
            // 玩家请求VIP信息
            // --------------------
            msgHandler[EMessageType.CMD_SPR_GETVIPINFO] = (msgReader) => {
                const fields = gameIns.getStringMsgFields(msgReader);
            ...
            // 玩家领取VIP等级奖励
            // --------------------
            msgHandler[EMessageType.CMD_SPR_GETVIPLEVELAWARD] = (msgReader) => {
                const fields = gameIns.getStringMsgFields(msgReader);
                ...
                switch (result) {
                    case -1:
                        uiMgr.hintText(ConfigLoca.UI_VIP_贵族等级不够无法领取);
                        break;
                    case -2:
                        uiMgr.hintText(ConfigLoca.UI_VIP_RewardAlwaysGet);
                        break;
                    case -3:
                        uiMgr.hintText(ConfigLoca.UI_VIP_GetLose);
                        break;
                    case -4:
                        uiMgr.hintText(ConfigLoca.UI_COMMON_背包已满请清理背包后再领取);
                        break;
                }
            };
            // --------------------
            // 玩家VIP等级升级
            // --------------------
            msgHandler[EMessageType.CMD_SPR_VIPLEVELUP] = (msgReader) => {
                const fields = gameIns.getStringMsgFields(msgReader);
            ...
            // 攻击同步
            // --------------------
            msgHandler[EMessageType.CMD_SPR_ATTACK] = (msgReader) => {
            ...
            // 伤害消息
            // --------------------
            msgHandler[EMessageType.CMD_SPR_INJURE] = (msgReader) => {
            ...
            // 技能升级消息
            // --------------------
            msgHandler[EMessageType.CMD_SPR_UPSKILLLEVEL] = (msgReader) => {
                const dtMsg = NetMsg.SCSkillLevelUp.decode(msgReader);
                if (!dtMsg)
                    return;
                if (dtMsg.RoleID !== Global.Data.roleData.RoleID)
                    return;
                switch (dtMsg.State) {
                    case -1:
                        uiMgr.hintText(Loca.getLang("升级失败，还没有学习该技能！"));
                        break;
                    case -2:
                        uiMgr.hintText(Loca.getLang("升级失败，静态数据读取错误！"));
                        break;
                    case -3:
                        uiMgr.hintText(Loca.getLang("升级失败，已经是最高级了！"));
                        break;
                    case -4:
                        uiMgr.hintText(Loca.getLang("升级失败，重生等级没有达到需求等级！"));
                        break;
                    case -5:
                        uiMgr.hintText(Loca.getLang("升级失败，角色等级没有达到需求等级！"));
                        break;
                    case -6:
                        uiMgr.hintText(Loca.getLang("升级失败，该技能训练度不够！"));
                        break;
                    case -7:
                    ...
                    case -8:
                        ...
                        break;
                    case -9:
                        uiMgr.hintText(Loca.getLang("升级失败，熟练度不够无法升级！"));
                        break;
                }
                if (dtMsg.State < 0)
                    return;
            ...
            // 角色参数变化消息
            // --------------------
            msgHandler[EMessageType.CMD_SPR_ROLEPARAMSCHANGE] = (msgReader) => {
                const fields = gameIns.getStringMsgFields(msgReader);
                const roleId = parseInt(fields[0]);
            ...
            // 修改快捷使用配置
            // --------------------
            msgHandler[EMessageType.CMD_SPR_MODKEYS] = (msgReader) => {
                const fields = gameIns.getStringMsgFields(msgReader);
                if (fields[1] === "0") {
            ...
            // 受击消息
            // --------------------
            msgHandler[EMessageType.CMD_SPR_HITED] = (msgReader) => {
            ...
            // 同步释放招式消息
            // --------------------
            msgHandler[EMessageType.CMD_SPR_MAGIC_ATTACK] = (msgReader) => {
            ...
            // 角色加载完毕的消息
            // --------------------
            msgHandler[EMessageType.CMD_SPR_LOADALREADY] = (msgReader) => {
             ...
             * 角色移动数据封装
             */
            msgHandler[EMessageType.CMD_SPR_CHANGEPOS] = (msgReader) => {
                const fields = gameIns.getStringMsgFields(msgReader);
            ...
            // 其他玩家的消息
            // --------------------
            msgHandler[EMessageType.CMD_OTHER_ROLE] = (msgReader) => {
                const datMiniRole = NetMsg.RoleDataMini.decode(msgReader);
                let datRole = null;
                if (datMiniRole)
                    datRole = Global.ClientDataToRoleDataMini(datMiniRole);
                else
                    datRole = NetMsg.RoleData.decode(msgReader);
                if (!datRole) {
                    return;
                }
            ...
            // --------------------
            msgHandler[EMessageType.CMD_SPR_SEVEN_DAY_ACT_QUERY] = (msgReader) => {
            ...
            // --------------------
            msgHandler[EMessageType.CMD_SPR_SEVEN_DAY_ACT_GET_AWARD] = (msgReader) => {
                const aFields = gameIns.getStringMsgFields(msgReader);
                ...
                if (code > 0) {
                    switch (code) {
                        case 1:
                            uiMgr.hintText(ConfigLoca.UI_SEVENDAYLOGIN_NoActTimes);
                            break; // 不在活动时间
                        case 2:
                            uiMgr.hintText(ConfigLoca.UI_SEVENDAYLOGIN_ServerConfigError);
                            break; // 服务器配置出错
                        case 3:
                            uiMgr.hintText(ConfigLoca.UI_SEVENDAYLOGIN_BagNoHas);
                            break; // 背包不足
                        case 4:
                            uiMgr.hintText(ConfigLoca.UI_SEVENDAYLOGIN_DataBaseError);
                            break; // 数据库异常
                        case 5:
                            uiMgr.hintText(ConfigLoca.UI_SEVENDAYLOGIN_不满足领奖条件);
                            break; // 不满足领奖条件
                        case 6:
                            uiMgr.hintText(ConfigLoca.UI_SEVENDAYLOGIN_客户端访问参数错误);
                            break; // 客户端访问参数错误
                        case 7:
                            uiMgr.hintText(ConfigLoca.UI_SEVENDAYLOGIN_DiamondNoHas);
                            break; // 钻石不足		
                        case 8:
                            uiMgr.hintText(ConfigLoca.UI_SEVENDAYLOGIN_可抢购数量不足);
                            break; // 可抢购数量不足
                        default:
                            uiMgr.hintText(ConfigLoca.UI_SEVENDAYLOGIN_GetRewardError.replace("{0}", code.toString()));
                            break;
                    }
                }
                else {
                    ...
                    switch (actType) {
                        case FuLiActivityEnum.QiRiKuangHuanLogin:
                            gameEventBus.sevenDayLoginRewardGet.event(position);
                            break; // 7日登陆奖励领取
                    }
                }
            };
            ...
            // 同步怪物掉落拥有者信息
            // --------------------
            msgHandler[EMessageType.CMD_SPR_UPDATE_MONSTER_BELONGTO_INFO] = (msgReader) => {
                const aFields = gameIns.getStringMsgFields(msgReader);
                const nRoleID = parseInt(aFields[0]);
             ...
             * 功能开启提示
             */
            msgHandler[EMessageType.CMD_SPR_ClientFunOpenTiShi] = (msgReader) => {
                const aFields = gameIns.getStringMsgFields(msgReader);
             ...
             * 功能开启功能领取
             */
            msgHandler[EMessageType.CMD_SPR_ClientFunOpenTiShiRewardPickUp] = (msgReader) => {
                const aFields = gameIns.getStringMsgFields(msgReader);
            ...
            // 停止移动
            // --------------------
            msgHandler[EMessageType.CMD_SPR_STOPMOVE] = (msgReader) => {
                const aFields = gameIns.getStringMsgFields(msgReader);
                const nRoleID = parseInt(aFields[0]);
            ...
            // 复活消息（人/怪物）
            // --------------------
            msgHandler[EMessageType.CMD_SPR_REALIVE] = (msgReader) => {
            ...
            // 自动分配加点
            // --------------------
            msgHandler[EMessageType.CMD_SPR_SETAUTOASSIGNPROPERTYPOINT] = (msgReader) => {
                const aFields = gameIns.getStringMsgFields(msgReader);
                const nRoleID = parseInt(aFields[0]);
            ...
            // 推荐属性加点
            // --------------------
            msgHandler[EMessageType.CMD_SPR_EXECUTERECOMMENDPROPADDPOINT] = (msgReader) => {
                const aFields = gameIns.getStringMsgFields(msgReader);
            ...
            // 全套加成属性值更新
            // --------------------
            msgHandler[EMessageType.CMD_UPDATEALLTHINGINDEXS] = (msgReader) => {
                const aFields = gameIns.getStringMsgFields(msgReader);
                const nRoleID = parseInt(aFields[0]);
                if (nRoleID === gameIns.gameState.RoleID) {
            ...
            // Buff数据
            // --------------------
            msgHandler[EMessageType.CMD_SPR_BUFFERDATA] = (msgReader) => {
                ...
                if (nFindIdx < 0)
                    aBuffDatas.push(datBuff);
                else
                    aBuffDatas[nFindIdx] = datBuff;
                // TODO: updateUI
            };
            // --------------------
            // 刷新图标状态信息
            // --------------------
            msgHandler[EMessageType.CMD_SPR_REFRESH_ICON_STATE] = (msgReader) => {
            ...
            /** 通知切换地图 */
            msgHandler[EMessageType.CMD_SPR_NOTIFYCHGMAP] = (msgReader) => {
                const aFields = gameIns.getStringMsgFields(msgReader);
                const nRoleId = parseInt(aFields[0]);
            ...
            /** 获取活动数据 */
            msgHandler[EMessageType.CMD_SPR_GETHUODONGDATA] = (msgReader) => {
            ...
            /** 在线奖励信息 */
            msgHandler[EMessageType.CMD_SPR_UPDATEEVERYDAYONLINEAWARDGIFTINFO] = (msgReader) => {
                const aFields = gameIns.getStringMsgFields(msgReader);
                const nRoleId = parseInt(aFields[0]);
            ...
            /** 领取每日在线奖励 */
            msgHandler[EMessageType.CMD_SPR_GETEVERYDAYONLINEAWARDGIFT] = (msgReader) => {
                const aFields = gameIns.getStringMsgFields(msgReader);
                const nRoleId = parseInt(aFields[0]);
                ...
                switch (nRet) {
                    case -1:
                        ...
                        break;
                    case -2:
                        uiMgr.hintText(ConfigLoca.UI_Error_ConfigError);
                        break;
                    case -3:
            ...
            /** 等级奖励信息 */
            msgHandler[EMessageType.CMD_SPR_QUERYUPLEVELGIFTINFO] = (msgReader) => {
            ...
            /** 领取等级奖励 */
            msgHandler[EMessageType.CMD_SPR_GETUPLEVELGIFTAWARD] = (msgReader) => {
                const aFields = gameIns.getStringMsgFields(msgReader);
                ...
                if (nRet === 1) {
                    ...
                    switch (nRet) {
                        case -20:
                            ...
                            break;
                        case -101:
                            uiMgr.hintText(ConfigLoca.UI_Grade_Get_Failure_LevelNotEnough);
                            break;
                        case -103:
                            uiMgr.hintText(ConfigLoca.UI_Grade_Get_Failure_GradeHadGet);
                            break;
                        default: break;
                    }
                }
            ...
            /** 查询商城数据 */
            msgHandler[EMessageType.CMD_SPR_FETCHMALLDATA] = (msgReader) => {
            ...
            /** 抢购商城购买物品 */
            msgHandler[EMessageType.CMD_SPR_MALLQIANGGOUBUYGOODS] = (msgReader) => {
                const aFields = gameIns.getStringMsgFields(msgReader);
                ...
                if (result < 0) {
                    switch (result) {
                        case -1:
                            uiMgr.hintText(ConfigLoca.UI_COMMON_BuyGoodsAlwaysXiaJia);
                            break;
                        case -2:
                            uiMgr.hintText("钻石余额不足，无法购买物品\nTODO:产出链接");
                            break;
                        case -3:
                            uiMgr.hintText(ConfigLoca.UI_COMMON_BagFull_请清理出空格后再购买);
                            break;
                        case -20003:
                            uiMgr.hintText(ConfigLoca.UI_COMMON_GoodsAlwaysSoldOver);
                            break;
                        case -20004:
                            uiMgr.hintText(ConfigLoca.UI_COMMON_BuyNumsMoreThan);
                            break;
                        case -20005:
                            uiMgr.hintText(ConfigLoca.UI_COMMON_BuyNumsMoreThan1);
                            break;
                        case -20006:
                            uiMgr.hintText(ConfigLoca.UI_COMMON_BuyNumsMoreThan2);
                            break;
                        default:
            ...
            /** 钻石货币数量改变 */
            msgHandler[EMessageType.CMD_SPR_USERMONEYCHANGE] = (msgReader) => {
                const aFields = gameIns.getStringMsgFields(msgReader);
            ...
            /** 金币数量改变 */
            msgHandler[EMessageType.CMD_SPR_USERYINLIANGCHANGE] = (msgReader) => {
                const aFields = gameIns.getStringMsgFields(msgReader);
                const roleId = parseInt(aFields[0]);
            ...
            /** 绑钻数量改变 */
            msgHandler[EMessageType.CMD_SPR_USERGOLDCHANGE] = (msgReader) => {
                const aFields = gameIns.getStringMsgFields(msgReader);
                const roleId = parseInt(aFields[0]);
            ...
            /** 钻石、银币商城商品购买 */
            msgHandler[EMessageType.CMD_SPR_MALL_BUY] = (msgReader) => {
                const aFields = gameIns.getStringMsgFields(msgReader);
                const result = parseInt(aFields[0]);
                if (result < 0) {
                    switch (result) {
                        case -1:
                            uiMgr.hintText(ConfigLoca.UI_COMMON_BuyGoodsXiaJia);
                            break;
            ...
            /** 开始潜心修炼 */
            msgHandler[EMessageType.CMD_SPR_STARTMEDITATE] = (msgReader) => {
                const aFields = gameIns.getStringMsgFields(msgReader);
                const nRoleId = parseInt(aFields[0]);
            ...
            /** 获取潜心修炼时间信息 */
            msgHandler[EMessageType.CMD_SPR_GETMEDITATETIMEINFO] = (msgReader) => {
                const aFields = gameIns.getStringMsgFields(msgReader);
                const nRoleId = parseInt(aFields[0]);
            ...
            /** 获取潜心修炼经验 */
            msgHandler[EMessageType.CMD_SPR_GETMEDITATEEXP] = (msgReader) => {
                const aFields = gameIns.getStringMsgFields(msgReader);
                const nRet = parseInt(aFields[0]); // 领取结果
                ...
                if (nRet === 1) {
                    ...
                    switch (nRet) {
                        ...
                        case -2:
                            ...
                            break;
                        case -10:
                            uiMgr.hintText(ConfigLoca.UI_Get_Failure_VipLevelNotEnough);
                            break;
                        case -11:
                            uiMgr.hintText(ConfigLoca.UI_Get_Failure_VipLevelNotEnough);
                            break;
         ...
         */
        processNpcClickonResult(npcData) {
            ...
            if (showNpcDialog) {
                ...
                Global.Log.Error("NPCDialogPart not done!!!");
                // to do ...
            }
            else {
                let taskClass = 0;
         ...
         * @param newTask
         */
        showNPCDoingTaskWindow(npcRoleId, taskID, npcExenstionID, caching = false, newTask = false) {
            // 暂时直接返回交取任务,以便进行测试
        ...
        handleAutoFight(msgReader) {
            ...
            if (autoFight.FightType === AutoFightCmds.Start || autoFight.FightType === AutoFightCmds.PlayerClickStart) {
                ...
                if (ret < 0) {
                    ...
                    if (-2 === ret) {
                        // 地图中禁止挂机
                        uiMgr.hintText(ConfigLoca.AUTO_FIGHT_NOT_ALLOWED, TableUtils.getMapNameByCodeEx(gameState.roleData.MapCode));
                    }
                    else {
                        // 开始挂机
                        GameMode.getLocalPlayerController().serverStartAutoFight();
                    }
                }
                else {
                    if (autoFight.FightType === AutoFightCmds.PlayerClickStart) {
                ...
                GameMode.getLocalPlayerController().serverEndAutoFight();
            }
        }
        /**
         * 技能信息同步回调
        ...
        handleCompleteTask(msgReader) {
            ...
            if (taskVO.TaskClass === TaskClasses.Main) { // 更新已经完成的主线任务Id
         ...
         * @param extCallBack
         */
        triggerTaskPlot(mode, timeParameters, extCallBack) {
            // to do ... 继续实现
            return false;
        }
        /**
        ...
        handleGetAttribute(msgReader) {
            ...
            const roleID = equipPropsData.RoleID;
            if (roleID === Global.Data.roleData.RoleID) {
                const oldPoints = new Array(UnitPropIndexes.Max);
                ...
                if (equipPropsData.CombatForce !== Global.Data.roleData.CombatForce) {
                    Global.Data.roleData.CombatForce = equipPropsData.CombatForce;
                    gameEventBus.combatForceChange.event();
                }
                if (equipPropsData.ChangeLifeCount !== Global.Data.roleData.ChangeLifeCount) {
                    Global.Data.roleData.ChangeLifeCount = equipPropsData.ChangeLifeCount;
        ...
        */
        ...
        handleModGoods(msgReader) {
            ...
            switch (ModGoods.State) {
                ...
                case -1000:
                    ...
                    return;
                case -4:
                    uiMgr.hintText(Loca.getLang("当前骑宠栏位已被占满"));
                    return;
                case -5:
                    uiMgr.hintText(Loca.getLang("当前萌宠栏位已被占满"));
                    return;
                case -1:
                    uiMgr.hintText(Loca.getLang("服务器配置错误或数据库操作失败"));
                    return;
            }
            if (ModGoods.State < 0)
                return;
         ...
         */
        _updateActorLife(nActorID, nLifeV, nMagicV, nMaxLifeV, nMaxMagicV) {
        ...
        handleMapChange(msgReader) {
            ...
            if (datMsg.State >= 0) {
                ...
                switch (datMsg.State) {
                    case -1001:
                        uiMgr.hintText(Loca.getLang("4479"));
                        break; // 角色重生次数不够，无法切换地图
                    case -1002:
                        uiMgr.hintText(Loca.getLang("4480"));
                        break; // 角色级别不够，无法切换地图
                    case -1003: break;
         ...
         * @param fristState
         */
        processFirstGoods(dId, goodsId, fristState) {
         ...
         * @returns {}
         */
        processImgAnimation(systemOpenId) {
            Super.GData.waitingForSystemHelp = GongnengYugaoMgr.readyFlyingImgAnimation(systemOpenId);
        }
    }
    GameMode.PlayingMode_NetMsg = PlayingMode_NetMsg;
})(GameMode || (GameMode = {}));
//# sourceMappingURL=PlayingMode_NetMsg.js.map