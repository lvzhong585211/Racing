    ...
    LayaAirFileTools.prototype.getFileList = function (url, extension) {
        ...
        function nextfind(url) {
            ...
            fileList.forEach(function (filepath) {
                filepath = path.join(url, filepath);
...
var htmlHandlerScript = function () {
    ...
    for (i = 0; i < fileNodes.length; i++) {
        var _currentNode = fileNodes[i];
        _currentNode.url = _currentNode.url.replace(workspace + "/src/", "js/").replace(".ts", ".js");
    }
    for (i = 0; i < fileNodes.length; i++) {
        ...
        for (var __j = i; __j < fileNodes.length; __j++) {
            var checkNode = fileNodes[__j];
    ...
    if (oldscript && oldscript.length > 1) {
        ...
        var addscript = "";
        for (var _j = 0; _j < sriptList.length; _j++) {
            var fileItem = sriptList[_j];
