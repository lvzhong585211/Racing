...
namespace Global {
	/** 默认的物品结束时间 */
	export const ConstGoodsEndTime = "1900-01-01 12:00:00";
	/** 背包总页数 */
	export const BAG_PAGES: number = 5;
	/** 背包每页行数 */
	export const BAG_PAGE_ROWS: number = 5;
	/** 背包每页列数 */
	export const BAG_PAGE_COLS: number = 5;
	/** 背包最大的格子数量 */
	export const MaxBagGridNum: number = BAG_PAGES * BAG_PAGE_ROWS * BAG_PAGE_COLS; // 125;

	/** 获取背包格子的最大个数（已解锁可用的格子数） */
	export function GetTotalMaxBagGridCount(): number {
	...
	let mapBagIndex: Map<number, number>;
	/** 格子索引到Data索引的映射（key:GridIndex => value:BagIndex） */
	let mapGridIndex: Map<number, number>;
    /**
     ...
     */
	function initListIndexMapping(nPages: number, nPageRows: number, nPageCols: number) {
		mapBagIndex = new Map<number, number>();
		mapGridIndex = new Map<number, number>();
     ...
     * @param nBagIdx 数据索引
     */
	export function getGridIndexByBagIndex(nBagIdx: number) {
		if (!mapBagIndex) {
			// 注意：暂时三个参数都使用背包的，后期有更大的列表时可以考虑扩大
			initListIndexMapping(BAG_PAGES, BAG_PAGE_ROWS, BAG_PAGE_COLS);
		}
		if (mapBagIndex.has(nBagIdx)) {
			return mapBagIndex.get(nBagIdx);
		}
		Log.Assert(false, "BagIdx beyond range!!!");
		return -1;
	}
     ...
     * @param nGridIdx 格子索引
     */
	export function getBagIndexByGridIndex(nGridIdx: number) {
		if (!mapGridIndex) {
			// 注意：暂时三个参数都使用背包的，后期有更大的列表时可以考虑扩大
			initListIndexMapping(BAG_PAGES, BAG_PAGE_ROWS, BAG_PAGE_COLS);
		}
		...
		Log.Assert(false, "GridIdx beyond range!!!");
		return -1;
	}
	 ...
	 * @param gd 道具数据
	 */
	export function DestroyGoods(gd: NetMsg.IGoodsData): void {
		// TODO: 发送消息
     ...
     * @param goodsId 物品ID
     */
	export function GetGoodsGridNumByIDOfMall(goodsId: number): number {
		if (Data.MallData) {
			if (!String.IsNullOrWhiteSpace(Data.MallData.MallXmlString)) {
	 ...
	 * @param useOldGrid 
	 */
	export function CanAddGoods(goodsID: number, newGoodsNum: number, binding: number, endTime: string = ConstGoodsEndTime, useOldGrid: boolean = false): boolean {
	 ...
	 * 包裹是否已经满
	 */
	export function IsBagFull(): boolean {
		if (Global.Data.roleData.GoodsDataList == null) {
			return false;
		}
		let totalGridNum = 0;
	 ...
	 * 获取包裹剩余空格数量
	 */
	export function GetBaoGuoSpaceCount(): number {
     ...
     * @param goodsID 道具ID
     */
	export function GetTotalGoodsCountByID(goodsID: number): number {
	 ...
	 * @param goodsID 道具ID
	 */
	export function GetTotalBindingGoodsCountByID(goodsID: number): number {
		if (null == Global.Data.roleData.GoodsDataList) return 0;
	 ...
	 * @param strRequire 
	 */
	export function GetTotalGoodsCountByRequire(strRequire: string = ""): number {
		if (null == Global.Data.roleData.GoodsDataList) return 0;
		...
		if (strRequire !== "") {
			...
			for (let i = 0; i < lstGoodsData.length; i++) {
				...
				if (gd.GoodsID !== goodsID) continue;
				if (IsGoodsTimeOver(gd)) continue; // 如果已经超时
	 ...
	 * @param goodsDataList 
	 */
	export function GetGoodsDataByDbID(dbID: number, goodsDataList: NetMsg.IGoodsData[] = null): NetMsg.IGoodsData {
		if (null == goodsDataList) goodsDataList = Global.Data.roleData.GoodsDataList;
		if (null == goodsDataList) return null;
	 ...
	 * @param goodsID 
	 */
	export function GetGoodsDataByID(goodsID: number): NetMsg.IGoodsData {
		if (null == Global.Data.roleData.GoodsDataList) return null;
		const lstGoodsData = Global.Data.roleData.GoodsDataList;
		for (let i = 0; i < lstGoodsData.length; i++) {
	 ...
	 * @param isBind 
	 */
	export function GetIsBindGoodsDataByID(goodsID: number, isBind: boolean): NetMsg.IGoodsData {
		if (null == Global.Data.roleData.GoodsDataList) return null;
	 ...
	 */
	export function AddGoodsData(goodsData: NetMsg.IGoodsData): void {
		if (null == Global.Data.roleData.GoodsDataList) {
	 ...
	 * @param goodsData 
	 */
	export function RemoveGoodsData(goodsData: NetMsg.IGoodsData): void {
		if (null == Global.Data.roleData.GoodsDataList) return;
		const lstGoodsData = Global.Data.roleData.GoodsDataList;
	 ...
	 * @param newGoodsCount 
	 */
	export function CanTakeNewGoodsByGridNum(newGoodsCount: number): boolean {
	 ...
	 */
	function GetGoodsUsedGrid(): number {
		if (Global.Data.roleData.GoodsDataList == null) return 0;
		const lstGoodsData = Global.Data.roleData.GoodsDataList;
		let ret = 0;
		for (let i = 0; i < lstGoodsData.length; i++) {
	 ...
	 * @param goodsID 
	 */
	export function UsingGoodsByID(goodsID: number): boolean {
	 ...
	 * @param ignoreUsing 
	 */
	export function GetGoodsDataIndexByDbID(dbID: number, ignoreUsing: boolean = true): number {
		if (null == Global.Data.roleData.GoodsDataList) return -1;
	 ...
	 * @param isusing 
	 */
	export function GetGoodsDataByType(dbID: number, goodsID: number, isusing: number = 0): NetMsg.IGoodsData {
		if (Global.Data.roleData.GoodsDataList == null) return null;
		let goodVO = tableMgr.goodsTable.Find(goodsID);
		if (null == goodVO) return null;

		const lstGoodsData = Global.Data.roleData.GoodsDataList;
		const categoriy1 = goodVO.Categoriy;
		for (let i = 0; i < lstGoodsData.length; i++) {
			if (lstGoodsData[i].Id === dbID) continue;
			if (isusing !== lstGoodsData[i].Using) continue;
			goodVO = tableMgr.goodsTable.Find(lstGoodsData[i].GoodsID);
			if (null == goodVO) continue;
     ...
     * @param errHint 
     */
	export function CanDirectUseGoods(goodsID: number, errHint: boolean = true): boolean {
		const goodVO = tableMgr.goodsTable.Find(goodsID);
		if (!goodVO) return false;
		const categoriy = goodVO.Categoriy;
		if (categoriy >= (<number>ItemCategories.TouKui) && categoriy < (<number>ItemCategories.EquipMax)) return true;
		const glui = goodVO.GlUI;
		if (glui > 0) return true;

		const goodsName = Loca.getLang(goodVO.Title);
		const usingMode = goodVO.UsingMode;
		if (usingMode <= 0) {
			if (errHint) {
				uiMgr.hintText(String.Format(Loca.getLang("【{0}】不能被直接使用"), goodsName));
			}
			return false;
		}
		return true;
     ...
     * @param goodsID 道具ID
     */
	export function GetGoodsToTypeLimitString(goodsID: number): Map<string, number> {
		...
		const typeArr: string[] = toType.split(",");
		const propertyArr: string[] = toTypeProperty.split(",");
		// 配置有错误，仍旧表示没限制
     ...
     * @param errHint 
     */
	function CanUseGoodsByExtraTypeProperty(toType: string, toTypeProperty: string, errHint: boolean = true): boolean {
		// 这样表示没限制
		if ("-1" === toType || toType.length <= 0) return true;
		const typeArr = toType.split(",");
		const propertyArr = toTypeProperty.split(",");
		// 配置有错误，仍旧表示没限制
		if (typeArr.length !== propertyArr.length) return true;
		...
		for (let index = 0; index < typeArr.length; index++) {
			...
			if (!IsRoleReachLimit(type, gateValue)) {
				if (errHint) {
					uiMgr.hintText(String.Format(Loca.getLang("需要{0}才能使用"), GetEquipLimitString(type, gateValue)));
				}
				return false;
			}
		}
     ...
     * @param gateValue 
     */
	function IsRoleReachLimit(type: string, gateValue: string): boolean {
		// 限制值用整数值
		const theGateValue = parseInt(gateValue);
		let curValue = theGateValue + 1; // 默认让可以使用，客户端暂时不好取这些值
		if (String.isEqualIgnoreCase(type, GoodRequirementTypes.WingSuit)) { // 翅膀阶级
			if (Global.Data.roleData.MyWingData != null) {
				curValue = Global.Data.roleData.MyWingData.WingID;
			}
		} else if (String.isEqualIgnoreCase(type, GoodRequirementTypes.ChengJiuLevel)) { // 成就阶数
			curValue = GetRoleCommonUseParamsValue(RoleCommonUseIntParamsIndexs.ChengJiu);
		} else if (String.isEqualIgnoreCase(type, GoodRequirementTypes.JunXianLevel)) { // 军衔阶数
			curValue = GetRoleCommonUseParamsValue(RoleCommonUseIntParamsIndexs.ShengWangLevel);
		} else if (String.isEqualIgnoreCase(type, GoodRequirementTypes.ZhuanShengLevel)) { // 角色重生等级
			curValue = Global.Data.roleData.ChangeLifeCount;
		} else if (String.isEqualIgnoreCase(type, GoodRequirementTypes.Level)) { // 角色等级
			curValue = Global.Data.roleData.Level;
		} else if (String.isEqualIgnoreCase(type, GoodRequirementTypes.HuFuSuit)) { // 护身符阶数
			curValue = GetHuShengFuLevel();
		} else if (String.isEqualIgnoreCase(type, GoodRequirementTypes.DaTianShiSuit)) { // 大天使阶数
			curValue = GetMaxDaTianShiLevel();
		} else if (String.isEqualIgnoreCase(type, GoodRequirementTypes.VIP)) { // VIP等级
			curValue = GetVIPLeve();
		} else if (String.isEqualIgnoreCase(type, GoodRequirementTypes.NeedMarry)) { // 需要结婚
		...
		} else if (String.isEqualIgnoreCase(type, GoodRequirementTypes.NeedBangHui)) { // 需要加入帮会
			if (!IsHavingBangHui(Data.roleData)) {
				curValue = 0;
			}
		}
     ...
     * @param gateValue 
     */
	function GetEquipLimitString(type: string, gateValue: string): string {
		// 限制值用整数
		const theGateValue = parseInt(gateValue);
		let curValue = "";
		if (String.isEqualIgnoreCase(type, GoodRequirementTypes.WingSuit)) { // 翅膀阶数
			curValue = String.Format(Loca.getLang("翅膀阶数{0}"), theGateValue);
		} else if (String.isEqualIgnoreCase(type, GoodRequirementTypes.EquipSuit)) { // 装备阶数
			curValue = String.Format(Loca.getLang("装备阶数{0}"), theGateValue);
		} else if (String.isEqualIgnoreCase(type, GoodRequirementTypes.QiangHuaLevel)) { // 强化等级
			curValue = String.Format(Loca.getLang("强化等级{0}"), theGateValue);
		} else if (String.isEqualIgnoreCase(type, GoodRequirementTypes.ZhuiJiaLevel)) { // 附加等级
			curValue = String.Format(Loca.getLang("附加等级{0}"), theGateValue);
		} else if (String.isEqualIgnoreCase(type, GoodRequirementTypes.ChengJiuLevel)) { // 成就阶数
			curValue = String.Format(Loca.getLang("成就阶数{0}"), theGateValue);
		} else if (String.isEqualIgnoreCase(type, GoodRequirementTypes.JunXianLevel)) { // 军衔阶数
			curValue = String.Format(Loca.getLang("军衔阶数{0}"), theGateValue);
		} else if (String.isEqualIgnoreCase(type, GoodRequirementTypes.ZhuanShengLevel)) { // 角色重生数
			curValue = String.Format(Loca.getLang("角色重生数{0}"), theGateValue);
		} else if (String.isEqualIgnoreCase(type, GoodRequirementTypes.Level)) { // 角色等级
			curValue = String.Format(Loca.getLang("角色等级{0}"), theGateValue);
		} else if (String.isEqualIgnoreCase(type, GoodRequirementTypes.HuFuSuit)) { // 护身符阶数
			curValue = String.Format(Loca.getLang("护身符阶数{0}"), theGateValue);
		} else if (String.isEqualIgnoreCase(type, GoodRequirementTypes.PetLevel)) { // 宠物等级
			curValue = String.Format(Loca.getLang("宠物等级{0}"), theGateValue);
		} else if (String.isEqualIgnoreCase(type, GoodRequirementTypes.YuanSuZhiXinLevel)) { // 元素之心等级
			curValue = String.Format(Loca.getLang("元素之心等级{0}"), theGateValue);
		} else if (String.isEqualIgnoreCase(type, GoodRequirementTypes.DaTianShiSuit)) { // 大天使阶数
			curValue = String.Format(Loca.getLang("传说装阶数{0}"), theGateValue);
		} else if (String.isEqualIgnoreCase(type, GoodRequirementTypes.VIP)) { // VIP等级
			curValue = String.Format(Loca.getLang("贵族等级{0}"), theGateValue);
		} else if (String.isEqualIgnoreCase(type, GoodRequirementTypes.NeedTask)) { // 主线任务
			const voTask = tableMgr.tasksTable.Find(theGateValue);
			if (voTask) {
				curValue = String.Format(Loca.getLang("需要完成{0}重{1}级主线任务{2}"), voTask.MinZhuanSheng, voTask.MinLevel, voTask.Title);
			}
		} else if (String.isEqualIgnoreCase(type, GoodRequirementTypes.CanNotBeyondLevel)) { // 等级
			const aStrVal = gateValue.split("|");
			curValue = String.Format(Loca.getLang("达到{0}重{1}级以上无法使用"), aStrVal[0], aStrVal[1]);
		} else if (String.isEqualIgnoreCase(type, GoodRequirementTypes.FEIANQUANQU)) { // 安全区
			curValue = Loca.getLang("不能在安全区使用");
		} else if (String.isEqualIgnoreCase(type, GoodRequirementTypes.NeedMarry)) { // 需要结婚
			curValue = Loca.getLang("结婚");
		} else if (String.isEqualIgnoreCase(type, GoodRequirementTypes.NeedBangHui)) { // 需要帮会
			curValue = Loca.getLang("加入帮会");
		}

		if (String.isEqualIgnoreCase(type, GoodRequirementTypes.JunXianLevel)) { // 要对军衔进行特殊处理
			// TODO: 军衔表暂时没有
		...
		} else if (String.isEqualIgnoreCase(type, GoodRequirementTypes.ChengJiuLevel)) {
     ...
     * @param goodsID 道具ID
     */
	export function JugeOccupationGoodsID(goodsID: number): boolean {
     ...
     */
	export function CanUseGoods(goodsID: number, errHint: boolean = true, jugeOccupation: boolean = true, bJudgeAttr: boolean = true): boolean {
		const goodVO = tableMgr.goodsTable.Find(goodsID);
		if (!goodVO) return false;
		...
		if (jugeOccupation) {
			if (toOccupation >= 0) {
				if (0 === ((1 << CalcOriginalOccupationID(Global.Data.roleData.Occupation)) & toOccupation)) {
					if (errHint) {
		...
		if (<number>ItemCategories.Fashion === categoriy) {
			if (IsOperateUnPermitInKuaFuMapCheck(errHint, true)) {
				return false;
			}
		}
		...
		if (errHint) {
			...
			if (execMagic && -1 !== execMagic.indexOf("SUB_ZUIEZHI")) {
				if (Global.Data.roleData.PKValue <= 0) {
					uiMgr.hintText(String.Format(Loca.getLang("罪恶值已经为0，不需要使用【{0}】"), goodsName));
					return false;
				}
			}
		}
     ...
     * @param errHint 
     */
	export function CanUseGoodsAttr(goodsID: number, errHint: boolean): boolean {
		const goodVO = tableMgr.goodsTable.Find(goodsID);
		if (!goodVO) return false;

		const categoriy = TableUtils.getFieldNumber(goodVO.Categoriy);
		...
		if (toSex >= 0) {
			if (Global.Data.roleData.RoleSex !== toSex) {
				if (errHint) uiMgr.hintText(Loca.getLang("性别不对应，不能使用！"));
				return false;
			}
		}

		if (tozhuangsheng >= 0) {
			if (categoriy !== (<number>ItemCategories.ItemNormalPack) && categoriy !== (<number>ItemCategories.ItemUpPack)) {
				if (Global.Data.roleData.ChangeLifeCount === tozhuangsheng) {
					if (toLevel >= 0) {
						if (Global.Data.roleData.Level < toLevel) {
							if (errHint) uiMgr.hintText(Loca.getLang("只有等级达到要求才能可使用！"));
							return false;
						}
					}
				}
				else if (Global.Data.roleData.ChangeLifeCount < tozhuangsheng) {
					if (errHint) uiMgr.hintText(Loca.getLang("只有重生等级达到要求才能可使用！"));
					return false;
				}
			}
		}

		if (categoriy >= 0 && categoriy < (<number>ItemCategories.EquipMax)) {
			if (GetCurrentRoleProp(1, UnitPropIndexes.Strength) < goodVO.Strength) {
				if (errHint) uiMgr.hintText(Loca.getLang("您当前的力量值不够，无法佩戴！"));
				return false;
			}
			else if (GetCurrentRoleProp(1, UnitPropIndexes.Intelligence) < goodVO.Intelligence) {
				if (errHint) uiMgr.hintText(Loca.getLang("您当前的智力值不够，无法佩戴！"));
				return false;
			}
			else if (GetCurrentRoleProp(1, UnitPropIndexes.Dexterity) < goodVO.Dexterity) {
				if (errHint) uiMgr.hintText(Loca.getLang("您当前的敏捷值不够，无法佩戴！"));
				return false;
			}
			else if (GetCurrentRoleProp(1, UnitPropIndexes.Constitution) < goodVO.Constitution) {
				if (errHint) uiMgr.hintText(Loca.getLang("您当前的体力值不够，无法佩戴！"));
				return false;
			}
		}
		return true;
	 ...
	 * @param calcTotal 
	 */
	export function GetGoodsDataPrice(goodsData: NetMsg.IGoodsData, calcTotal: boolean = true): number {
		if (null == goodsData) return 0;
		if (goodsData.GCount <= 0) return 0;
		const goodVO = tableMgr.goodsTable.Find(goodsData.GoodsID);
		if (null == goodVO) return 0;

		const suitID = goodVO.SuitID;
		let level = goodsData.ForgeLevel;
		if (level > MaxForgeLevel()) { // 防止作弊
			level = 0;
		}

		let totalLevel = 0;
		for (let i = 0; i <= level && i <= MaxForgeLevel(); i++) {
			totalLevel += i;
		}
		let quality = goodsData.Quality;
		if (quality >= <number>GoodsQuality.Max) {
			quality = <number>GoodsQuality.Gold;
		}
		let totalQuality = 0;
	 ...
	 */
	function DateTimeEqual(strDateTime1: string, strDateTime2: string): boolean {
     ...
     * @param goodsData 
     */
	export function IsTimeLimitGoods(goodsData: NetMsg.IGoodsData): boolean {
		if (String.IsNullOrWhiteSpace(goodsData.Endtime)) {
			return false;
		}
     ...
     * @param goodsData 
     */
	export function IsGoodsTimeOver(goodsData: NetMsg.IGoodsData): boolean {
	 ...
	 * @param goodsID 
	 */
	export function GetDummyGoodsData(goodsID: number): NetMsg.IGoodsData {
		const goodVO = tableMgr.goodsTable.Find(goodsID);
		if (null == goodVO) return null;
		...
		goodsData.Starttime = ConstGoodsEndTime;
		goodsData.Endtime = ConstGoodsEndTime;
	 ...
	 * @param bornIndex 
	 */
	export function GetDummyGoodsDataEx(goodsID: number, forgeLevel: number, quality: number, binding: number, gcount: number, bornIndex: number): NetMsg.IGoodsData {
		const goodsData = new NetMsg.GoodsData();
		goodsData.Id = -1;
		goodsData.GoodsID = goodsID;
		goodsData.Using = 0;
	 ...
	 * @param endTime 
	 */
	export function GetDummyGoodsDataMu(goodsID: number, forgeLevel: number = 0, zhuijiaLevel: number = 0, zhuoyueIndex: number = 0,
		lucky: number = 0, binding: number = 0, gcount: number = 0, zhuanshengLevel: number = 0, washProps: number[] = null,
		startTime: string = ConstGoodsEndTime, endTime: string = ConstGoodsEndTime): NetMsg.IGoodsData {
		const goodsData = new NetMsg.GoodsData();
		goodsData.Id = -1;
		goodsData.GoodsID = goodsID;
		goodsData.Using = -1;
	 ...
	 */
	export function GetDummyGoodsDataStr(value: string): NetMsg.IGoodsData {
	 ...
	 * @param bCloneUsing 
	 */
	export function CloneGoodsData(gd: NetMsg.IGoodsData, bCloneUsing: boolean = false): NetMsg.IGoodsData {
		const goodsData = new NetMsg.GoodsData();
     ...
     * @param bagIndex 
     */
	export function GetFakeEquipGoodsData(goodsID: number, forge_Level: number, bagIndex: number = 0): NetMsg.IGoodsData {
     ...
     * 获取人物身上的装备列表
     */
	export function GetUsingGoodsDataList(): Map<number, NetMsg.IGoodsData> {
	 ...
	 * 时装装的表独立出来了，在这个清理下
	 */
	export function ClearFashionAndTitleData(): void {
	 ...
	 * @param gd 
	 */
	export function UpdateRoleUsingGoodsDataList(gd: NetMsg.IGoodsData): void {
		if (!gd) return;
	...
	const GoodsShaderIDsCachingDict = new Map<number, number[]>(); // 物品的ShaderID列表缓存字典
	/**
	 * 根据物品ID获取shaderID列表
	 * @param goodsID 道具ID
	 */
	function GetGoodsShaderIDArraysByID(goodsID: number): number[] {
		let shaderIDs: number[] = null;
		...
		const goodVO = tableMgr.goodsTable.Find(goodsID);
		if (!goodVO) return null;
		const shaderID = goodVO.ShaderID;
		shaderIDs = String2IntArray(shaderID, ",");
		if (!shaderIDs) return null;
	 ...
	 * @param forge_Level 
	 */
	export function GetGoodsShaderIDsByID(goodsID: number, forge_Level: number): number {
		if (forge_Level <= 0) {
			return 0;
		}
	 ...
	 * @param occupation 
	 */
	export function CalcWeaponState(weaponGoodsDataList: NetMsg.IGoodsData[], weaponGameObjectList: Object[], occupation: number): WeaponStates {
		...
		for (let i = 0; i < weaponGoodsDataList.length; i++) {
			...
			if (!goodVO) continue;
			const categoriy = goodVO.Categoriy;
			if (categoriy >= (<number>ItemCategories.WuQi_Jian) && categoriy <= (<number>ItemCategories.WuQi_Dao)) {
				if (<number>ItemCategories.WuQi_Dun === categoriy) {
					continue;
				}
				const tempWS = GetGoodsActionNameByID(weaponGoodsDataList[i].GoodsID);
		...
		if (0 === CalcOriginalOccupationID(occupation)) { // 如果是战士
	 ...
	 * @param needCheck 
	 */
	export function ToUseGoodsDBId(goodsDBId: number, needCheck: boolean = true): void {
	 ...
	 * 当前地图是否显示跟随宠物
	 */
	export function IsShowPetInCurMap(): boolean {
	 ...
	 * 获取使用的骑乘
	 */
	function GetActiveHorseGoodsData(): NetMsg.IGoodsData {
		if (!Data.equipPet) return null;
		for (let i = 0; i < Global.Data.equipPet.length; i++) {
			if (Data.equipPet[i].Using === 1) {
				return Data.equipPet[i];
			}
		}
		return null;
	}
	 ...
	 * @param failHint 
	 */
	export function TryRideHorseOn(failHint: boolean = false): void {
		// TODO:
	}
	 ...
	 * 卸下坐骑
	 */
	export function TryRideHorseOff(): void {
		// TODO:
	}
	 ...
	 * @param failHint 
	 */
	export function TryRideJiJiaOnFast(failHint: boolean = false): void {
		// TODO:
	}
	 ...
	 * @param offHorse 
	 */
	export function TryRideJiJiaOn(failHint: boolean = false, offHorse: boolean = true): void {
		// TODO:
	}
	 ...
	 * @param failHint 
	 */
	function TryRideJiJiaOff(failHint: boolean = false): void {
		// TODO:
	}
	 ...
	 * @param failHint 
	 */
	export function TrySwitchJiJia(failHint: boolean = false): void {
		// TODO:
	}
	 ...
	 * @param failHint 
	 */
	export function TrySwitchHorse(failHint: boolean = false): boolean {
		// TODO:
		return true;
	}
	 ...
	 * 获取是否正在骑乘状态
	 */
	export function GetIsRidingHorse(): boolean {
		const curHorseData = GetActiveHorseGoodsData();
		return curHorseData && curHorseData.Using === 1 && Data.roleData.HorseDbID > 0;
	}

	/**
	 * 该接口仅获取当前主角是否骑乘机甲， 并不是获取当前是否是机甲状态
	 */
	export function GetIsRidingJiJia(): boolean {
		// TODO:
		return false;
	}
     ...
     * @param sParam 
     */
	export function ToUseGoods(goodsData: NetMsg.IGoodsData, needCheck: boolean = true, isBatchUse: boolean = false, sParam: string = ""): void {
		if (needCheck) {
			if (!CanUseGoods(goodsData.GoodsID, true)) return;
			if (!CanDirectUseGoods(goodsData.GoodsID, true)) return;
		...
		if (category >= 0 && category < (<number>ItemCategories.EquipMax)) {
			const actionType = goodVO.ActionType; // 佩戴方式
			const handType = goodVO.HandType; // 放置位置 
			let gdList: NetMsg.IGoodsData[] = null;
			if (category >= (<number>ItemCategories.WuQi_Jian) && category <= (<number>ItemCategories.WuQi_NuJianTong)) { // 判断要佩戴的道具否是武器
				gdList = Super.FindWuQi(category, actionType, handType);
				currentMJSType = GetMJSType();
			} else {
				// 找出要除武器外要卸载的装备
				gdList = Super.FindEquip(category);
			}
			if ((<number>ItemCategories.MengChongWu) !== category && (<number>ItemCategories.ChongWu) !== category
				&& (<number>ItemCategories.ShouHuChong) !== category && (<number>ItemCategories.Fashion) !== category) {
			...
			for (let i = 0; i < gdList.length; i++) {
				if (null != gdList[i]) {
					...
					if (gdList[i].Using === 1) {
						...
						if ((<number>ItemCategories.JieZhi) !== category && unhandType !== (<number>HandTypes.ZuoYouShou)) { // 戒指和手镯特殊处理，使用BagIndex标示左右, 0左, 1右
							gdList[i].BagIndex = gdBagIndex; // 将位置替换
						}
						if ((<number>ItemCategories.Fashion) !== category) {
			...
			if (goodsData.Using === 0) {
				...
				// 穿戴前做装备条件判断========
				if (<number>ItemCategories.JieZhi === category || handType === <number>HandTypes.ZuoYouShou) { // 戒指和手镯特殊处理，使用BagIndex标示左右, 0左, 1右
					// 对于戒指和手镯特殊处理，获取其要能放置的位置
					if (category >= <number>ItemCategories.WuQi_Jian && category <= <number>ItemCategories.WuQi_NuJianTong) {
						category = <number>ItemCategories.WuQi_Jian;
					}
					if (category === <number>ItemCategories.ChongWu) { // 守护宠和宠物共占同一个位置，同时只能佩戴一种
						category = <number>ItemCategories.ShouHuChong;
					}
					goodsData.BagIndex = Super.FindEquipBagIndex(category);
				}

				if ((3 === Global.Data.roleData.Occupation) && <number>ItemCategories.WuQi_Zhang === category) { // 魔剑士装备魔杖时做特殊处理
					goodsData.BagIndex = Super.FindEquipBagIndex(category);
				}
				if (<number>ItemCategories.Fashion === category) {
					...
					if (bCanSendActivateFashion) {
						...
						if (fashionInfo.has(goodsData.GoodsID)) {
							...
							const occupation = CalcOriginalOccupationID(Global.Data.roleData.Occupation);
				...
				if (gd) {
					...
					if (canUseGoods) {
						if (!Global.GoodsCoolDown(gd.GoodsID)) {
							...
							uiMgr.hintText(String.Format(Loca.getLang("【{0}】在冷却中, 无法使用"), goodsName));
						}
					} else {
	 ...
	 * @param GoodsData 
	 */
	function CheckBuffer(goodsData: NetMsg.IGoodsData): boolean {
		const goodsBufferId = GetGoodsBufferID(goodsData.GoodsID);
		// 没有绑定buffer，通过验证
		if (goodsBufferId < 0) return true;

		// 可以叠加的直接通过
		// 可以叠加的buffer很少，只有烤火，同类型vip, 不同类型vip会进行覆盖，同类型 和 不同类型的经验buffer不能叠加
		// 烤火
		if ((<number>BufferItemTypes.ZhuFu) === goodsBufferId) {
			return true;
		}
		let hasCheck = false;
		// VIP
		if ((<number>BufferItemTypes.MonthVIP) === goodsBufferId) {
			return true;
		}

		// 二锅头, 生命/魔法圣水, 幸运/卓越神水
		if (<number>BufferItemTypes.ErGuoTou === goodsBufferId ||
			<number>BufferItemTypes.TimeAddLifeNoShow === goodsBufferId ||
			<number>BufferItemTypes.TimeAddMagicNoShow === goodsBufferId ||
			<number>BufferItemTypes.SLDL_ADDLUCKYATTACKPERCENTTIMER === goodsBufferId ||
			<number>BufferItemTypes.SLDL_ADDFATALATTACKPERCENTTIMER === goodsBufferId) {
			...
			if (goodsData.GoodsID === goodsID2) {
				...
				const bufferGoodsID = <number>bufferData.BufferVal;
		...
		const expArr: number[] = [
			<number>BufferItemTypes.MutilExperience,
			<number>BufferItemTypes.FiveExperience,
			<number>BufferItemTypes.ThreeExperience,
			<number>BufferItemTypes.DblExperience
		];
		let currIndex = expArr.indexOf(goodsBufferId);
		if (currIndex >= 0) {
			let existExpBuffer = false;
			if (GetBufferDataByGoodsID(goodsData.GoodsID) != null) {
		...
		const attackArr: number[] = [
			<number>BufferItemTypes.TimeAddAttack,
			<number>BufferItemTypes.TimeAddDSAttack,
			<number>BufferItemTypes.TimeAddMAttack
		];
		currIndex = attackArr.indexOf(goodsBufferId);
		if (currIndex >= 0 && IsBufferExist(BufferItemTypes.PKKingBuffer)) {
			uiMgr.hintText(Loca.getLang("您已拥有怒斩·PK王BUFFER,不能使用该物品"));
			return false; // 直接返回不允许使用
		}

		// 对于其它的buffer，只要不存在，就返回true
		if (!hasCheck && !IsBufferExist(goodsBufferId)) {
			return true;
		}
	 ...
	 * @param goodsID 
	 */
	export function IsEquip(goodsID: number): boolean {
		const cat = GetCategoriyByGoodsID(goodsID);
		if (cat >= (<number>ItemCategories.TouKui) && cat < (<number>ItemCategories.EquipMax) && cat !== (<number>ItemCategories.JieZhi) && cat !== (<number>ItemCategories.XiangLian)) {
			return true;
		}
		return false;
	}
	 ...
	 * @param goodsID 
	 */
	export function GetChangeType(goodsID: number): ModEquipTypes {
		if (goodsID === -1) {
			return ModEquipTypes.Mod_Equip_All;
		}
		const categoriy = GetCategoriyByGoodsID(goodsID);
		if (categoriy >= (<number>ItemCategories.WuQi_Jian) && categoriy < (<number>ItemCategories.EquipMax)) {
			return ModEquipTypes.Mod_Equip_Weapon;
		} else if (categoriy >= (<number>ItemCategories.TouKui) && categoriy <= (<number>ItemCategories.XueZi)) {
			return ModEquipTypes.Mod_Equip_Body;
		} else if (categoriy === (<number>ItemCategories.ChiBang)) {
	 ...
	 * @param goodsData 
	 */
	export function SetEquipGoodsZhanLiStat(icon: MyUI.GoodsIcon, goodsData: NetMsg.IGoodsData) {
		if (!icon || !goodsData) {
			return;
		}
		...
		if (category >= (<number>ItemCategories.TouKui) && category < (<number>ItemCategories.EquipMax)) {
			// 特殊的额外限制，由ToType 决定 ToTypeProperty值的意义，不同项用逗号隔开
			const toType = goodVO.ToType.toString();
			const toTypeProperty = goodVO.ToTypeProperty.toString();
			const baoguoID = goodVO.BaoguoID.toString();
			...
			if (!CanUseGoodsByExtraTypeProperty(toType, toTypeProperty, false)) {
				if (String.IsNullOrEmpty(baoguoID)) {
					return;
				}
			}

			// 判断职业
			if (0 === ((1 << CalcOriginalOccupationID(Global.Data.roleData.Occupation)) & goodVO.ToOccupation)) {
				return;
			}
			...
			for (let i = 0; i < gdList.length; i++) {
				if (gdList[i] != null) {
					iEquipZl += GetGoodsDataZhanLi(gdList[i]);
				}
			}
			const zl = GetGoodsDataZhanLi(goodsData);
	...
	let CanUseGoodsByTaskDict: Map<number, number> = null;
	/**
	 * 判断物品是否能够使用
	 * 只有在功能没有开启的情况下，才返回false，默认返回true
	 * @param goodsID 
	 */
	export function IsCanUseGoodsByGongnengID(goodsID: number): boolean {
		if (null == CanUseGoodsByTaskDict) {
			...
			CanUseGoodsByTaskDict = new Map<number, number>();
	...
	//#region ////////////// 角色装备属性 //////////////
	// 锻造的加成属性比例    
	let ForgeLevelAddAttackRates: number[] = null; // 加成攻击
	let ForgeLevelAddDefenseRates: number[] = null; // 加成防御
	let ForgeLevelAddMaxLifeVRates: number[] = null; // 加成生命上限
	let ForgeLevelPet: number[] = null; // 宠物属性加成
	let ChiBangForgeLevelAddDefenseRates: number[] = null; // 翅膀加成防御
	let ChiBangForgeLevelAddShangHaiJiaCheng: number[] = null; // 翅膀伤害加成
	let ChiBangForgeLevelAddShangHaiXiShou: number[] = null; // 翅膀伤害吸收
	const EnchanceLevelAddRates: number[] = null; // 提品的的成功概率
	 ...
	 * @param extPropIndex 
	 */
	export function GetEquipForgeAddActiveExtraValue(gd: NetMsg.IGoodsData, extPropIndex: number): number {
		// 不满足条件的直接返回
		...
		for (let index = 0; index < activateList.length; index++) {
			...
			if (3 !== activateItem.length) continue;
	 ...
	 * @param gd 
	 */
	export function GetEquipForgeAddActiveExtraLifeValue(gd: NetMsg.IGoodsData): number {
	 ...
	 * @param level 
	 */
	export function GetRoleBaseLifeV(occupation: number, level: number): number {
     ...
     * @param goodsID 
     */
	export function GetGoodsEquipPropsDoubleList(goodsID: number): number[] {
		let equipFields: number[] = [];
	 ...
	 * @param index 
	 */
	export function GetBufferGoodsID(bufferType: number, index: number): number {
		if (index < 0) return 0;
		let paramName = "";
		...
		if (paramName.length <= 0) return 0;
	 ...
	 * @param goodsID 
	 */
	export function GetGoodsEquipPropsStringForBufferTips(goodsID: number): string {
	 ...
	 * @param goodsID 
	 */
	export function GetEquipForgeAddActivateList(goodsID: number): string[] {
		// 查找物品项，找到相关强化索引参数ExecMagic
		const goodVO = tableMgr.goodsTable.Find(goodsID);
		if (!goodVO) return null;
     ...
     */
	export function GetEquipForgeAddBaseValue(gd: NetMsg.IGoodsData, extPropIndex: number): number {
		const forgeLevel = gd.ForgeLevel;
		// 由于强化开关处于关掉状态，装备在15阶以上，Tips里的属性强化值为0，所以将此处的最大等级条件限制写死为20，原为(Global.MaxForgeLevel)
		const nCategory = GetCategoriyByGoodsID(gd.GoodsID);
	...
	let ZhuoYueAddDefenseRates: number[] = null;
    /**
     * 获得卓越装备增加的防御力加成比例
     * @param flag 卓越信息
     */
	export function GetZhuoYueAddDefenseRates(flag: number): number {
	...
	let ZhuoYueAddAttackRates: number[] = null;
    /**
     * 获得卓越装备增加的攻击力加成比例
     * @param flag 卓越信息
     */
	export function GetZhuoYueAddAttackRates(flag: number): number {
     ...
     * @param flag 卓越信息
     */
	export function GetZhuoYueAddIndex(flag: number): number {
	 ...
	 * @param attackType 
	 */
	export function GetBornAttackValue(bornIndex: number, attackType: number): number {
	 ...
	 * @param extPropIndex 
	 */
	export function GetEquipBornAddBaseValue(gd: NetMsg.IGoodsData, extPropIndex: number): number {
	...
	let ZhuijiaLevelAddAttackRates: number[] = null; // 追加增加的攻击属性
	let ZhuijiaLevelAddDefenseRates: number[] = null; // 追加增加的防御属性
    /**
     * 返回装备追加属性增加的基础值
     * @param gd 装备数据
     * @param extPropIndex 属性索引
     */
	export function GetEquipZhuijiaAddBaseValue(gd: NetMsg.IGoodsData, extPropIndex: number): number {
		const zhuijiaLevel = gd.AppendPropLev;
		if (zhuijiaLevel > MaxZhuijiaLevel || zhuijiaLevel < 0) {
			return 0;
		}
	...
	let ZhuanshengLevelAddAttackRates: number[] = null; // 重生加的攻击属性
	let ZhuanshengLevelAddDefenseRates: number[] = null; // 重生增加的防御属性
    /**
     * 返回装备重生属性增加的基础值
     * @param gd 装备数据
     * @param extPropIndex 属性索引
     */
	export function GetEquipZhuanshengAddBaseValue(gd: NetMsg.IGoodsData, extPropIndex: number): number {
		const zhuanshengLevel = gd.ChangeLifeLevForEquip;
		if (zhuanshengLevel > MaxZhuanshengLevel || zhuanshengLevel < 0) {
			return 0;
		}
	 ...
	 * @param fillCount 
	 */
	export function GetBaseAttributeStrFromPropertyList(equipFields: number[], defaultExpression: boolean = true, fillCount: number = 0): string {
		let str = "";
		let extPropName = "";
	 ...
	 * @param levelFactor 
	 */
	export function CalStatueExpression(baseAttr: number, grade: number, level: number, gradeFactor: number = 0, levelFactor: number = 0): number {
	 ...
	 * @param str 
	 */
	export function ProcessStr(str: string): string {
	 ...
	 * @param goodVO 
	 */
	export function GetEquipExtPropsItemVal(gd: NetMsg.IGoodsData, da: number[], itemIndex: number, goodVO: tables.GoodsVO): number {
		if (null == goodVO) return 0.0;
		if (null == gd) return 0.0;
		// 套装相关---暂时不起作用
		const suitID = goodVO.SuitID;
		// 装备原始属性
	 ...
	 * @param sa 
	 */
	export function RecalcEquipProp(goodVO: tables.GoodsVO, goodsData: NetMsg.IGoodsData, sa: number[]): number[] {
		if (null == goodsData) {
			return sa;
		}
	 ...
	 * @param gd 
	 */
	export function GetZhuoyueAttributeCountByGoodsData(gd: NetMsg.IGoodsData): number {
     ...
     */
	export function GetZhuoyueAttributeCount(excellenceInfo: number): number {
		if (excellenceInfo <= 0) {
			return 0;
		}
		...
		for (let i = 0; i < maxNum; i++) {
			if (GetIntSomeBit(excellenceInfo, i) === 1) {
				count = count + 1;
			}
		}
		return count;
     ...
     */
	export function GetMaxZhuijiaLevelByGoodsData(goodsData: NetMsg.IGoodsData): number {
	 ...
	 * @param goodsData 
	 */
	export function GetMaxWashPropsCountByGoodsData(goodsData: NetMsg.IGoodsData): number {
		const zhuoyueNum = GetZhuoyueAttributeCount(goodsData.ExcellenceInfo);
		if (zhuoyueNum === 0) {
			return 0;
		}
     ...
     */
	export function GetColorByGoodsData(goodsData: NetMsg.IGoodsData): string {
	 ...
	 * @param goodsData 
	 */
	export function GetStrColorByGoodsData(goodsData: NetMsg.IGoodsData): string {
     ...
     * @param goodsData 道具数据
     */
	export function GetGoodsColorByGoodsData(goodsData: NetMsg.IGoodsData): GoodsColor {
     ...
     * @param funcName 
     */
	export function GetGoodsExecMagicdoubleParamsList(goodsId: number, funcName: string = ""): number[] {
		const intArr: number[] = [];
		const goodVO = tableMgr.goodsTable.Find(goodsId);
		if (!goodVO) return intArr;
     ...
     * @param goodsId 
     */
	function GetGoodsBufferID(goodsId: number): number {
		const goodVO = tableMgr.goodsTable.Find(goodsId);
		if (!goodVO) return -1;
		...
		if (-1 === bufferId && !String.IsNullOrEmpty(funcName)) {
     ...
     * @param goodsId 
     */
	export function GetGoodsBindBufferID(goodsId: number): number {
		const goodVO = tableMgr.goodsTable.Find(goodsId);
		if (!goodVO) return -1;
		// 格式:FUNCTIONTEST(1,22,323,9)
		const excuteMagic = TableUtils.getFieldString(goodVO.ExecMagic);
     ...
     * @param funcName 
     */
	function GetBufferIdByFuncName(funcName: string): number {
		const mapFunc = GetFuncNameToBufferIdMap();
		for (const [key, value] of mapFunc) {
			if (String.isEqualIgnoreCase(key, funcName)) {
				return value;
			}
		}
		return -1;
	...
	let FuncNameToBufferIdMap: Map<string, BufferItemTypes> = null; // 脚本函数到bufferid的映射
    /**
     *  脚本函数到bufferid的映射
     */
	function GetFuncNameToBufferIdMap(): Map<string, BufferItemTypes> {
		if (null == FuncNameToBufferIdMap) {
			FuncNameToBufferIdMap = new Map<string, BufferItemTypes>();
	...
	/** 添加新的物品cooldown通知 */
	export let NotifyGoodsAddCoolDown: DPSelectedItemEventHandler = null;

	const GoodsCoolDownDict = new Map<number, CoolDownItem>();
	/**
	 * 判断物品是否处于冷却状态
	 * @param goodsID 
	 */
	export function GoodsCoolDown(goodsID: number): boolean {
		let coolDownItem: CoolDownItem = null;
	 ...
	 * @param goodsID 
	 */
	export function GetGoodsCoolDownTicks(goodsID: number): number {
		let coolDownItem: CoolDownItem = null;
		if (!GoodsCoolDownDict.has(goodsID)) {
			coolDownItem = GoodsCoolDownDict.get(goodsID);
			return 0;
	 ...
	 * @param goodsID 
	 */
	export function AddGoodsCoolDown(goodsID: number): void {
		...
		AddCoolDownItem(GoodsCoolDownDict, goodsID, nowTicks, cdTime * 1000);
		if (cdGroup > 0) {
			for (let i = 0; i < Global.Data.roleData.GoodsDataList.length; i++) {
				...
				if (cdGroup === goodVO.ShareGroupID) { // 同组
					AddCoolDownItem(GoodsCoolDownDict, goodsData.GoodsID, nowTicks, pubCDTime * 1000);
				}
			}
		}
		// 通知外部
		if (NotifyGoodsAddCoolDown) {
			NotifyGoodsAddCoolDown(null, DPSelectedItemEventArgs.Empty);
		}
	}
	 ...
	 */
	export function GetFashionEquipGoodsDataList(GoodsID: number, occupation: number, FashionForgeLev: number): NetMsg.IGoodsData[] {
		const equipGoodsDataList = new Array<NetMsg.IGoodsData>();
		...
		let szriDic: Map<number, tables.ShiZhuangResVO> = null;
		if (dic.has(GoodsID)) {
	...
	const shiZhuangDic = new Map<number, Map<number, tables.ShiZhuangResVO>>(); // 时装字典信息
	/**
	 * 获取时装字典信息
	 */
	export function GetFashionInfo(): Map<number, Map<number, tables.ShiZhuangResVO>> {
		if (shiZhuangDic.size > 0) return shiZhuangDic;
		const aVORes = tableMgr.shiZhuangResTable.AllRows();
		aVORes.forEach(element => {
			if (!shiZhuangDic.has(element.GoodsID)) {
				const dic = new Map<number, tables.ShiZhuangResVO>();
	 ...
	 * 获取时装列表
	 */
	export function GetRoleFashionList(): NetMsg.IGoodsData[] {
		const lst: NetMsg.IGoodsData[] = [];
		if (Data.fashionAndTitleList) {
			for (let i = 0; i < Data.fashionAndTitleList.length; ++i) {
				const data = Data.fashionAndTitleList[i];
				if (!data) continue;
				if (ItemCategories.Fashion === GetCategoriyByGoodsID(data.GoodsID)) {
					if (1 === data.Using) lst.unshift(data);
					else lst.push(data);
				}
			}
		}
     ...
     * @param id 节点ID
     */
	export function GetXilianXmlNode(id: string): tables.XiLianShuXingVO {
		return tableMgr.xiLianShuXingTable.Find(id);
	}

	const washPropsNames: string[] = ["MaxLifeV", "AddAttackInjure", "DecreaseInjureValue", "AddAttack", "AddDefense", "HitV", "Dodge", "LifeSteal"];
    /**
     * 获取某件装备的洗炼属性上限
     * @param gd 装备数据
     */
	export function GetXilianPropsUpLimitDict(gd: NetMsg.IGoodsData): Map<number, number> {
		const goodVO = tableMgr.goodsTable.Find(gd.GoodsID);
		if (!goodVO) return null;
		const voXiLian = GetXilianXmlNode(goodVO.XiLian.toString());
		if (!voXiLian) return null;
	...
	let XilianPropsUpFactorDict: Map<number, number> = null;
    /**
     * 获取洗炼属性因子
     * @param gd 
     */
	export function GetXilianPropsUpFactor(gd: NetMsg.IGoodsData): number {
		const factor = 0;
		if (null == XilianPropsUpFactorDict) {
			XilianPropsUpFactorDict = new Map<number, number>();
		...
		const k = <number>GetGoodsColorByGoodsData(gd);
     ...
     * @param gd 道具数据
     */
	function IsToXilianPropsUpLimit(gd: NetMsg.IGoodsData): boolean {
		if (!gd) return false;
     ...
     * 获取身上所有装备的洗炼属性值相加的和
     */
	function GetSumOfAllEquipXilianValue(): number {
		let result = 0;
     ...
     * 获取身上所有装备的追加等级相加的和
     */
	function GetSumOfAllEquipZhuijiaValue(): number {
		let result = 0;
		const usingGoodsDataList = GetUsingGoodsDataList();
		if (!usingGoodsDataList || usingGoodsDataList.size <= 0) {
			return result;
		...
		for (const [key, value] of usingGoodsDataList) {
			...
			if (!goodVO) continue;
			const categoriy = goodVO.Categoriy;
			if (categoriy === <number>ItemCategories.ShouHuChong
				|| categoriy === <number>ItemCategories.ChongWu
				|| categoriy === <number>ItemCategories.JieHunJieZhi) {
				continue;
			}
     ...
     * 获取身上所有装备的强化等级相加的和
     */
	function GetSumOfAllEquipQianghuaValue(): number {
		let result = 0;
		const usingGoodsDataList = GetUsingGoodsDataList();
		if (!usingGoodsDataList || usingGoodsDataList.size <= 0) {
			return result;
		...
		for (const [key, value] of usingGoodsDataList) {
			...
			if (categoriy === <number>ItemCategories.ShouHuChong
				|| categoriy === <number>ItemCategories.ChongWu
				|| categoriy === <number>ItemCategories.JieHunJieZhi
				|| categoriy === <number>ItemCategories.Fashion) {
				continue;
			}
     ...
     * @param gd2 装备数据2
     */
	function IsXilianProps1MoreThan2UpLimit(gd1: NetMsg.IGoodsData, gd2: NetMsg.IGoodsData): boolean {
		if (!gd1) return false;
		if (!gd1.WashProps) return false;
		const dict2 = GetXilianPropsUpLimitDict(gd2);
		const factor2 = GetXilianPropsUpFactor(gd2);
		if (!dict2) return false;
	...
	let DBIdsDictByXilian: Map<number, number> = null;
    /**
     * 字典中是否包含指定ID
     * @param dbID 
     */
	function IsContainDBIDByXilian(dbID: number): boolean {
		if (!DBIdsDictByXilian) return false;
		if (DBIdsDictByXilian.has(dbID)) return true;
		return false;
	}
     ...
     * @param dbID 
     */
	function AddDBIdToDictByXilian(dbID: number) {
		if (!DBIdsDictByXilian) DBIdsDictByXilian = new Map<number, number>();
     ...
     * @param dbID 
     */
	function RemoveDBIdFromDictByXilian(dbID: number) {
		if (!DBIdsDictByXilian) return;
		if (DBIdsDictByXilian.has(dbID)) {
     ...
     * @param goodsData 
     */
	export function GetGoodsSaleToNpcPrice(goodsData: NetMsg.IGoodsData): number {
		let needYinLiang = 0;
     ...
     * @param goodsData 
     */
	export function GetGoodsSaleToNpcZaizao(goodsData: NetMsg.IGoodsData) {
		let result = 0;
		const voGoods = tableMgr.goodsTable.Find(goodsData.GoodsID);
		if (voGoods) {
			const sXiShu = tableMgr.sysParamsTable.getParam("ZhuoYueHuiShouZaiZaoXiShu");
			if (String.IsNullOrWhiteSpace(sXiShu)) {
				return result;
			}
     ...
     * @param goodsData 道具数据
     */
	export function GetGoodsSaleToNpJiFen(goodsData: NetMsg.IGoodsData): number {
		let needYinLiang = 0;
		const goodVO = tableMgr.goodsTable.Find(goodsData.GoodsID);
		if (goodVO) {
			// 获取卓越属性对应的加成比例
	...
	let PetExp: number[] = null; // 计算宠物兑换斗气缓存各级经验链表
    /**
     * 计算宠物兑换斗气
     * @param goodsData 道具数据
     */
	export function GetPetPrice(goodsData: NetMsg.IGoodsData): number {
		const goodVO = tableMgr.goodsTable.Find(goodsData.GoodsID);
		let EXP = goodVO.ZhanHunPrice;
		if (!PetExp) {
			const aVO = tableMgr.petLevelUpTable.AllRows();
			PetExp = new Array<number>(aVO.length + 1);
			aVO.forEach(element => {
