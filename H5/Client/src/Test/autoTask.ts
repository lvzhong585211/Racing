...
namespace AutoTest {
     ...
     */
    export function submitAndAskTask(npcRoleID: number, taskID: number, npcExenstionID: number): void {
     ...
     */
    function getQueryString(paramName: string): string {
     ...
     */
    function getWSUrl(paramName: string, defaultValue: string): string {
     ...
     */
    class RobotBase {
        protected mTcpToGameServer: Net.Socket4Message = null; // 到GameServer的Tcp连接
        protected mAccount: string;             // 帐号
        protected mRoleId: number = -1;         // 角色的Id
        protected mStoped: boolean = false;     // 此机器人是否停止了

        protected mRoleData: NetMsg.RoleData = null;    // 机器人角色的数据
        protected mOrgCoord: Laya.Point = null;         // 机器人的初始坐标

        /**
         * 登录到UserLoginServer与GameServer
         * @param account 使用的帐号,密码同帐号
         */
        public async loginToServer(account: string): Promise<boolean> {
            let socketToLoginServer = new Net.Socket4Message();
            console.log("   .连接LoginServer");
            const connectResult = await socketToLoginServer.connectByUrl(_loginServerUrl);
            if (!connectResult) {
                // 显示连接LoginServer失败
            ...
            // 等待登录返回        
            const retData = await socketToLoginServer.WaitStringMessage(msgType, timeOut);
            ...
            const errMap: { [key: string]: string } = {
                "-1": ConfigLoca.UI_LOGIN_USERWRONG,                // 用户名或密码错误
                "-2": ConfigLoca.UI_LOGIN_VERSIONWRONG,             // 客户端的版本太旧
                "-3": ConfigLoca.UI_LOGIN_USER_NOTEXIST,            // 用户名不存在
                "-4": ConfigLoca.UI_LOGIN_USER_NOTEXIST_PLATFORM,   // 平台账号登录验证失败
            ...
            const connectRet = await webTcp.connectByUrl(_gameServerUrl);
            if (!connectRet) {
            ...
            const clientHeartId: number = -1;

            // 偿试登录
            const RoleRandToken = -1;
            let loginInfo = `${userId}:${userName}:${userToken}:${RoleRandToken}:${Global.VersionInfo.appVersion}:1:robotdeviceid`;

            loginInfo = Logic.KuaFuLoginManager.GetKuaFuLoginString(loginInfo); // 追加跨服登录信息
            let loginInfoFinal = `${loginInfo}:Develop:-1`;

            // 发送登录消息
            webTcp.sendString(EMessageType.CMD_LOGIN, loginInfoFinal);

            // 等待登录返回
            dataFields = await webTcp.WaitStringMsgFields(EMessageType.CMD_LOGIN, timeOut);
            if (dataFields == null) {
            ...
            // 监听所有的网络消息
            webTcp.onMsg(EMessageType.CMD_SOCKET_DEFAULT, null, (uMsgType: EMessageType, msgReader: protobuf.Reader) => {
            ...
            // 等待角色列表返回
            const roleListFields = await webTcp.WaitStringMsgFields(EMessageType.CMD_ROLE_LIST, timeOut);
            ...
            if (!dtRole) {
                ...
                // 等待创建角色返回
                const roleListFields = await webTcp.WaitStringMsgFields(EMessageType.CMD_CREATE_ROLE, timeOut);
            ...
            // 开始登入游戏世界
            const sCmdMsg = `${userId}:${dtRole.roleID}::${Global.VersionInfo.versionDesc}  `; // 后两个参数先不传
            webTcp.sendString(EMessageType.CMD_INIT_GAME, sCmdMsg);

            // 等待登入返回
            const msgReader = await webTcp.WaitMessage(EMessageType.CMD_INIT_GAME, timeOut);
         ...
         * @param args 消息的参数
         */
        public sendMsgToGS(sendFn: (...args: any[]) => void, ...args: any[]) {
            // 为了复用代码,我们这里重新设定一下游戏实例
            const oldGameIns = gameIns;
            gameIns = {
                gameState:
                { RoleID: this.mRoleId },
                sendDataToGS: <T>(msgType: EMessageType, msgObject: T, encode?: { (msgObject: T, writer: protobuf.Writer) }) => {
                    this.sendDataToGS(msgType, msgObject, encode);
                },
                sendStringToGS: (msgType: EMessageType, msgData: string) => {
                    this.sendStringToGS(msgType, msgData);
                },
            } as GameMode.GameInstance;
    ...
    let _mapDatas: Logic.GMapData[];    // 缓冲加载的关卡寻路信息

    /** 获取指定关卡的寻路数据 */
    async function getMapData(levelId: number, sceneId: number): Promise<Logic.GMapData> {
        if (_mapDatas == null) {
            _mapDatas = new Array<Logic.GMapData>();
        }
        ...
        if (!mapData) { // 如果关卡寻路信息不存在,则加载
            ...
            await mapData.load(levelId, sceneId);
        }
 ...
 */
        public async loginToServer(account: string): Promise<boolean> {
            const ret = await super.loginToServer(account);
            if (ret) {
                // 开始随机战斗的逻辑
                this.randomFight();
            }
        ...
        */
        private async randomFight() {
            while (!this.mStoped) {
                ...
                // 等待怪物查找返回的消息
                const reader = await this.mTcpToGameServer.WaitMessage(EMessageType.CMD_SPR_FINDMONSTER, 1);
                if (reader) {
    ...
    const _webTcp: Net.Socket4Message = null; // 拦截消息发送用的socket
    let _loginServerUrl: string = null;     // loginServer的地址
    let _gameServerUrl: string = null;      // GameServer的地址

    async function _robotType(account: string, robotType: number) {
     ...
     */
    async function _robotRandomMove(account: string) {
        console.log("=== 正在登录随机移动机器人(%s)", account);
        const robot = new RobotRandomMove();
        const ret = await robot.loginToServer(account);
        if (ret) {
     ...
     */
    async function _robotRandomFight(account: string) {
     ...
     * 自动登录,在主城自由走动的机器人
     */
    export async function robotRandomMove() {
        ...
        for (let index = robotStartIndex, endIndex = robotStartIndex + robotCount; index < endIndex;) {
            ...
            const ret = await _robotType("robot" + index, robotType);
            if (ret) {
                loginedRobot++;
                ++index;
            }
