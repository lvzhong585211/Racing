namespace tables {
	...
	export interface IDataTable {
		readonly mTableName: string;		// 数据表的名称
		/**
		 * 加载数据表
		 * 注： 这必须在加载了对应的表资源后才可以
	 ...
	 */
	export class DataTable<TRowType, TKeyType> implements IDataTable {
		public readonly mTableName: string;				// 数据表的名称
		public readonly mKeyFieldName: string;			// 索引列的名称
		protected rowDataCache: Map<TKeyType, TRowType> | TRowType[];	// 数据表的行

		public static mAllTables = new Array<IDataTable>();	// 保存所有的数据表对象，仅供DataTableManager使用
		protected mRowConverter: { (jsonRowData: any): TRowType };	// 行对象的转换方法
		protected mRowCtor: { new(): TRowType; };				// 行对象的构造函数
		private mKeyType: string;								// 索引字段的类型
		 ...
		 */
		public constructor(tableName: string, keyFieldName?: string, keyType?: TKeyType, rowCtor?: { new(): TRowType; }, rowConverter?: { (jsonRowData: any): TRowType }) {
		 ...
		 * 注： 仅仅供DataTableManager调用
		 */
		public loadTable(tableResUrl?: string): boolean {
			...
			if (this.mKeyFieldName) {
				...
				this.rowDataCache = new Map<TKeyType, TRowType>();
				jsonData.forEach((rowData, index) => {
					let rowObj: any = rowData;
					...
					this.addRowDataToMap(key, rowObj);
				});
			}
			else {
		 ...
		 * @param value value值
		 */
		protected addRowDataToMap(key: TKeyType, value: TRowType) {
			if (this.rowDataCache instanceof Map) {
				this.rowDataCache.set(key, value);
			}
		}
		 ...
		 */
		public Find(keyFieldValue: TKeyType): TRowType {
			if (this.rowDataCache instanceof Map) {
		 ...
		 * @param val 属性值
		 */
		public FindByAttrAndValue(attr: string, val: string): TRowType {
			if (this.rowDataCache instanceof Map) {
				...
				return this.rowDataCache.find(
					(element) => element[attr] && element[attr].toString() === val,
				);
			}
			return null;
		}
		 ...
		 * @param val 属性值
		 */
		public FindArrayByAttrAndValue(attr: string, val: string): TRowType[] {
			const lst: TRowType[] = [];
			if (this.rowDataCache instanceof Map) {
				for (const [key, element] of this.rowDataCache) {
					if (element[attr] && element[attr].toString() === val) {
		 ...
		 */
		public AllRows(): TRowType[] {
			if (this.rowDataCache instanceof Map) {
				const retData = new Array<TRowType>();
         ...
         */
		public allRowsRef(): Map<TKeyType, TRowType> | TRowType[] {
			return this.rowDataCache;
		}
	}
}