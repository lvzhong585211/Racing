namespace MyUI.Role {
	 ...
	 * 技能界面
	 */
	export class SkillPart extends ui.PlayerBag.SkillPartUI implements IWindowPart {
		private m_lstSkillItem: MyUI.SkillItem[]; // 技能项列表
		private m_selectedItem: MyUI.SkillItem; // 选中的技能项
		private m_lstConfigItem: MyUI.SkillItem[]; // 配置页中的技能项列表
		private m_lstPageButton: Laya.Button[]; // 技能配置页按钮列表
		private m_nCurConfigItemIdx: number = -1; // 当前配置页正在配置的技能Item索引
		...
		public createChildren() {
			...
			// 文本及样式设置
			Style.prepareHtmlFont20Left(this._txtConfigDesc, ColorCode.normalH);
			Style.prepareHtmlFont18Left(this._txtMPConsume, ColorCode.normalH);
			Style.prepareHtmlFont18Left(this._txtCooldown, ColorCode.normalH);
			Style.prepareHtmlFont18Left(this._txtReleaseScope, ColorCode.normalH);
			Style.prepareHtmlFont18Left(this._txtSkillDesc, ColorCode.normalH);
			...
			this._txtConfigDesc.innerHTML = ConfigLoca.UI_COMMON_SkillConfigTitle; // 提示：1...，2...
			this._inputCfgName.text = Loca.getLang("11592"); // 请输入技能配置名称
			this._btnRename.label = Loca.getLang("12832"); // 命名

			// 技能Item项事件注册
			this.m_lstSkillItem.forEach(
				element => element.on(Laya.Event.CLICK, this, this._onSkillItemClick)
			);
			// 技能配置按钮选中状态改变处理
			...
			// 技能配置页按钮点击处理
			this.m_lstPageButton.forEach(
				element => element.clickHandler = new Laya.Handler(this, this._onPageButtonClick, [element.name])
			);
			// 技能配置Item项事件注册
			this.m_lstConfigItem.forEach(
				(element, index) => element.on(Laya.Event.CLICK, null, () => {
					// 1是职业基础技能，不能选中
					if (index !== 1) this.curConfigItemIdx = index;
				})
			);
			// 重置按钮点击处理
			this._btnReset.clickHandler = new Laya.Handler(null, () => {
				this.m_lstConfigItem.forEach(
					(element, index) => { // 1是职业基础技能，不需要重置
						if (index !== 1) {
		 ...
		 * @param e 
		 */
		private _onSkillItemClick(e: Laya.Event) {
			// TODO: 引导、音效
			// 增加技能配置界面显示判断，解决在选中技能情况下打开配置界面后，选中的技能无法配置成功问题
			if (this.m_selectedItem === e.currentTarget && this._groupConfig.visible === false) return;
			if (this.m_selectedItem) this.m_selectedItem.selectImgVisibility = false;
			this.m_selectedItem = e.currentTarget as SkillItem;
		 ...
		 * @param sBtnName 按钮名字
		 */
		private _onPageButtonClick(sBtnName: string) {
			this.m_lstPageButton.forEach(
				element => element.selected = element.name === sBtnName
			);
		 ...
		 */
		private _onSkillUpLevel(dtUpLevel: NetMsg.ISCSkillLevelUp) {
			...
			if (this.m_selectedItem === itmSkill) this._updateSkillDetailInfo(this.m_selectedItem);
			this._updateTxtTotalMoJing();
		 ...
		 * @param itmSkill 
		 */
		private _updateSkillDetailInfo(itmSkill: SkillItem) {
			if (!itmSkill.skillVO) return;
			// 技能图标
			...
			// 技能描述
			const nSkillID: number = itmSkill.skillID;
			let nSkillLv: number = itmSkill.skillLevel;
			...
			if (voLevel) {
				strDesc = Loca.getLang("本级效果") + "<br>" + Global.GetRealDesc(realStrDesc, voLevel.DescriptionParams);// Global.String.Format(realStrDesc, voLevel.DescriptionParams);
			}
			voLevel = Global.getMagicLevelVO(nSkillID, nTotalLv + 1);
			if (voLevel) {
				strDesc += "<br><br>" + Loca.getLang("下级效果") + "<br>" + Global.GetRealDesc(realStrDesc, voLevel.DescriptionParams);//Global.String.Format(realStrDesc, voLevel.DescriptionParams[0].split(","));
			}
			...
			if (jingshi <= 0) jingshi = 0;
			else if (jingshi < 1) jingshi = 1;
			...
			if (Global.GetRoleCommonUseParamsValue(RoleCommonUseIntParamsIndexs.TianDiJingYuan) < jingshi) {
				this._txtMoJingConsume.color = `#${ColorCode.no}`;
				this._btnOneKeyUp.disabled = true;
			} else {
				this._txtMoJingConsume.color = `#${ColorCode.value}`;
			...
			if (voLevel.NeedJinBi > Global.GetRoleOwnNumByMoneyType(MoneyTypes.TongQian)) {
				this._txtGoldConsume.color = `#${ColorCode.no}`;
			} else {
				this._txtGoldConsume.color = `#${ColorCode.value}`;
			}
			const vLevel = TableUtils.getFieldNumber(voLevel.NeedZhuanSheng);
			if (vLevel !== 0) {
				this._txtNeedLevel.text = Global.String.Format(Loca.getLang("2312"), vLevel, voLevel.NeedRoleLevel); // 最低{0}重{1}级
			} else {
				this._txtNeedLevel.text = Global.String.Format(Loca.getLang("2311"), voLevel.NeedRoleLevel); // 最低{0}级
			}
			if (Global.Data.roleData.ChangeLifeCount > vLevel
				|| (Global.Data.roleData.ChangeLifeCount === vLevel && Global.Data.roleData.Level >= voLevel.NeedRoleLevel)) {
				this._txtNeedLevel.color = `#${ColorCode.value}`;
			} else {
				this._txtNeedLevel.color = `#${ColorCode.no}`;
			}
		}
		 ...
		 * 更新所有技能Item的小红点
		 */
		private _updateAllItemRedMark() {
			this.m_lstSkillItem.forEach(
				element => this._updateOneItemRedMark(element)
			);
		}
		 ...
		 * @param itmSkill 
		 */
		private _updateOneItemRedMark(itmSkill: SkillItem) {
		 ...
		 * 获取当前角色的技能VO列表
		 */
		private _getSkillVOList(): tables.MagicInfoVO[] {
			const lstVO: tables.MagicInfoVO[] = [];
			...
			if (mapVO instanceof Map) {
				mapVO.forEach(
					value => {
						...
						if (TableUtils.getFieldNumber(value.ToOcuupation) === nOccu && TableUtils.getFieldNumber(value.ActionIndex) < 1000 && TableUtils.getFieldNumber(value.preWeakMagicId) <= 0) {
							lstVO.push(value);
						}
					}
				);
		 ...
		 * @param nSkillUsedNum 
		 */
		private _getCanLevelUp(nSkillID: number, nSkillLv: number, nSkillUsedNum: number): boolean {
			const voLevel = Global.getMagicLevelVO(nSkillID, nSkillLv);
			if (!voLevel || voLevel.Level >= 100) return false;
		 ...
		 * @param nPage 页码
		 */
		private _updateConfigInfo(nPage: number = 0) {
			...
			let itmKey: QuickKeyItem;
			let itmSkill: SkillItem;
		 ...
		 * 当前配置页正在配置的技能Item索引
		 */
		private set curConfigItemIdx(value: number) {
		 ...
		 */
		private _addSkillToConfigPage(nSkillId: number, nIndex: number): boolean {
			const nCurPage = Super.GData.m_nChoosePage;
			...
			if (nIndex >= QuickKeyItems.length) return false;
		 ...
		 */
		private _saveConfigPageSkill(bRename: boolean = false) {
			// TODO: 敏感词
			// if (bRename)
			...
			const nCurPage = this.m_lstPageButton.findIndex(
