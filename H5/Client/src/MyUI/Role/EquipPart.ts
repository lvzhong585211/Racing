namespace MyUI.Role {
	 ...
	 * 角色装备界面
	 */
	export class EquipPart extends ui.PlayerBag.EquipPartUI implements IWindowPart {
		private equipIcon: Map<number, MyUI.GoodsIcon>;
		private m_characterView: Logic.CharacterView;
		private m_uiScene: Laya.Scene;
		...
		public createChildren() {
			...
			this.equipIcon = new Map<number, MyUI.GoodsIcon>();
		...
		public destroy(destroyChild?: boolean) {
			...
			if (this.m_characterView) this.m_characterView.destroy(destroyChild);
			if (this.m_uiScene) this.m_uiScene.destroy(destroyChild);
		 ...
		 * 更新角色模型
		 */
		private async _updateCharacterView() {
		 ...
		 * @param value 
		 */
		private _onEquipLoad(dtGoods: NetMsg.IGoodsData) {
			...
			if (nCategory === ItemCategories.JieZhi) {
				...
				if (dtGoods.BagIndex === HandTypes.ZuoShou) nCategory = ItemCategories.JieZhi;
				else if (dtGoods.BagIndex === HandTypes.YouShou) nCategory = (ItemCategories.EquipMax + ItemCategories.JieZhi);
		 ...
		 * @param value 
		 */
		private _onEquipUnload(dtGoods: NetMsg.IGoodsData) {
			if (!this.visible) return; // 隐藏的时候不刷新，节省性能
		 ...
		 */
		private _getUsingGoodsDataByCategory(nCategory: number): NetMsg.IGoodsData {
