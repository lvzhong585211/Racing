...
*/
namespace MyUI {
    export class RockerView extends ui.MainUI.RockerUI {
        ...
        private onMove(e: Laya.Event): void {
            ...
            const locationPos: Laya.Point = this.globalToLocal(new Laya.Point(Laya.stage.mouseX, Laya.stage.mouseY), false);
            // 更新摇杆控制点位置
            ...
            // 计算控制点在摇杆中的角度
            const dx: number = this.deltaX * this.deltaX;
            const dy: number = this.deltaY * this.deltaY;
            ...
            if (dx + dy >= 80 * 80) {
                ...
                const x: number = Math.floor(Math.sin(this.radians) * 80 + this.originPiont.x);
                const y: number = Math.floor(Math.cos(this.radians) * 80 + this.originPiont.y);
                this.knob.pos(x, y);
            }
            else {
                // 不超过80像素取原坐标
