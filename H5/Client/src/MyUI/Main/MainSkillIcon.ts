namespace MyUI {
	 ...
	 * 主界面技能图标
	 */
	export class MainSkillIcon extends ui.MainUI.MainSkillIconUI {
		private m_compCooldown: Laya.Component; // 冷却组件

		private m_nDrawRadius: number = 0; // 冷却组件半径
		private m_dtSkill: NetMsg.ISkillData; // 技能Data
		private m_nTotalCooldown: number = 0; // 总冷却时间
		private m_nEndTicks: number = 0; // 冷却结束时间

		// 由于Layabox自己提供的EffectAnimation不是按渲染帧逻辑执行的,所以当帧数不稳定时,会出现卡顿的感觉,这里我们自己实现动画效果
		private mAniTime = 0;		// 保存动画的剩余时间(秒)
		private mAniTotalTime = 1;	// 动画的总时间(秒)
		private mAniTargetScale = 1; // 目标缩放值
		private mAniStartScale = 1;	// 起始缩放值
		private mOrgScale = 1;		// 初始值.注: 使用者可能会在界面编辑器中修改初始缩放值
		private mAniEndCallback: Function = null;	// 保存动画结束时的回调
		private mAniCalcT: Function = null;			// 计算插值T的函数
		 ...
		 * 技能ID
		 */
		public get skillID(): number { return this.m_dtSkill ? this.m_dtSkill.SkillID : 0; }

		/**
		 * 设置背景皮肤
		 */
		public set bgSkin(value: string) {
			this._imgBg.skin = Global.getMainAtlasImgPath(value);
			this.m_nDrawRadius = this._imgBg.width / 2 - 4;
			enlargeHitArea(this, this._imgBg.width, this._imgBg.height);
		}
		 ...
		 */
		public updateUI(dtSkill: NetMsg.ISkillData = null) {
			if (!dtSkill) {
				this.clearUI();
				return;
			}
		 ...
		 * 设置总冷却时间
		 */
		public set totalCooldown(value: number) {
			this.m_nTotalCooldown = value;
			this.m_nEndTicks = value + TimeManager.getCorrectLocalTime();
			if (value > 0) Laya.timer.frameLoop(1, this, this._updateCooldown);
		}
		 ...
		 * @param e 
		 */
		private _onPressHandler(e: Laya.Event) {
		 ...
		 * @param e 
		 */
		private _onRaiseHandler(e: Laya.Event) {
			// 初始化动画
