...
*/
namespace MyUI {
	export class MainFunOpenTiShi extends ui.MainUI.MainFunOpenTiShiUI {
		private funOpenId = 0;                     // 功能开启ID
		private funOpenTaskNums = 0;               // 功能开启任务数量
		private funOpenPickUpState = false;       // 功能开启奖励领取状态
		constructor() {
			super();
			this.init();
		}
		...
		private init() {
			this.visible = false;
			Style.prepareHtmlFont18Center(this._tishiName, ColorCode.normalH);
		 ...
		 * @param e 
		 */
		private onClickStage(e: laya.events.Event) {
			const fPart = windowMgr.openWindow<FunOpenTiShiPart>(WindowID.FunOpenTiShiPart);
		 ...
		 */
		updateFunOpenTiShiShow(openId: number, taskNums: number, pickUpState: boolean) {
		 ...
		 * @returns {} 
		 */
		showFunOpenTiShiObjState(isShowState: boolean) {
			this.funOpenId = -1;
			// FXGO.gameObject.SetActive(false);
			// pSystem.gameObject.SetActive(false);
