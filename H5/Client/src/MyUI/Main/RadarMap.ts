namespace MyUI {
	 ...
	 * 主界面雷达图
	 */
	export class RadarMap extends ui.MainUI.RadarMapUI {
		private m_cacheLeader: Logic.Player; // 缓存一个主角的引用方便使用
		private m_cacheLevel: Logic.Level; // 缓存一个关卡的引用方便使用
		private m_fScaleX: number; // 实际地图和小地图的横向比例大小
		private m_fScaleY: number; // 实际地图和小地图的纵向比例大小
		private m_bakCenterPos = new Laya.Point(); // 地图圆框中心点位置（主角需要始终定位于此）

		private m_lastLeaderPos = new Laya.Point(); // 上次设置的主角位置
		private m_currLeaderPos = new Laya.Point(); // 主角当前所处的位置
		private m_vec3Rotation = new Laya.Vector3(); // 角色旋转四元数输出向量值
		private m_currOtherPos = new Laya.Point(); // 其他对象当前所处的位置
		private m_nUpdateAccuTime = 0; // 刷新累计时间

		private m_mapPoints = new Map<number, MapPoint>(); // 雷达图上对象点的列表
		private m_lstPointsPool = new Array<MapPoint>(); // 回收暂时不用的对象点
		private m_lstWaitingDels = new Array<MapPoint>(); // 待删除的对象点列表
		 ...
		 * @param nLevelID 关卡ID
		 */
		public updateUI(nLevelID: number) {
		 ...
		 */
		private deleteActors(aActors: Logic.AActor[]) {
			aActors.forEach(
				actor => {
		 ...
		 * @param roleID 角色ID
		 */
		private addPoint(actor: Logic.AActor, roleID: number): void {
			if (Logic.isViewActor(actor)) {
		 ...
		 */
		private deletePoint(nRoleID: number) {
