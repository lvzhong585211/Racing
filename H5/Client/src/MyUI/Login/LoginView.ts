namespace MyUI {
    ...
    export class LoginView extends ui.login.LoginViewUI {
        private gameServerList: GameServerInfo[] = null;   // GameServer的列表信息
        ...
        constructor() {
            ...
            this.btnLogin.clickHandler = new OpIntervalHandler(1000, this, this.OnLoginBtnClick);
            this.btnSpeedy.clickHandler = new OpIntervalHandler(1000, this, this.OnSpeedyBtnClick);
            this.btnRegister.clickHandler = new OpIntervalHandler(1000, this, this.OnRegisterBtnClick);
        }

        /**
         * 获取GameServer的列表
         */
        private QueryGameServerList() {
            ...
            xhr.once(Laya.Event.COMPLETE, this, (data) => {
                ...
                let selectedServer: GameServerInfo;
         ...
         */
        private getGameServerUrl(): string {
         ...
         */
        private async LoginServerNow(account: string, pwd: string) {
            // 注意: 下面的代码实现是异步执行的!!!
            ...
            try {
                ...
                const connectResult = await socketToLoginServer.connectByUrl(GameConfig.LoginServerUrl);
                if (!connectResult) {
                    // 显示连接LoginServer失败
                ...
                // 等待返回消息
                const retData = await socketToLoginServer.WaitStringMessage(msgType, 3);
                ...
                if (retData != null) {
                    ...
                    if (err) {  // 有错误,输出提示
                        ...
                        const loginRet = await gameIns.loginToGameServer(gameServerUrl, userId, userName, userToken, (errInfo) => {
