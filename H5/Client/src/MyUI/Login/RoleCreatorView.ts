namespace MyUI {
    ...
    import String = Global.String;

	/**
	 * 创建角色界面
	 */
    export class RoleCreatorView extends ui.login.RoleCreatorUI {
        private m_itmSelected: RoleCreatorItem;     // 当前选择的项
        private m_nLastClickTime: number;           // 上次点击时间
        private m_lsSurname: string[]; // 姓列表
        private m_lsMaleName: string[]; // 男名列表
        private m_lsFemaleName: string[]; // 女名列表

        public readonly uiEventActorChanged = "eventActorChanged";  // 当选中的角色改变时,通过 event 发送
        public readonly uiEventReturn = "eventReturn";              // 当点击返回按钮时,通过 event 发送

        constructor(preMode: EnumGameMode) {
            super();
         ...
         * @param e 
         */
        private onItemOccupationClick(e: Event) {
            if (!e || !(e.target instanceof RoleCreatorItem)) {
                return;
            }
            ...
            this.m_itmSelected = <RoleCreatorItem>e.target;
            this.m_itmSelected.setSelected(true);

            // 更新职业描述文本显示
            const nOccuID: number = this.m_itmSelected.getOccupation();
        ...
        private onRandomButtonClick() {
            ...
            if (!this.m_lsSurname) {
                ...
                lsNodes.forEach(element => {
                    if (element.xing) this.m_lsSurname.push(element.xing);
                    if (element.nan) this.m_lsMaleName.push(element.nan);
                    if (element.nv) this.m_lsFemaleName.push(element.nv);
                });
            }
