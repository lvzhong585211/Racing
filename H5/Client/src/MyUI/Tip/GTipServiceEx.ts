namespace MyUI {
	export class GTipServiceEx {
		/** Tip对应的Icon */
		public static TipsSender: GoodsIcon = null;
		/** 如果窗口打开的是纯背包，则此属性为true，如果包含其它的部分，如：交易、仓库等，则为false */
		public static SelfBagOnly: boolean = true;
		/** 佩戴后属性的装备，是左手、右手，此值为一个key */
		public static HandKey: number = ExtPropIndexes.Max + 3;
		/** 哪个手佩戴 */
		public static HandValue: number = -1;

		/** 货币单位图片名称 */
		public static PriceIconUnits: string[] = [
			"money_gold",
			"money_jifen",
			"money_diamond",
		...
		private static GoodTips: GoodsTip = null; // 物品类道具Tip
		private static EquipTips: EquipTip = null; // 装备类道具Tip
		private static _GoodData: NetMsg.IGoodsData = null; // 临时GoodsData
		private static _goodsOwner: GoodsOwnerTypes = GoodsOwnerTypes.None;
		private static IsWaitting = false; // 是否是等待显示（向服务器请求数据中...）
		 ...
		 */
		public static GetPriceInfoUnit(nPriceTypeIdex: number): string {
		 ...
		 * @param price 购买一个的价格
		 */
		public static GetMaxBuyNum(goodsPriceUnit: number, price: number): number {
			let money = 0;
			if (goodsPriceUnit === <number>GoodsPriceUnitTypes.Jinbi) {
				money = Global.Data.roleData.YinLiang;
			} else if (goodsPriceUnit === <number>GoodsPriceUnitTypes.Jifen) {
				money = Global.GetRoleOwnNumByMoneyType(MoneyTypes.JingYuanZhi);
			} else if (goodsPriceUnit === <number>GoodsPriceUnitTypes.Zhuanshi) {
				money = Global.Data.roleData.UserMoney;
			} else if (goodsPriceUnit === <number>GoodsPriceUnitTypes.BindZhuanshi) {
				money = Global.Data.roleData.Gold;
			} else if (goodsPriceUnit === <number>GoodsPriceUnitTypes.BindJinBi) {
				money = Global.Data.roleData.Money1;
			} else if (goodsPriceUnit === <number>GoodsPriceUnitTypes.KingOfBattlePoint) {
		 ...
		 * @param goodData 
		 */
		public static ShowTip(sender: GoodsIcon, tipType: TipTypes, goodsOwner: GoodsOwnerTypes, goodData: NetMsg.IGoodsData) {
			if (GTipServiceEx.IsWaitting) return;
			if (!goodData) return;
		 ...
		 */
		static ShowMallTip(sender: GoodsIcon, tipType: TipTypes, goodsOwner: GoodsOwnerTypes, goodsPriceUnit: GoodsPriceUnitTypes, price: number, goodData: NetMsg.IGoodsData) {
			if (GTipServiceEx.IsWaitting) return;
			if (!goodData) return;
		 ...
		 * @param priceInfo 
		 */
		private static ShowGoodsTipWindow(goodsOwner: GoodsOwnerTypes, GoodData: NetMsg.IGoodsData, priceInfo: string = null) {
			GTipServiceEx._GoodData = GoodData;
			GTipServiceEx.GoodTips = windowMgr.openWindow<GoodsTip>(WindowID.GoodsTip);
		 ...
		 * @param priceInfo 
		 */
		private static ShowEquipTipWindow(goodsOwner: GoodsOwnerTypes, GoodData: NetMsg.IGoodsData, priceInfo: string = null) {
			...
			GTipServiceEx.EquipTips = windowMgr.openWindow<EquipTip>(WindowID.EquipTip);
			GTipServiceEx.EquipTips.DPSelectedItem = (s, e) => {
				if (e.IDType === <number>TipsOperationTypes.Close) {
					windowMgr.closeWindow(WindowID.EquipTip); // 关闭装备类道具Tip窗口
				} else if (e.IDType === <number>TipsOperationTypes.SwitchHand) {
					...
					GTipServiceEx.ShowTipsResultWindow(<number>ItemCategories.Fashion !== Global.GetCategoriyByGoodsID(GoodData.GoodsID), tipsResultDict, GTipServiceEx.HandValue);
				} else if (e.IDType === <number>TipsOperationTypes.ShangJia) {
					// TODO:
					// PlayZone.GlobalPlayZone.OpenWuPinShangJiaWindow(_GoodData, 0);
				}
			};
		 ...
		 * @param goodsID 
		 */
		private static IsShowTipsResultWindow(resultDict: Map<number, number>, goodsOwner: GoodsOwnerTypes, goodsID: number): boolean {
			if (!resultDict) {
				return false;
			}
			...
			if (!resultDict.has(<number>ExtPropIndexes.AttackSpeed)) {
				return false;
			}
			...
			if ((<number>ItemCategories.ShouHuChong === goodVO.Categoriy) || (<number>ItemCategories.ChongWu === goodVO.Categoriy) || (<number>ItemCategories.Fashion === goodVO.Categoriy)) {
				return false;
			}
		 ...
		 * @param nHandValue 
		 */
		private static ShowTipsResultWindow(tipsResultState: boolean, tipsResultDict: Map<number, number>, nHandValue: number) {
			// TODO: Tip对比窗口
			// int tipsPosx = 115;
		...
		public static ClearChildWindow() {
