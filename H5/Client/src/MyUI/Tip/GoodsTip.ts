namespace MyUI {
	 ...
	 * 物品类道具Tip
	 */
	export class GoodsTip extends ui.tip.GoodsTipUI {
		/** 最后一次点击的物品ID，方便快速获取道具 */
		public static LastGoodsID: number;

		/** UI操作执行回调函数 */
		public DPSelectedItem: DPSelectedItemEventHandler;

		private PriceTypeIndex: number = 0; // 价格类型
		private BuyOnePrice: number = 0; // 物品单价
		private BuyNum: number = 0; // 数量
		private MaxBuyNum: number = 0; // 当前价格类型可以购买的最大数量
		private CaifenNum: number = 0; // 拆分数量
		private MaxCaifenNum: number = 0; // 当前物品可拆分的最大数量，总数-1
		private goodsID: number = 0; // 当前物品的GoodsID
		private _goodsData: NetMsg.IGoodsData = null; // 当前物品的数据
		private _goodsOwner: GoodsOwnerTypes = GoodsOwnerTypes.None; // 物品所属类型
		private m_nEndTime: number = 0; // 道具有效结束时间
		...
		constructor() {
			...
			Style.prepareHtmlFont18Left(this._txtType, ColorCode.normalH);
			Style.prepareHtmlFont18Left(this._txtLevel, ColorCode.normalH);
			Style.prepareHtmlFont18Left(this._txtOccu, ColorCode.normalH);
			Style.prepareHtmlFont18Left(this._txtValidTime, ColorCode.normalH);
			Style.prepareHtmlFont18Left(this._txtPrice, ColorCode.normalH);
			Style.prepareHtmlFont18Left(this._txtDesc, ColorCode.normalH);
			Style.prepareHtmlFont18Right(this._txtCount, ColorCode.normalH);
				...
				if (this._btnBuy.name === TipsOperationTypes.Goumai.toString() || this._btnBuy.name === TipsOperationTypes.JiaoYiShuoGouMai.toString()) {
					...
					if (num > this.MaxBuyNum) {
						if (this.PriceTypeIndex === GoodsPriceUnitTypes.Jinbi || this.PriceTypeIndex === GoodsPriceUnitTypes.BindJinBi) {
							...
							// 点击花费金币执行请求
							const costJinBihandler: DPSelectedItemEventHandler = (sender, args) => {
								if (0 === args.ID) { // 确定
							...
							uiMgr.hintText(Global.String.Format(Loca.getLang("{0}不足"), GTipServiceEx.GetPriceInfoUnit(this.PriceTypeIndex)));
						}
					} else if (num > 0) {
						if (this._btnBuy.name === TipsOperationTypes.JiaoYiShuoGouMai.toString()) {
		 ...
		 * @param priceInfo 
		 */
		public RenderTips(goodsOwner: GoodsOwnerTypes, goodsData: NetMsg.IGoodsData, selfBagOnly: boolean, priceInfo: string = null) {
		 ...
		 * @param priceInfo 
		 */
		private SetText(goodsOwner: GoodsOwnerTypes, goodsData: NetMsg.IGoodsData, priceInfo: string = null) {
			...
			//#endregion
			//#region ======== 道具类型 ========
			this._txtType.innerHTML = Global.GetColorStringForNGUIText(ColorCode.normal, ConfigLoca.UI_GoodsTip_Title_Type, ColorCode.value, Global.GetGoodsType(voGoods.Categoriy));
			//#endregion
			//#region ======== 使用等级 ========
			...
			sColor = ColorCode.value;
			if (!Global.validLevel(nNeedLifeCnt, nNeedLv)) {
				sColor = ColorCode.no;
			}
			this._txtLevel.innerHTML = Global.GetColorStringForNGUIText(ColorCode.normal, ConfigLoca.UI_GoodsTip_Title_Level, sColor, sLvCondi);
			//#endregion
			//#region ======== 适用职业 ========
			sColor = ColorCode.value;
			if (voGoods.ToOccupation >= 0) {
				...
				this._txtOccu.innerHTML = Global.GetColorStringForNGUIText(ColorCode.normal, ConfigLoca.UI_GoodsTip_Title_Occupation, sColor, OccupationStr);
			} else {
				this._txtOccu.innerHTML = Global.GetColorStringForNGUIText(ColorCode.normal, ConfigLoca.UI_GoodsTip_Title_Occupation, sColor, ConfigLoca.UI_OCCUPATION_COMMON_USE);
			}
			//#endregion
			//#region ======== 道具描述 ========
			...
			if (goodsData.Endtime && goodsData.Endtime !== Global.ConstGoodsEndTime) {
				...
				this._txtValidTime.innerHTML = Global.GetColorStringForNGUIText(ColorCode.normal, desc_str, ColorCode.green, time_str);
			...
			if (voGoods.UsingNum > 1) {
				this._txtCount.innerHTML = Global.GetColorStringForNGUIText(ColorCode.normal, Loca.getLang("剩余次数:"), ColorCode.value, nGoodsCnt);
			} else {
				if (nGoodsCnt <= 0) nGoodsCnt = 1; // 既然有物品Tips，且显示数量文本,则该物品必然有1个，这里强制将物品数量为0且显示物品数量文本的Tips的物品数量修正为1最小值
				this._txtCount.innerHTML = Global.GetColorStringForNGUIText(ColorCode.normal, Loca.getLang("数量:"), ColorCode.value, nGoodsCnt);
			}
			//#endregion
			//#region ======== 道具价格 ========
			...
			if (goodsOwner === GoodsOwnerTypes.mallSale || goodsOwner === GoodsOwnerTypes.NPCSale
				...
				if (priceInfo) {
					...
					const priceInfoUnit = GTipServiceEx.GetPriceInfoUnit(this.PriceTypeIndex);
		...
		private SetMenus(goodsOwner: GoodsOwnerTypes, goodsData: NetMsg.IGoodsData, selfBagOnly: boolean) {
			...
			if (goodsOwner === GoodsOwnerTypes.SelfBag) {
				...
				if (Global.g_bIsTipsShowCuiHuiBtn && ItemCategories.ItemMengchongEgg !== goodVO.Categoriy) {
					showNodeWithLayout(this._btnDestroy);
				}
			} else if (goodsOwner === GoodsOwnerTypes.JiaoYiShuo) {
				// TODO: 交易Tip显示
			...
			} else if (goodsOwner === GoodsOwnerTypes.Exchange || goodsOwner === GoodsOwnerTypes.SelfPet || goodsOwner === <number>GoodsOwnerTypes.SelfPet) {
		...
		private SetPropsPanel(goodsOwner: GoodsOwnerTypes, goodsData: NetMsg.IGoodsData) {
			...
			if (goodsOwner === GoodsOwnerTypes.NPCSale || goodsOwner === GoodsOwnerTypes.mallSale
				...
				showNodeWithLayout(this._groupSplit);
			} else {
				if (goodsOwner === GoodsOwnerTypes.SelfBag && goodsData.GCount > 1
					...
					hideNodeWithLayout(this._groupSplit);
				}
			}
			// 设置按钮个数及按钮宽度
			...
			// 设置底板背景的高度
			this.callLater(
				() => this._contentBg.height = this._groupContent.height + 10
			);
		}
		 ...
		 * 刷新价格
		 */
		private RefreshPrice(tempBuyNum: number = -1) {
			if (tempBuyNum === -1) tempBuyNum = this.BuyNum;
			const price = this.BuyOnePrice * tempBuyNum;
			const fontColor = (tempBuyNum <= this.MaxBuyNum) ? ColorCode.value : ColorCode.no;
			this._txtPrice.innerHTML = Global.GetColorStringForNGUIText(ColorCode.normal, Loca.getLang("总价: "), fontColor, price);
			// 价格大于0时，算一下钻石图标的位置
			if (price > 0)
				this._imgMoney.x = this._txtPrice.x + this._txtPrice.contextWidth + 3;
			this._imgMoney.skin = Global.getCommonAtlasImgPath(GTipServiceEx.PriceIconUnits[this.PriceTypeIndex]);
		 ...
		 * @param id 
		 */
		private TipsCallBack(type: TipsOperationTypes, id: number = 0) {
			this.DPSelectedItem(this, new DPSelectedItemEventArgs(TipsOperationTypes.Close, 0));
			if (type > <number>TipsOperationTypes.Close && type < (<number>TipsOperationTypes.Max) || type === <number>TipsOperationTypes.UseMengchong) {
				if (null != GTipServiceEx.TipsSender && null != GTipServiceEx.TipsSender.DPSelectedItem) {
					GTipServiceEx.TipsSender.DPSelectedItem(this, new DPSelectedItemEventArgs(type, id));
				}
			}
			if (<number>TipsOperationTypes.ShangJia === type) {
