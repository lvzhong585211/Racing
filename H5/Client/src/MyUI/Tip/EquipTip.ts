namespace MyUI {
	 ...
	 * 装备类道具Tip
	 */
	export class EquipTip extends ui.tip.EquipTipUI {
		/** UI操作执行回调函数 */
		public DPSelectedItem: DPSelectedItemEventHandler;

		private BuyNum = 0; // 数量
		private BuyOnePrice = 0; // 物品单价
		private PriceTypeIndex = 0; // 价格类型
		private MaxBuyNum = 0; // 当前价格类型可以购买的最大数量
		private m_nDBID = 0; // 当前Tip的道具DBID

		private ChineseNum: string[] = [Loca.getLang("零"), Loca.getLang("一"), Loca.getLang("二"), Loca.getLang("三"), Loca.getLang("四"),
		Loca.getLang("五"), Loca.getLang("六"), Loca.getLang("七"), Loca.getLang("八"), Loca.getLang("九"), Loca.getLang("十")]; // 中文数字
		private zuoyueSpriteNames: string[] = ["tip_chuanshuo_lv", "tip_chuanshuo_lan", "tip_chuanshuo_zi"]; // 卓越属性图片名称
		...
		constructor() {
			...
			Style.prepareHtmlFont22Center(this._txtTitle, ColorCode.normalH);
			Style.prepareHtmlFont18Right(this._txtOccu, ColorCode.normalH);
			Style.prepareHtmlFont18Left(this._txtLevel, ColorCode.normalH);
			Style.prepareHtmlFont18Left(this._txtCondition0, ColorCode.normalH);
			Style.prepareHtmlFont18Left(this._txtCondition1, ColorCode.normalH);
			Style.prepareHtmlFont18Left(this._txtCondition2, ColorCode.normalH);
			Style.prepareHtmlFont18Left(this._txtCondition3, ColorCode.normalH);
			Style.prepareHtmlFont18Left(this._txtBasic, ColorCode.normalH);
			Style.prepareHtmlFont18Left(this._txtAddition, "#bbad6a");
			Style.prepareHtmlFont18Left(this._txtRefined, ColorCode.normalH);
			Style.prepareHtmlFont18Left(this._txtRandom, ColorCode.zhuoYue1H);
			Style.prepareHtmlFont18Left(this._txtLucky, `#8a85de`);
			Style.prepareHtmlFont18Left(this._txtRebirth, ColorCode.normalH);
			Style.prepareHtmlFont18Left(this._txtPrice, ColorCode.normalH);
			Style.prepareHtmlFont18Right(this._txtDurability, ColorCode.normalH);
			...
			this._btnWear.clickHandler = new Handler(null, () => {
				if (!GTipServiceEx.TipsSender) return;
				const dtGoods = GTipServiceEx.TipsSender.goodsData;
				if (!dtGoods) return;
			...
			this._btnProcess.clickHandler = new Handler(null, () => {
				if (!GongnengYugaoMgr.HintGongNengOpened(GongNengIDs.LianLu)) {
					return;
				}
			...
			this._btnSale.clickHandler = new Handler(null, () => {
				if (this._btnSale.name === "") {
					...
					if (gd) {
						...
						if (!(GongnengYugaoMgr.IsGongNengOpened(GongNengIDs.ZaiZao).isOpen)) {
							if (goodVO.SuitID >= Global.ShenqiZaizaoSuit) {
								const sName = GongnengYugaoMgr.GetGongNengName(GongNengIDs.ZaiZao);
		 ...
		 * @param handTypes 
		 */
		public RenderTips(goodsOwner: GoodsOwnerTypes, goodsData: NetMsg.IGoodsData, selfBagOnly: boolean, priceInfo: string = null, handTypes: number = -1) {
			this.m_nDBID = goodsData.Id;
			GoodsTip.LastGoodsID = goodsData.GoodsID;
		 ...
		 * @param handTypes 
		 */
		private SetText(goodsOwner: GoodsOwnerTypes, goodsData: NetMsg.IGoodsData, priceInfo: string = null, handTypes: number = -1) {
			const goodVO = tableMgr.goodsTable.Find(goodsData.GoodsID);
			let fontColor = ColorCode.zhuoYue0;
			let nPosY = 0;
			//#region ======== 装备名字 ========
			let str0 = "";
			let str1 = "";
			...
			if ((zhuoyueNum > 0) && (zhuoyueNum <= 2)) {
				fontColor = ColorCode.zhuoYue1;
				str0 += Loca.getLang(UIHelper.ZuoyueTitleNames[0]);
			} else if ((zhuoyueNum >= 3) && (zhuoyueNum <= 4)) {
				fontColor = ColorCode.zhuoYue2;
				str0 += Loca.getLang(UIHelper.ZuoyueTitleNames[1]);
			} else if ((zhuoyueNum >= 5) && (zhuoyueNum <= 6)) {
				fontColor = ColorCode.zhuoYue3;
				str0 += Loca.getLang(UIHelper.ZuoyueTitleNames[2]);
			}
			str0 += Loca.getLang(goodVO.Title);
			if ((<number>ItemCategories.ShouHuChong === goodVO.Categoriy) || (<number>ItemCategories.ChongWu === goodVO.Categoriy) || (<number>ItemCategories.MengChongWu === goodVO.Categoriy)) {
				fontColor = goodVO.GoodsColor;
				str0 = Loca.getLang(goodVO.Title);
			}
			str0 = Global.GetColorStringForNGUIText(fontColor, str0);
			if ((<number>ItemCategories.ShouHuChong !== goodVO.Categoriy) && (<number>ItemCategories.ChongWu !== goodVO.Categoriy) && (<number>ItemCategories.MengChongWu !== goodVO.Categoriy)
				&& (<number>ItemCategories.Decoration !== goodVO.Categoriy) && (<number>ItemCategories.Fashion !== goodVO.Categoriy)) {
			...
			if (goodVO.Categoriy === <number>ItemCategories.JieHunJieZhi) {
			...
			if (toZhuansheng > 0) {
				this._txtLevel.innerHTML = Global.GetColorStringForNGUIText(ColorCode.normal, Loca.getLang("所需等级") + ": ", fontColor, Global.String.Format(Loca.getLang("{0}[{1}重]"), level, toZhuansheng));
			} else {
				this._txtLevel.innerHTML = Global.GetColorStringForNGUIText(ColorCode.normal, Loca.getLang("所需等级") + ": ", fontColor, Math.max(level, 1));
			}
			//#endregion
			//#region ======== 所需条件 ========
			const aTxtCond: HTMLDivElement[] = [
				this._txtCondition0,
				this._txtCondition1,
				this._txtCondition2,
				this._txtCondition3
			...
			if (goodVO.Strength > 0) {
				...
				if (strength < goodVO.Strength) {
					fontColor = ColorCode.no;
					str0 = Global.GetColorStringForNGUIText(ColorCode.normal, Loca.getLang("所需力量:") + " ", fontColor, `${strength}/${goodVO.Strength}`);
				} else {
					str0 = Global.GetColorStringForNGUIText(ColorCode.normal, Loca.getLang("所需力量:") + " ", fontColor, `${goodVO.Strength}`);
				}
				showNodeWithLayout(aTxtCond[nTxtIdx]);
			...
			if (goodVO.Dexterity > 0) {
				...
				if (dexterity < goodVO.Dexterity) {
					fontColor = ColorCode.no;
					str0 = Global.GetColorStringForNGUIText(ColorCode.normal, Loca.getLang("所需敏捷:") + " ", fontColor, `${dexterity}/${goodVO.Dexterity}`);
				} else {
					str0 = Global.GetColorStringForNGUIText(ColorCode.normal, Loca.getLang("所需敏捷:") + " ", fontColor, `${goodVO.Dexterity}`);
				}
				showNodeWithLayout(aTxtCond[nTxtIdx]);
				aTxtCond[nTxtIdx++].innerHTML = str0;
			}
			if (goodVO.Intelligence > 0) {
				...
				if (intelligence < goodVO.Intelligence) {
					fontColor = ColorCode.no;
					str0 = Global.GetColorStringForNGUIText(ColorCode.normal, Loca.getLang("所需智力:") + " ", fontColor, `${intelligence}/${goodVO.Intelligence}`);
				} else {
					str0 = Global.GetColorStringForNGUIText(ColorCode.normal, Loca.getLang("所需智力:") + " ", fontColor, `${goodVO.Intelligence}`);
				}
				showNodeWithLayout(aTxtCond[nTxtIdx]);
				aTxtCond[nTxtIdx++].innerHTML = str0;
			}
			if (goodVO.Constitution > 0) {
				...
				if (constitution < goodVO.Constitution) {
					fontColor = ColorCode.no;
					str0 = Global.GetColorStringForNGUIText(ColorCode.normal, Loca.getLang("所需体力:") + " ", fontColor, `${constitution}/${goodVO.Constitution}`);
				} else {
					str0 = Global.GetColorStringForNGUIText(ColorCode.normal, Loca.getLang("所需体力:") + " ", fontColor, `${goodVO.Constitution}`);
				}
				showNodeWithLayout(aTxtCond[nTxtIdx]);
				aTxtCond[nTxtIdx++].innerHTML = str0;
			}
			for (; nTxtIdx < aTxtCond.length; nTxtIdx++) {
				hideNodeWithLayout(aTxtCond[nTxtIdx]);
			}
			//#endregion
			//#region ======== 装备属性 ========
			const equipFields_1 = Global.GetGoodsEquipPropsDoubleList(goodsData.GoodsID);
			//#region 结婚戒指Tip做特殊处理
			if (categoriy === <number>ItemCategories.JieHunJieZhi) {
				if (goodsData.ForgeLevel <= 0) {
				...
				hideNodeWithLayout(this._groupAddition);
			}
			//#endregion
			//#region 基础属性
			if (categoriy === <number>ItemCategories.MengChongWu) {
				hideNodeWithLayout(this._groupBasic);
			} else if (categoriy === <number>ItemCategories.ShouHuChong || categoriy === <number>ItemCategories.ChongWu) {
				const aAttr = Global.GetPetAttribute(goodsData);
				const curAttr = aAttr[1];
				if (!curAttr || curAttr.length === 0) {
					hideNodeWithLayout(this._groupBasic);
				} else {
					showNodeWithLayout(this._groupBasic);
					this._txtBasic.innerHTML = curAttr.join("<br>");
				}
				// 不能强化的装备不显示星星进度条
				hideNodeWithLayout(this._groupStar0);
				hideNodeWithLayout(this._groupStar1);
			} else if (categoriy === <number>ItemCategories.Fashion) {
				hideNodeWithLayout(this._groupBasic);
				hideNodeWithLayout(this._groupLucky);
			} else {
				showNodeWithLayout(this._groupBasic);
			...
			if (this._groupBasic.visible) {
				nPosY = 96; // 文本默认位置
				if (!this._groupStar1.visible) nPosY = this._groupStar1.y;
				if (!this._groupStar0.visible) nPosY = this._groupStar0.y;
				this._txtBasic.y = nPosY;
			}
			//#endregion
			//#region 附加属性
			if (categoriy === <number>ItemCategories.HuFu || categoriy === <number>ItemCategories.ShouHuChong
				|| categoriy === <number>ItemCategories.ChongWu || categoriy === <number>ItemCategories.JieHunJieZhi
				|| categoriy === <number>ItemCategories.Decoration || categoriy === <number>ItemCategories.Fashion
				|| categoriy === <number>ItemCategories.MengChongWu) {
				hideNodeWithLayout(this._groupAddition);
			} else {
				...
				if (Global.String.IsNullOrWhiteSpace(str0)) {
					...
					showNodeWithLayout(this._groupAddition);
			...
			if (Global.String.IsNullOrWhiteSpace(str0)) {
				hideNodeWithLayout(this._groupRefined);
			} else {
				// this._txtRefined.color = "#" + Global.GetColorByGoodsData(goodsData).toString(16);
				showNodeWithLayout(this._groupRefined);
			}
			//#endregion
			//#region 随机属性
			...
			if (!Global.String.IsNullOrWhiteSpace(str0)) {
				if (this._imgZhuoYue) {
					if ((categoriy === <number>ItemCategories.ShouHuChong) || (categoriy === <number>ItemCategories.ChongWu)) {
				...
				showNodeWithLayout(this._groupRandom);
			} else {
				if (this._imgZhuoYue) {
					this._imgZhuoYue.visible = false;
				}
				hideNodeWithLayout(this._groupRandom);
			}
			//#endregion
			//#region 幸运属性
			...
			if (Global.String.IsNullOrEmpty(str0)) {
				...
				if (categoriy === <number>ItemCategories.Fashion) {
					hideNodeWithLayout(this._groupLucky);
				} else {
					showNodeWithLayout(this._groupLucky);
				}
			}
			//#endregion
			//#region 重生属性
			...
			if (Global.String.IsNullOrEmpty(str0)) {
				hideNodeWithLayout(this._groupRebirth);
			} else {
				showNodeWithLayout(this._groupRebirth);
			}
			//#endregion
			//#endregion
			//#region ======== 装备耐久 ========
			...
			if (goodsData.Strong >= Math.floor(equipFields_1[0])) {
				hintStrongText = Global.GetColorStringForNGUIText(ColorCode.no, Loca.getLang("(已破损，请修理)"));
			} else {
			...
			this._txtDurability.innerHTML = Global.GetColorStringForNGUIText(ColorCode.normal, Loca.getLang("耐久: "), ColorCode.value, hintStrongText);
			//#endregion
			//#region ======== 物品出售价格 ========
			if (goodsOwner === <number>GoodsOwnerTypes.mallSale || goodsOwner === <number>GoodsOwnerTypes.NPCSale
				|| goodsOwner === <number>GoodsOwnerTypes.OtherOnSale || goodsOwner === <number>GoodsOwnerTypes.JiaoYiShuo) {
				// TODO: 交易的显示以后处理
				// if (priceInfo) {
				...
				if (price > 0) {
					this._txtPrice.innerHTML = Global.GetColorStringForNGUIText(ColorCode.normal, Loca.getLang("售价: "), ColorCode.value, price);
					if (isMojing) {
						if (categoriy === <number>ItemCategories.ShouHuChong || categoriy === <number>ItemCategories.ChongWu) {
			...
			} else if (goodsOwner === <number>GoodsOwnerTypes.SysGifts || goodsOwner === <number>GoodsOwnerTypes.SelfStall || goodsOwner === <number>GoodsOwnerTypes.None) {
				this._txtPrice.innerHTML = "";
				this._imgMoney.skin = null;
			}
			...
			if (categoriy === <number>ItemCategories.MengChongWu) {
				...
				this._txtPrice.innerHTML = Global.GetColorStringForNGUIText(ColorCode.no, Loca.getLang("不可出售"));
				this._imgMoney.skin = null;
			} else if (categoriy === <number>ItemCategories.JieHunJieZhi) {
				this._txtDurability.innerHTML = "";
			} else if (categoriy === <number>ItemCategories.Fashion) {
				this._txtPrice.innerHTML = Global.GetColorStringForNGUIText(ColorCode.no, Loca.getLang("不可出售"));
				this._imgMoney.skin = null;
		 ...
		 * @param selfBagOnly 
		 */
		private SetMenus(goodsOwner: GoodsOwnerTypes, goodsData: NetMsg.IGoodsData, selfBagOnly: boolean) {
			// 先隐藏menu中所有的按钮
			setActiveChildren(this._groupButton, false);
			if (!goodsData) return;
			const goodVO = tableMgr.goodsTable.Find(goodsData.GoodsID);
			if (!goodVO || goodVO.Categoriy === ItemCategories.JieHunJieZhi) return;
			...
			if (goodsOwner === GoodsOwnerTypes.SelfBag) {
				if (goodsData.Using === 1) {
					if (goodVO.Categoriy === ItemCategories.ShouHuChong || goodVO.Categoriy === ItemCategories.ChongWu) {
						...
						showNodeWithLayout(this._btnScan);
					}
					// 佩戴在身上的装备只显示卸下与加工
					toggleNodeWithLayout(this._btnTakeOff, (goodVO.Categoriy < ItemCategories.WuQi_Jian || goodVO.Categoriy > ItemCategories.WuQi_NuJianTong));
					toggleNodeWithLayout(this._btnProcess, (goodVO.Categoriy !== ItemCategories.MengChongWu && goodVO.Categoriy !== ItemCategories.ShouHuChong
				...
				if (!selfBagOnly) { // 出售、仓库、交易时，在背包中只有：放入，出售 ,上架
					showNodeWithLayout(this._btnPutIn);
					showNodeWithLayout(this._btnSale);
					if (goodsOwner === <number>GoodsOwnerTypes.SelfStall) {
						hideNodeWithLayout(this._btnPutIn);
						hideNodeWithLayout(this._btnSale);
						showNodeWithLayout(this._btnUpShelf);
					}
				} else { // 纯背包中只显示：佩戴、加工、出售
					if (goodVO.Categoriy === ItemCategories.ShouHuChong || goodVO.Categoriy === ItemCategories.ChongWu) {
						this._btnUse.label = Loca.getLang("列队");
						this._btnUse.clickHandler = new Handler(null, () => { // 使用精灵 把精灵放入备战背包中
							if (!GongnengYugaoMgr.HintGongNengOpened(GongNengIDs.JingLingSystem)) {
								return;
							}
							this.TipsCallBack(TipsOperationTypes.UsePet);
						});
						showNodeWithLayout(this._btnUse);
					...
					toggleNodeWithLayout(this._btnWear, ShowPeiDai);
					toggleNodeWithLayout(this._btnProcess, (goodVO.Categoriy !== ItemCategories.MengChongWu && goodVO.Categoriy !== ItemCategories.ShouHuChong
						&& goodVO.Categoriy !== ItemCategories.HuFu && goodVO.Categoriy !== ItemCategories.ChongWu
						&& goodVO.Categoriy !== ItemCategories.Decoration && goodVO.Categoriy !== ItemCategories.Fashion));
					toggleNodeWithLayout(this._btnSale, (goodVO.Categoriy !== ItemCategories.MengChongWu && goodVO.Categoriy !== ItemCategories.Decoration
						&& goodVO.Categoriy !== ItemCategories.Fashion));
					toggleNodeWithLayout(this._btnDestroy, Global.g_bIsTipsShowCuiHuiBtn);
				}
			}
			if (goodsOwner === GoodsOwnerTypes.JiaoYiShuo) {
				showNodeWithLayout(this._btnBuy);
			} else if (goodsOwner === <number>GoodsOwnerTypes.Exchange || goodsOwner === <number>GoodsOwnerTypes.SelfPet
				|| goodsOwner === <number>GoodsOwnerTypes.SelfPet) { // 出售、仓库、交易窗口中的物品点击时，只显示：取回
				showNodeWithLayout(this._btnGetBack);
		 ...
		 * @param goodsData 
		 */
		private SetPropsPanel(goodsOwner: GoodsOwnerTypes, goodsData: NetMsg.IGoodsData) {
			// 设置按钮个数及按钮宽度
			const aBtns: laya.display.Sprite[] = [];
			const nCnt = this._groupButton.numChildren;
			for (let nIdx = 0; nIdx < nCnt; nIdx++) {
				const child = this._groupButton.getChildAt(nIdx) as laya.display.Sprite;
				if (child.visible) {
					aBtns.push(child);
				}
			}
		 ...
		 * @param categoriy 分类
		 */
		private GetBaseAttributeStr(gd: NetMsg.IGoodsData, equipFields_1: number[], categoriy: number = -1): string {
			const aStrAttrs: string[] = [];
			let str = "";
			let extPropName = "";
			let subIndex = 0;
			let extPropIndexMin = 0;
			let extPropIndexMax = 0;
			if ((categoriy > <number>ItemCategories.JieZhi && categoriy < (<number>ItemCategories.ChongWu))
				|| (categoriy > <number>ItemCategories.WuQi_Dao && categoriy !== <number>ItemCategories.WuQi_NuJianTong)
				|| categoriy === <number>ItemCategories.ShouHuChong
				...
				if (20 === Global.MaxForgeLevel()) {
					showNodeWithLayout(this._groupStar0);
					showNodeWithLayout(this._groupStar1);
				} else {
					if (gd.ForgeLevel <= 15) {
						showNodeWithLayout(this._groupStar0);
						hideNodeWithLayout(this._groupStar1);
				...
				if (subIndex === ExtPropIndexes.AttackSpeed) {
					...
					if (equipFields_1[extPropIndexMin] !== 0.0 || equipFields_1[extPropIndexMax] !== 0.0) {
						...
						if (ForgeAddBasePersent > 0) {
							if (categoriy !== <number>ItemCategories.JieHunJieZhi && categoriy !== <number>ItemCategories.Decoration && categoriy !== <number>ItemCategories.Fashion) {
								minForgeAddBaseValueStr = Global.GetColorStringForNGUIText(ColorCode.zhuoYue1, `(+${minForgeAddBaseValue})`);
								maxForgeAddBaseValueStr = Global.GetColorStringForNGUIText(ColorCode.zhuoYue1, `(+${maxForgeAddBaseValue})`);
			...
			for (subIndex = ExtPropIndexes.IncreasePhyAttack; subIndex < ExtPropIndexes.Max; subIndex++) {
				if (equipFields_1[subIndex] !== 0) {
					...
					if (ExtPropIndexes.ExtPropIndexPercents[subIndex] === 1) {
						...
						if (categoriy === <number>ItemCategories.ChiBang && subIndex === <number>ExtPropIndexes.SubAttackInjurePercent) {
							// 翅膀不能强化
						} else if (categoriy === <number>ItemCategories.Fashion) {
							// 时装特殊处理
						} else {
							if (ForgeAddBasePersent > 0) {
								if (categoriy !== <number>ItemCategories.JieHunJieZhi && categoriy !== <number>ItemCategories.Decoration && categoriy !== <number>ItemCategories.Fashion) {
									...
									ForgeAddStr = Global.GetColorStringForNGUIText(ColorCode.zhuoYue1, `(+${nBase}%)`);
						...
						if (ForgeAddBasePersent > 0) {
							if (categoriy !== <number>ItemCategories.JieHunJieZhi && categoriy !== <number>ItemCategories.Decoration && categoriy !== <number>ItemCategories.Fashion) {
								...
								ForgeAddStr = Global.GetColorStringForNGUIText(ColorCode.zhuoYue1, `(+${nOrigin})`);
		 ...
		 */
		private GetZhuijiaAttributeStr(gd: NetMsg.IGoodsData, equipFields_1: number[]): string {
			const aStrAttrs: string[] = [];
			let str = "";
			let extPropName = "";
			let subIndex = 0;
		 ...
		 * @param nCategoriy 分类
		 */
		private GetXilianAttributeStr(gd: NetMsg.IGoodsData, nCategoriy: number): string {
			const aStrAttrs: string[] = [];
			let str = "";
			let key = 0;
			let value = 0;
			const valueLength = 5;
			const bIsMengchong = (nCategoriy === <number>ItemCategories.MengChongWu);
			if (gd.ExcellenceInfo <= 0 && bIsMengchong === false) return str;
			...
			if (!gd.WashProps) {
				...
				for (let i = 0; i < gd.WashProps.length; i += 2) {
					...
					if (ExtPropIndexes.ExtPropIndexPercents[key] === 1) {
						...
						if (bIsMengchong) value = value / 100;
						str += `${Loca.getLang(ExtPropIndexes.ExtPropIndexChineseNames[key])} +${value}`;
					}
					str += this.GetSpaceString(Math.max(valueLength - value.toString().length, 0));
					if (dict) str += this.GetUpLimitValueStr(dict, factor, key, value);
					aStrAttrs.push(str);
				}
			}
			return aStrAttrs.join("<br>");
		 ...
		 * @param currentValue 
		 */
		private GetUpLimitValueStr(dict: Map<number, number>, factor: number, key: number, currentValue: number): string {
			let upLimitValue = 0;
			let str = "";
			if (dict) {
				if (dict.has(key)) {
					...
					if (currentValue >= upLimitValue) {
						str += Global.GetColorStringForNGUIText(ColorCode.gray, Loca.getLang("已达上限"));
					} else {
						str += Global.GetColorStringForNGUIText(ColorCode.gray, Global.String.Format(Loca.getLang("最高上限 +{0}"), upLimitValue));
					}
				}
			}
			return str;
		 ...
		 * @param equipFields_1 属性列表
		 */
		private GetZhuanshengAttributeStr(gd: NetMsg.IGoodsData, equipFields_1: number[]): string {
			const aStrAttrs: string[] = [];
			let str = "";
			let extPropName = "";
			let subIndex = 0;
		 ...
		 * @param gd 装备数据
		 */
		private GetZhuoyueAttributeStr(gd: NetMsg.IGoodsData): string {
			const aStrAttrs: string[] = [];
			let propStr = "";
		 ...
		 * @param flag 卓越属性索引
		 */
		private GetZhuoyuePropStr(flag: number): string {
			let propsStr = "";
			let propsValue = "";
			const propsID = flag;
		 ...
		 * @param gd 装备数据
		 */
		private GetXingyunAttributeStr(gd: NetMsg.IGoodsData): string {
			let str = "";
			if (gd.Lucky <= 0) return str;
		 ...
		 * @param num 
		 */
		private GetSpaceString(num: number): string {
			let str = "";
			while (num > 0) {
				str += "  ";
				num--;
		 ...
		 * @param handTypes 
		 */
		private TipsCallBack(type: TipsOperationTypes, id: number = 0, handTypes: HandTypes = HandTypes.None) {
			...
			if (TipsOperationTypes.UseMengchong === type) {
				GTipServiceEx.TipsSender.DPSelectedItem(this, new DPSelectedItemEventArgs(type, id));
			}
