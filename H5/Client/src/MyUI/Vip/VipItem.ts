namespace MyUI {
	...
	* VipItem vip等级对应的数据 
	*/
	export class VipItem extends ui.Vip.VipItemUI {
		// UI操作执行回调函数
		dPSelectedItem: DPSelectedItemEventHandler;
		private currViplevels = 0;
		constructor() {
			super();
			this.init();
		 ...
		 * @returns {} 
		 */
		set currVipLevel(value: number) { this.currViplevels = value; }
		get currVipLevel() { return this.currViplevels; }
		/**
		 * 所有触发的点击响应
		 * @param component 
		 */
		private onClickComponent(component: Laya.Event) {
			const clickObj = component.target as Laya.Component;
		 ...
		 * @returns {} 
		 */
		initData(vipLevel: number, rewardFlag: number) {
			...
			rewardStrList.forEach((goodsStr, index) => {
				...
				const goodsData = Global.GetDummyGoodsDataMu(parseInt(goodsStrList[0]), parseInt(goodsStrList[3]), parseInt(goodsStrList[4]),
					parseInt(goodsStrList[6]), parseInt(goodsStrList[5]), parseInt(goodsStrList[2]), parseInt(goodsStrList[1]));
			...
			descList.forEach((desc, index) => {
				const vipQuanXianItem = new VipQuanXianItem();
		 ...
		 * @returns {} 
		 */
		refreshRewardFlag(rewardFlag: number): void {
