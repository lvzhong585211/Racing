namespace GameMode {
    ...
    class GameState {
        ...
        public UserToken: string;            // 用户令牌
        public UserIsAdult: number = 0;      // 用户是否成人
        public RoleRandToken: number = -1;   // 游戏服务器的随机令牌
        public RoleID: number = -1;          // 当前的角色ID
        public RoleSex: number = 0;          // 角色性别
        public RoleName: string = "";        // 角色名称
        public GmCommandState: number = 1;   // GM命令标记

        /** 角色是否已经正式登陆 */
        public PlayGame: boolean = false;
        /** 是否存在二级密码 */
        public HasSecondPassword: number = -1;
        /** 是否需要验证二级密码 */
        public NeedVerifySecondPassword: number = -1;
        /** 登录的游戏服务器信息 */
        public serverInfo: GameServerInfo;

        /** 当前本地玩家的角色数据 */
        public roleData: NetMsg.RoleData;
        /** 结婚状态数据 */
        public MarriageData: NetMsg.MarriageData;
        /** 对方结婚数据 */
        public OtherMarriageData: NetMsg.MarriageData_EX;

        /**
         * 查找对应NPCID的任务状态
         * @param npcID 指定要获取任务状态的NPCID
         */
        public getNPCTaskState(npcID: number): NetMsg.NPCTaskState {
            if (this.roleData.NPCTaskStateList) {
                for (let index = this.roleData.NPCTaskStateList.length - 1; index >= 0; --index) {
                    ...
                    if (taskState.NPCID === npcID)
                        return taskState as NetMsg.NPCTaskState;
                }
            }
            return null;
        }
    ...
    /** 慢速帧调用接口 */
    interface ISlowUpdate {
		/**
         * 1秒种最多执行5次的更新函数.适合不及时的逻辑
         * @param elapsedTime 上次调用以来经过的时间(秒)
         */
        slowUpdate(elapsedTime: number): void;
    }

    /** 是否实现了帧调用接口 */
    export function isFrameMoveImp(value: any): value is IFrameMove {
        return value.frameMove !== undefined;
    }

    /** 是否实现了慢速帧调用接口 */
    export function isSlowUpdateImp(value: any): value is ISlowUpdate {
     ...
     * @desc 游戏管理器,总入口
     */
    export class GameInstance extends ModeManager {

        private _chargeServerUrl: string = null;            // 充值服务器的地址
        public set chargeServerUrl(value: string) {
            this._chargeServerUrl = value;
        }

        private mPersistentLevel: Laya.Scene = null;        // 持久关卡,同时只能存在一个,即加载一个新的PersistentLevel时会卸载当前的.
        private mPersistentLevelUrl: string = null;          // 持久关卡的路径
        private mLevelLoadCount: number = 0;                 // 保存当前关卡的加载计数.注:只有当关卡计数相等时才表示加载成功的关卡有效,否则应该丢弃掉

        private mGameState: GameState = new GameState();    // 保存GameInstance相关的数据
        private mTcpToGameServer: Net.Socket4Message = null; // 到GameServer的Tcp连接

        private mUIMgr: MyUI.UIManager;                     // UI管理器
        private mTableMgr: tables.DataTableManager;         // 数据表管理器
        private mAtlasMgr: MyUI.AtlasManager;               // 图集管理器

        private mParticlesManager: Logic.ParticlesManager;  // 粒子管理器
         ...
         * @param eGameMode
         */
        protected createMode(eGameMode: EnumGameMode): ModeBase {
            let modeToCreate: ModeBase = null;
            switch (eGameMode) {
                case EnumGameMode.Logo:
                    modeToCreate = new GameMode.LogoMode();
                    break;
                case EnumGameMode.Loading:
                    modeToCreate = new LoadingMode();
                    break;
         ...
         */
        protected slowUpdate(elapsedTime: number) {
         ...
         */
        public async loadPersistentLevel(url: string, progress?: Laya.Handler, bForceLoad: boolean = false): Promise<boolean> {
            ...
            let loadingLevel: Laya.Scene = null;
        ...
        protected checkMemoryLeak(): boolean {
         ...
         */
        public async loginToGameServer(gsUrl: string, userId: string, userName: string, userToken: string, onError?: Function): Promise<Boolean> {
            ...
            let errInfo: string = null;
            const webTcp = new Net.Socket4Message();

            do {
                // 连接到GameServer
                const connectRet = await webTcp.connectByUrl(gsUrl);
                if (!connectRet) {
                ...
                // 偿试登录
                let loginInfo =
                    `${userId}:${userName}:${userToken}:${this.mGameState.RoleRandToken}:${Global.VersionInfo.appVersion}:${
                    this.mGameState.UserIsAdult}:`;
                ...
                // 等待登录返回
                const loginRet: string = await webTcp.WaitStringMessage(EMessageType.CMD_LOGIN, 2000);
                if (loginRet == null) {
         ...
         */
        private onRecvGameServerMsgPacket(uMsgType: EMessageType, msgReader: protobuf.Reader): boolean {
         ...
         */
        private onDisconnectFromGameServer() {
         ...
         */
        public sendDataToGS<T>(msgType: EMessageType, msgObject: T, encode?: { (msgObject: T, writer: protobuf.Writer) }) {
            this.mTcpToGameServer.sendData<T>(msgType, msgObject, encode);
        }
         ...
         * @param msgData
         */
        public sendStringToGS(msgType: EMessageType, msgData: string) {
         ...
         */
        public async WaitGSStringMessage(msgType: EMessageType, timeOut?: number): Promise<string> {
         ...
         * * 注: 此函数不应该同时大量使用,估计会有性能问题
         */
        public async WaitGSStringMsgFields(msgType: EMessageType, timeOut?: number): Promise<string[]> {
         ...
         * @return 返回值为拆分后的数据数组
         */
        public getStringMsgFields(msgReader: protobuf.Reader): string[] {
            if (msgReader == null) {
                return null;
            }
		 ...
		 * 向GS发送时间同步消息,并等待消息返回
		 */
        public async TimeSynchronization(): Promise<void> {
            if (!this.mTcpToGameServer) {
                return;
            }
            ...
            // 等待并解析同步时间消息
            const syncTimeInfo = await this.WaitGSStringMsgFields(EMessageType.CMD_SYNC_TIME);
            const clientOldTime = parseInt(syncTimeInfo[1]);	    // 客户端发送时间同步消息时的时间(毫秒)
            const clientNowTime = TimeManager.now();			    // 客户端的当前时间(毫秒)
            const serverTimeTicks: Long = Long.fromString(syncTimeInfo[2]); // 服务器端时间(Ticks)
            const pingTime = (clientNowTime - clientOldTime) / 2;       // 取得消息延迟,即本次同步时间的消息到服务器的时间(毫秒)
            let serverTime: number = serverTimeTicks.divide(10000).toNumber();  // 把Ticks转换成毫秒. ,见C#的DateTime.Ticks            
