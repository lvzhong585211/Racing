namespace GameMode {
    ...
    class RolesViewComponent extends Laya.Script {
        public mActorsView = new Map<number, Logic.CharacterView>();
    }

    /**
     * @desc 选择角色模块
     */
    export class SelectorMode extends ModeBase {
        private m_vewSelector: MyUI.RoleSelectorView;                   // 选择角色界面
        private m_dtRoleDataList: MyUI.RoleSelectorItemDataList;        // 角色列表数据

        private scene: Laya.Scene;                                      // 场景
        private mActorsView: Map<number, Logic.CharacterView>;         // 角色展示

        private DirectLogin: boolean = false;                           // 是否直接登录
         ...
         * @param index
         */
        private onSelectedActor(roleID: number) {
        ...
        private async _loadSceneAndActors() {
            let rolesViewComponent: RolesViewComponent = null;

            // 查找已经存在的角色模型映射表
            if (gameIns.persistentLevel != null) {
                rolesViewComponent = gameIns.persistentLevel.getScriptByType(RolesViewComponent) as RolesViewComponent;
            ...
            if (!this.mActorsView) {
                this.mActorsView = new Map<number, Logic.CharacterView>();
            }
            ...
            const loadSuccess = await gameIns.loadPersistentLevel("xuanjue");
            if (!loadSuccess) {
                return;        // 加载场景失败,下面的逻辑不再处理了.
            }

            if (!rolesViewComponent) { // 存储共享模型的脚本还不存在,添加一个
                rolesViewComponent = gameIns.persistentLevel.addScript(RolesViewComponent) as RolesViewComponent;
        ...
        public onRecvGameServerMsgPacket(uMsgtype: EMessageType, msgReader: protobuf.Reader): boolean {
            switch (uMsgtype) {
                ...
                case EMessageType.CMD_SPR_GETROLEUSINGGOODSDATALIST: { // 角色使用的物品列表
                    ...
                    Global.changePlayerEquip(charView, roleData.GoodsDataList, Logic.CharacterView.CalcSlotTypeLogin);       // 换装
                    return true;
                }
         ...
         * @param aStrField 
         */
        private onPreremoveRoleMsg(aStrField: string[]) {
         ...
         * @param aStrField 
         */
        private onRemoveRoleMsg(aStrField: string[]) {
            const nRoleID = parseInt(aStrField[0]);
