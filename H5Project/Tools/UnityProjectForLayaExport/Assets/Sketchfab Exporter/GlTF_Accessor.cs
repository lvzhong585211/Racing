...
public class GlTF_Accessor : GlTF_Writer {
	...
	/// <returns></returns>
	public long AlignOffset()
	{
		long align = GetComponentSize();
		long orgOffset = bufferView.currentOffset + bufferView.byteOffset;
		long retOffset = (orgOffset + align - 1) / align * align;
		if (retOffset - orgOffset > 0)
		{
			bufferView.PopulateNop((int)(orgOffset - retOffset));
		}
		return retOffset;
	}
