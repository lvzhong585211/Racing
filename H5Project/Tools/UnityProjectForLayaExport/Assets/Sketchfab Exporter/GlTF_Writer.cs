...
public class GlTF_Writer {
	...
	public static string exporterVersion = "0.9.1";
	public static Regex rgx = new Regex("[^a-zA-Z0-9 -_.]");

	public static bool exportMesh = true;			// 导出模型
	public static bool exportAnimation = true;		// 导出动画
	public static bool clearNoUsedNode = false;		// 是否清除没有引用到的节点,以节省空间.
	...
	private long Align4(long orgOffset)
	{
		return (orgOffset + 4 - 1) / 4 * 4;
	}

	private void WriteViewToBin(GlTF_BufferView binView)
	{
		binFile.Seek(binView.byteOffset, SeekOrigin.Begin);
		binView.memoryStream.WriteTo(binFile);
	}
	...
	public virtual void Write () {
		...
		// 计算偏移量,需要字节对齐
		long bufferByteLength = 0;
		GlTF_BufferView preBufferView = null;
		foreach (var bufView in bufferViews)
		{
			if (bufferByteLength != 0)
			{
				bufferByteLength = Align4(bufferByteLength);
			}
			bufView.byteOffset = bufferByteLength;
			bufferByteLength += bufView.byteLength;
			bufView.bin = binary;
		}
