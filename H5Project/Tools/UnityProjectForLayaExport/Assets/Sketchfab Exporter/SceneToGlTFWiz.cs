...
{
	public UInt32 exportOption = 0xFFFFFF;	// 导出数据的选项
	...
	public void ExportCoroutine(string path, Preset presetAsset, bool buildZip, bool exportPBRMaterials, bool doConvertImages = true)
	{
	...
	/// <returns></returns>
	public bool nodeIsUsed(GlTF_Node node)
	{
		if (node.skinIndex >= 0 && GlTF_Writer.exportMesh)
			return true;	// 导出模型,且此节点绑定了模型
	...
	public IEnumerator Export(string rootPath, Preset presetAsset, bool buildZip, bool exportPBRMaterials, bool doConvertImages = false, bool exportTangent = false)
	{
		...
		{
			Debug.LogError("没有选中对象,无法继续导出");
			yield break;
		}

		if (string.IsNullOrEmpty(rootGameObject.name))
		{
			Debug.LogError("要导出的对象没有名称,无法继续导出");
			yield break;
		}

		var prefabType = PrefabUtility.GetPrefabType(rootGameObject);
		if (prefabType != PrefabType.PrefabInstance && prefabType != PrefabType.ModelPrefabInstance)
		{
			Debug.LogErrorFormat("要导出对象({0})的类型({1})不是prefab,无法导出", rootGameObject.name, prefabType.ToString());
			yield break;
		}
		...
		if (rootGameObject == null)
		{
			...
			correctionNode.id = rootGameObject.GetInstanceID().ToString();
			correctionNode.name = rootGameObject.name;
		...
		// 导出的文件名
		path += rootGameObject.name + ".gltf";

		// 是否只导出Mesh
		bool exportMeshOnly = (GlTF_Writer.exportMesh && !GlTF_Writer.exportAnimation);
		if(exportMeshOnly)
			writer.extraString.Add("meshOnly", "true");

		// first, collect objects in the scene, add to lists
		Transform[] transforms = rootGameObject.GetComponentsInChildren<Transform>(false);
		...
		foreach (Transform tr in trs)
		{
			...
			Mesh m = GetMesh(tr);
			if (m != null && GlTF_Writer.exportMesh)
			{
				GlTF_Mesh mesh = new GlTF_Mesh();
				mesh.unityMesh = m;
				mesh.name = m.name; //GlTF_Writer.cleanNonAlphanumeric(GlTF_Mesh.GetNameFromObject(m) + tr.name);
				...
				if (GlTF_Writer.exportMesh && m.boneWeights.Length > 0)
				{
				...
				if (exportTangent && m.tangents.Length > 0)
				{
			...
			// Parse animations
			if (GlTF_Writer.exportAnimation)
			{
			...
			if (GlTF_Writer.exportMesh && skinMesh != null && skinMesh.enabled && checkSkinValidity(skinMesh, trs) && skinMesh.rootBone != null)
			{
		...
		if (GlTF_Writer.exportMesh && GlTF_Writer.meshes.Count == 0)
		{
			Debug.LogError("没有找到需要导出的模型,请确认某些GameObject上带有 SkinMeshRenderer 或 MeshRenderer 组件!!!");
			yield return false;
		}

		// 优化没有用到的节点. 注:这会清除掉没有被皮肤或动作直接引用到的节点.这会导致某些末端的节点被去掉.
		if (GlTF_Writer.clearNoUsedNode)
		{
			// 收集所有被皮肤引用到的骨骼
			HashSet<Transform> bones = new HashSet<Transform>();
			if (GlTF_Writer.exportMesh)
			{
				foreach (Transform tr in trs)
				{
			...
			var nodes = GlTF_Writer.nodes;
			for (var index = nodes.Count - 1; index >= 0; --index)
			{
				var toCheckNode = nodes[index];
				...
				do
				{
					if (GlTF_Writer.exportMesh)
					{
						if (toCheckNode.skinIndex >= 0 && GlTF_Writer.exportMesh)
							break; // 导出模型,且此节点绑定了模型

						if (bones.Contains(toCheckNode.unityTranform))
						{
							break; // 被模型引用到了
						}
					}
				...
				{	// 这个节点没有被引用到,删除掉吧
					...
					GlTF_Writer.nodeNames.RemoveAt(index);
					trs.Remove(toCheckNode.unityTranform);
				}
			}
		}
		...
		// 查找并添加根节点与子节点
		{
			...
			for (var index = nodes.Count - 1; index >= 0; --index)
			{
				...
				if (exportMeshOnly)
				{	// 如果只是导出模型,我们只需要带Mesh的节点即可
					if(node.skinIndex < 0)
						continue;
				}

				if (node.unityTranform.parent == null || !trs.Contains(node.unityTranform.parent))
				{
				...
				{	// 只导出Mesh时,不需要带子节点
					// 添加子节点
					foreach (Transform t in node.unityTranform)
					{
						if (trs.Contains(t) && t.gameObject.activeInHierarchy)
		...
		Debug.LogFormat("<color=green>节点数: {0} \t 模型数:{1} \t 纹理数:{2} \t 动画数:{3} </color>", GlTF_Writer.nodes.Count, GlTF_Writer.meshes.Count, GlTF_Writer.textures.Count,
			GlTF_Writer.animations.Count);
